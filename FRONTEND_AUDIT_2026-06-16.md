...
| Code structure (TD) | 6/10 | Composer and Autopilot route files are now thin shells, but their extracted feature screen implementations are still too large. |
...
4. **Monster feature screens remain the main maintainability risk.**
   ...
   - The route-shell extraction reduces route-level blast radius and keeps lazy route contracts stable. The next maintainability step is panel-level decomposition into editor/forms, media/upload, preview, actions, diagnostics, and route-specific hooks.
