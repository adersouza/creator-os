...
class RenderQueue:
    ...
    def enqueue(self, *, job_key: str, command: list[str], cwd: Path,
        ...
        self.conn.execute(
            ...
            """,
            (job_id, job_key, json.dumps(command), str(cwd), max_attempts, int(time.time())),
        )
        row = self.conn.execute("SELECT job_id FROM queue_jobs WHERE job_key = ?", (job_key,)).fetchone()
        self.conn.commit()
    ...
    def claim(self, worker_id: str) -> dict[str, Any] | None:
        ...
        self.conn.commit()
        row = self.conn.execute("SELECT * FROM queue_jobs WHERE job_id = ?", (row["job_id"],)).fetchone()
    ...
    def finish(self, job_id: str, status: str, *, error_text: str | None = None) -> None:
        ...
        self.conn.execute(
            ...
            WHERE job_id = ?
            """,
            (status, int(time.time()), error_text[-2000:] if error_text else None, job_id),
        )
        self.conn.commit()
...
class RedisRenderQueue:
    ...
    def __init__(self, root: Path, url: str | None = None):
        try:
            ...
            raise RuntimeError("Redis queue backend requires `python3 -m pip install redis`") from e
        self.root = Path(root).resolve()
    ...
    def heartbeat(self, job_id: str, worker_id: str) -> None:
        self.redis.hset(self._key(f"job:{job_id}"), "heartbeat_at", str(int(time.time())))
    ...
    def recover_stale(self, stale_after_sec: int = 300) -> int:
        ...
        for job_id in self.redis.smembers(self._key("jobs")):
            ...
            if row.get("status") not in {"claimed", "running"}:
                continue
            heartbeat = int(row.get("heartbeat_at") or row.get("claimed_at") or row.get("started_at") or 0)
            if heartbeat >= cutoff:
                continue
    ...
    def _row_to_job(self, row: dict[str, str]) -> dict[str, Any]:
        ...
        data["command"] = json.loads(data.pop("command_json", "[]"))
        for key in ("attempts", "max_attempts", "created_at", "claimed_at", "started_at", "ended_at", "heartbeat_at"):
