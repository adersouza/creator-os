...
def _sidecar_paths(output_path: Path) -> dict[str, Path]:
    return {
        "virality": output_path.with_suffix(
            output_path.suffix + ".virality_report.json"
        ),
        "video_analysis": output_path.with_suffix(
            output_path.suffix + ".video_analysis.json"
        ),
    }
...
def _find_outputs(
    root: Path, *, clip: str | None = None, output_path: Path | None = None
) -> list[Path]:
    ...
    clip_dirs = (
        ...
        if clip
            ...
            path
            for path in sorted(proc.iterdir())
...
def _report_record(
    output_path: Path, reports: Sequence[str], *, status: str
) -> dict[str, Any]:
...
def _write_request_manifest(
    records: list[dict[str, Any]], *, provider_calls: int
) -> None:
...
def _normalize_virality_report(
    ...
    return {
        **report,
        ...
        or f"virality_{time.time_ns()}",
    }


def _normalize_video_analysis_report(
    report: dict[str, Any], *, output_path: Path, provider: str
) -> dict[str, Any]:
    features = report.get("winnerDnaFeatures")
    if not isinstance(features, dict):
        features = (
            report.get("winner_dna_features")
            if isinstance(report.get("winner_dna_features"), dict)
            else {}
        )
    scores = report.get("scores") if isinstance(report.get("scores"), dict) else {}
    pattern_card = (
        report.get("patternCard") if isinstance(report.get("patternCard"), dict) else {}
    )
    pattern_card = {
        ...
        "formatType": pattern_card.get("formatType")
        or features.get("camera")
        or "unknown",
        "hookType": pattern_card.get("hookType")
        or features.get("hook_type")
        or "unknown",
        **pattern_card,
    }
    return {
        **report,
        ...
        if isinstance(report.get("media"), dict)
...
def write_operator_reports(
    ...
    *,
    ...
    return {
        ...
        "sidecars": {
            key: str(path) for key, path in sidecars.items() if key in written
        },
        "provider": provider,
        "providerCalls": 0,
        "estimatedCostUsd": 0,
...
def _run_provider_command(
    ...
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"provider command failed for {output_path.name} {report}: {completed.stderr.strip()}"
        )
    try:
        payload = json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"provider command returned invalid JSON for {output_path.name} {report}"
        ) from exc
    if not isinstance(payload, dict):
        raise ValueError(
            f"provider command returned non-object JSON for {output_path.name} {report}"
        )
    return payload
    ...
    *,
    ...
    for output in outputs:
        if provider_command is None:
            records.append(
                _report_record(
                    output, selected_reports, status="operator_input_required"
                )
            )
            continue
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Request or write default-off reel analysis report sidecars."
    )
