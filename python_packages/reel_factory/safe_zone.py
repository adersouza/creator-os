    ...
    if not width or not height:
        ...
        for label, box in (("caption", caption), ("face", face)):
            if not box:
                continue
            collides = (
                box.y < top or box.bottom > bottom or box.x < left or box.right > right
            )
        ...
        if caption:
            bottom_ui_risk = max(
                0.0, min(1.0, (caption.bottom - bottom) / max(1.0, height - bottom))
            )
            right_rail_risk = max(
                0.0, min(1.0, (caption.right - right) / max(1.0, width - right))
            )
