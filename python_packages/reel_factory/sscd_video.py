...
def model():
    ...
    if _model is None:
        ...
        _model = torch.jit.load(MODEL_PATH); _model.eval()
    return _model
...
def main(source: str, out_dir: str) -> None:
    ...
    if not src.exists() or not od.is_dir():
        print(f"missing src or dir"); sys.exit(1)
    ...
    with tempfile.TemporaryDirectory() as td_str:
        ...
        for v in videos:
            try:
                ...
                print(f"{v.name}: error {e}"); continue
        ...
        if avgs:
            print(f"  max-sim summary:   mean={sum(avgs)/len(avgs):.4f}  "
                  f"min={min(avgs):.4f}  max={max(avgs):.4f}")


if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("usage: python3 sscd_video.py /path/to/source.mp4 /path/to/02_processed/clip_001", file=sys.stderr)
        sys.exit(2)
