...
class BenchmarkHarnessTests(unittest.TestCase):
    def test_create_benchmark_plan_requires_shared_motion_and_writes_two_conditions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            enhanced.write_text(json.dumps({
            ...
            result = create_benchmark_plan(root, [{
                "reel_id": "reel_001",
            ...
            self.assertEqual(plan["schema"], "reel_factory.visual_direction_benchmark.v1")
            self.assertEqual(plan["conditions"], ["neutral", "enhanced"])
            self.assertEqual(plan["reels"][0]["sharedKlingMotionPrompt"], "slow push in, subtle hip sway, steady handheld pacing")
            self.assertEqual(set(plan["reels"][0]["variants"]), {"neutral", "enhanced"})
            self.assertEqual(plan["reels"][0]["variants"]["neutral"]["gridStem"], "reel_001_neutral_grid")
            self.assertEqual(plan["reels"][0]["variants"]["enhanced"]["gridStem"], "reel_001_enhanced_grid")
            self.assertEqual(len(plan["reels"][0]["variants"]["neutral"]["panelAnimationStems"]), 6)
            self.assertEqual(len(plan["reels"][0]["variants"]["enhanced"]["panelAnimationStems"]), 6)
