...
from audio_provider import trending_cml_path, local_winners_path, watch_list_path
from audio_refresh import refresh_cml_from_export, refresh_from_review, refresh_latest_cml_export
...
class AudioRefreshTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for idx in range(1, 27):
                candidates.append({
                    ...
                    "source": "tiktok_cml" if idx < 24 else "local_archive_audio_id",
            ...
            self.assertEqual(len(json.loads(trending_cml_path(root).read_text())["tracks"]), 6)
            self.assertEqual(len(json.loads(local_winners_path(root).read_text())["tracks"]), 3)
            self.assertEqual(len(json.loads(watch_list_path(root).read_text())["tracks"]), 9)
    ...
    def test_refresh_cml_from_export_filters_to_allowed_style_tags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            export.write_text(json.dumps({
                "tracks": [
                    {"track_id": "a", "track_name": "Runway", "trend_rank": 2, "tags": ["fashion"]},
                    {"track_id": "b", "track_name": "Dirge", "trend_rank": 1, "tags": ["sorrow"]},
                    {"track_id": "c", "track_name": "Chill Fit", "trend_rank": 3, "tags": ["chill"]},
                ]
            }), encoding="utf-8")
    ...
    def test_refresh_latest_cml_export_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            export.write_text(json.dumps({
                "tracks": [
                    {"track_id": "a", "track_name": "Runway", "trend_rank": 1, "tags": ["fashion"]},
                ]
            }), encoding="utf-8")
            ...
            self.assertEqual(len(json.loads(trending_cml_path(root).read_text())["tracks"]), 1)
