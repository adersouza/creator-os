...
class NextSliceTests(unittest.TestCase):
    ...
    def test_segment_mode_uses_segment_probe_when_score_is_clear(self):
        calls = [
            ...
            PlacementSummary("left", {"side_left": 20.0, "side_right": 100.0}, 3, "left"),
        ]
    ...
    def test_segment_mode_prevents_short_left_right_flip(self):
        calls = [
            PlacementSummary("right", {"side_right": 10.0, "side_left": 100.0}, 3, "right"),
            PlacementSummary("left", {"side_left": 1.0, "side_right": 100.0}, 3, "left"),
        ]
    ...
    def test_segment_mode_moves_repeated_lane_for_retention_when_safe(self):
        calls = [
            PlacementSummary("top", {"top": 10.0, "bottom": 18.0, "center": 25.0}, 3, "top"),
            PlacementSummary("top", {"top": 10.0, "bottom": 18.0, "center": 25.0}, 3, "top"),
            PlacementSummary("top", {"top": 10.0, "bottom": 18.0, "center": 25.0}, 3, "top"),
        ]
    ...
    def test_long_side_caption_falls_back_to_lane(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            long_text = "this is a very long caption that should wrap into a tall side caption"
            plan = CaptionSegmentPlan(Path(tmp) / "a.png", 0.0, 2.0, long_text, "bottom")
            resolved = __import__("asyncio").run(resolve_segment_bands(
                Path("clip.mp4"),
                segments=[plan, CaptionSegmentPlan(Path(tmp) / "b.png", 2.0, 4.0, "short", "bottom")],
                source_band="bottom",
                placement_mode="segment",
                placement_signals="basic",
                recipe=Recipe("v01_original"),
    ...
    def test_segment_mode_changes_job_key_but_source_default_does_not(self):
        ...
        explicit_source_key = compute_job_key("hash", "caption", recipe, placement_mode="source")
        segment_key = compute_job_key("hash", "caption", recipe, placement_mode="segment")
    ...
    def test_metrics_import_and_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with metrics.open("w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=["filename", "views", "likes", "platform"])
                writer.writeheader()
                writer.writerow({"filename": out.name, "views": "1000", "likes": "50", "platform": "ig"})
                writer.writerow({"filename": "unknown.mp4", "views": "5", "likes": "1", "platform": "ig"})
    ...
    def test_export_approved_outputs_json_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest.upsert_video("clip_001", src, "src-hash", 2.5)
            manifest.add_variation("clip_001", recipe, "caption", out, key, 2.5, target_ratio="4:5")
            manifest.set_review_state(out.name, "approved")
            manifest.save()

            result = export_approved(root, account="acct", platform="ig", date="2026-05-13")

            self.assertEqual(result["count"], 1)
            self.assertEqual(result["items"][0]["target_ratio"], "4:5")
            self.assertEqual(result["items"][0]["audio_workflow"]["warning"], "missing_audio_intent")
    ...
    def test_export_approved_preserves_audio_intent_sidecar(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["items"][0]["audio_intent"]["status"], "recommended")
            self.assertEqual(result["items"][0]["generated_asset_lineage"]["source"]["patternCardId"], "pattern_1")
            self.assertEqual(result["items"][0]["platform_readiness"]["status"], "warn")
            self.assertTrue(result["items"][0]["audio_workflow"]["local_muxing_is_preview_only"])
