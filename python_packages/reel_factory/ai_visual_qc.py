...
def _extract_frames(video: Path, out_dir: Path, count: int = 6) -> list[tuple[float, Path]]:
...
def analyze_video(video: Path) -> AiQcRecord:
    ...
    with tempfile.TemporaryDirectory(prefix="ai_qc_") as tmp:
        ...
        if not frames:
            return record_from_scores(video.name, str(video), {"opencv_available": 1, "frame_count": 0})
