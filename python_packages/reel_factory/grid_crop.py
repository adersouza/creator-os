...
def extract_frame(source_video: Path, out_path: Path, *, time_sec: float = 0.25) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-ss", f"{max(0.0, time_sec):.3f}", "-i", str(source_video),
        "-frames:v", "1", "-q:v", "2", str(out_path),
    ], check=True)
    return out_path


def preset_boxes(width: int, height: int, *, columns: int, rows: int,
                 inset: int = 0) -> list[dict[str, Any]]:
...
def detect_visible_content_box(image_path: Path, *, threshold: int = 18) -> tuple[int, int, int, int]:
...
def infer_image_grid(width: int, height: int, *,
                     columns: int | None = None,
                     rows: int | None = None) -> dict[str, Any]:
    if columns and rows:
        return {"columns": int(columns), "rows": int(rows), "confidence": "operator_override"}
    ratio = width / height if height else 1.0
...
def _grid_boundary_score(image: Image.Image, content_box: tuple[int, int, int, int],
                         *, columns: int, rows: int) -> float:
...
def infer_image_grid_from_pixels(image: Image.Image, content_box: tuple[int, int, int, int], *,
                                 columns: int | None = None,
                                 rows: int | None = None) -> dict[str, Any]:
    ...
    for candidate_columns, candidate_rows in candidates:
        if candidate_columns == 1 and candidate_rows == 1:
            ...
            seam_scores = [
                ...
                for item in (seams.get("verticalSeams") or []) + (seams.get("horizontalSeams") or [])
            ]
            average_seam_score = sum(seam_scores) / len(seam_scores) if seam_scores else 0.0
            ...
            seam_count_reward = len(seam_scores) * 2.0 if seams.get("confidence") == "high" else 0.0
            panel_penalty = (candidate_columns * candidate_rows) * 0.05
            score = max(seam_score, boundary_score) + (average_seam_score * 0.25) + seam_count_reward - panel_penalty
...
def _projection_profile(image: Image.Image, content_box: tuple[int, int, int, int], *,
                        axis: str) -> np.ndarray:
    left, top, right, bottom = content_box
...
def _best_seam_near(profile: np.ndarray, *, expected: int, window: int) -> dict[str, Any]:
    if profile.size == 0:
...
def detect_grid_seams(image: Image.Image, content_box: tuple[int, int, int, int], *,
                      columns: int, rows: int) -> dict[str, Any]:
    left, top, right, bottom = content_box
    width = right - left
    height = bottom - top
    ...
    expected_x_lines = [int(round(left + width * col / columns)) for col in range(columns + 1)]
    expected_y_lines = [int(round(top + height * row / rows)) for row in range(rows + 1)]
    if not snapped:
...
def image_panel_crop_boxes(width: int, height: int, *, columns: int, rows: int,
                           content_box: tuple[int, int, int, int] | None = None,
                           inset: int = 2,
                           seam_lines: dict[str, list[int]] | None = None) -> list[dict[str, Any]]:
    ...
    if seam_lines:
        x_lines = [max(0, min(width, int(value))) for value in seam_lines.get("xLines", [])]
        y_lines = [max(0, min(height, int(value))) for value in seam_lines.get("yLines", [])]
...
def crop_image_grid_panels(image_path: Path, out_dir: Path, *,
                           columns: int | None = None,
                           rows: int | None = None,
                           smart: bool = True,
                           inset: int = 2,
    ...
    with Image.open(image_path) as raw:
        im = raw.convert("RGB")
        width, height = im.size
        content_box = detect_visible_content_box(image_path) if smart else (0, 0, width, height)
        ...
        seam_detection = detect_grid_seams(
            im,
            content_box,
            ...
            "confidence": "high",
            "snapped": False,
            "xLines": [int(round(width * col / grid["columns"])) for col in range(grid["columns"] + 1)],
            "yLines": [int(round(height * row / grid["rows"])) for row in range(grid["rows"] + 1)],
        }
        ...
        confidence = _manifest_confidence(str(grid.get("confidence") or "review"), seam_detection)
        panel_crops = []
        stem = prefix or image_path.stem
        for box in crop_boxes:
            panel = int(box["id"])
            out = out_dir / f"{stem}_panel_{panel:02d}.png"
            im.crop((box["x"], box["y"], box["x"] + box["w"], box["y"] + box["h"])).save(out)
            panel_crops.append({
                "panel": panel,
                "label": box["label"],
...
def validate_boxes(boxes: list[dict[str, Any]], *, width: int, height: int) -> list[dict[str, Any]]:
...
def build_crop_plan(root: Path, *, stem: str, source_video: Path,
                    frame_time: float = 0.25, columns: int | None = None,
                    rows: int | None = None, boxes: list[dict[str, Any]] | None = None,
                    render_mode: str = "fit_nocrop") -> dict[str, Any]:
    info = probe_video(source_video)
    if columns is None or rows is None:
        columns, rows = infer_grid_preset(info["width"], info["height"])
    box_list = boxes if boxes is not None else preset_boxes(info["width"], info["height"], columns=columns, rows=rows)
    now = int(time.time())
    return {
        ...
        "sourceDimensions": {"width": info["width"], "height": info["height"], "duration": info["duration"]},
...
def crop_video_with_box(source_video: Path, out_path: Path, box: dict[str, Any]) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def render_fit_nocrop(root: Path, *, source_video: Path, caption: str, out_path: Path) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def write_panel_lineage(root: Path, *, clip_path: Path, plan: dict[str, Any],
                        plan_path: Path, panel_video: Path, box: dict[str, Any],
                        panel_index: int) -> Path:
    payload = {
...
def install_cropped_panel(root: Path, *, plan: dict[str, Any], plan_path: Path,
                          panel_video: Path, box: dict[str, Any],
                          panel_index: int, caption: str) -> dict[str, Any]:
...
def write_review_sheet(root: Path, *, source_stem: str, installed: list[dict[str, Any]]) -> Path | None:
    if not installed:
        return None
    thumb_dir = root / "project_data" / "grid_crop_outputs" / source_stem / "_review_frames"
    ...
    for idx, row in enumerate(installed, start=1):
        ...
        subprocess.run([
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
            "-ss", "0.250", "-i", str(src), "-frames:v", "1",
            "-vf", "scale=360:-1", "-q:v", "3", str(thumb),
        ], check=True)
        thumbs.append(thumb)
    ...
    for idx, im in enumerate(images):
        ...
        im.close()
    out = root / "project_data" / "grid_crop_outputs" / source_stem / "_manual_grid_crop_review_sheet.jpg"
    sheet.save(out, quality=92)
    return out


def render_plan(root: Path, *, stem: str, captions: list[str] | None = None,
                render_captions: bool = True) -> dict[str, Any]:
    ...
    for box in plan.get("boxes") or []:
        ...
        panel_video = crop_video_with_box(source_video, crop_dir / f"{stem}_panel_{panel_index:02d}.mp4", box)
        ...
        if render_captions:
            out = root / "02_processed" / record["stem"] / f"{record['stem']}_h00_v09_caption_fit_nocrop_light.mp4"
            render_fit_nocrop(root, source_video=Path(record["source_path"]), caption=caption, out_path=out)
...
def preview_panel_image(root: Path, *, stem: str, panel_id: int) -> Path:
    ...
    if not frame.exists():
        extract_frame(Path(plan["sourceVideoPath"]), frame, time_sec=float(plan.get("frameTime") or 0.25))
    boxes = {int(b.get("id")): b for b in plan.get("boxes") or []}
    box = boxes[panel_id]
    out = root / "project_data" / "grid_crop_previews" / f"{stem}_panel_{panel_id:02d}.jpg"
    out.parent.mkdir(parents=True, exist_ok=True)
    with Image.open(frame) as im:
        im.crop((box["x"], box["y"], box["x"] + box["w"], box["y"] + box["h"])).save(out, quality=92)
    return out
