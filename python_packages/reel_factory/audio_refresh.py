...
def _load_json_or_csv(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise ValueError(f"{path} must contain a list or an object with tracks/items/candidates")
...
def _normalize_track(record: dict[str, Any], *, default_source: str,
...
def _write_pool(path: Path, *, pool: str, tracks: list[dict[str, Any]],
...
def refresh_from_review(root: Path, review_path: Path) -> dict[str, Any]:
    ...
    by_index = {int(row.get("review_index") or idx): row for idx, row in enumerate(rows, start=1)}
    cml = [
    ...
    local = [
        _normalize_track(by_index[idx], default_source="local_archive_audio_id", pool="local_winners")
    ...
    _write_pool(trending_cml_path(root), pool="cml_primary", tracks=cml, source_path=review_path)
    _write_pool(local_winners_path(root), pool="local_winners", tracks=local, source_path=review_path)
    _write_pool(watch_list_path(root), pool="watch_list", tracks=watch, source_path=review_path)
    result = {"source": "review", "cml": len(cml), "local_winners": len(local), "watch_list": len(watch)}
    _append_refresh_log(root, result)
    return result


def refresh_cml_from_export(root: Path, export_path: Path, *, limit: int = 100) -> dict[str, Any]:
    allowed = set(DEFAULT_ALLOWED_TAGS)
    tracks = []
    for record in _load_json_or_csv(export_path):
        track = _normalize_track(record, default_source="tiktok_cml", pool="cml_primary")
        if allowed and not (set(track.get("tags") or []) & allowed):
            continue
        tracks.append(track)
    tracks.sort(key=lambda row: (row.get("trend_rank", 999999), row["track_name"]))
    tracks = tracks[:limit]
    _write_pool(trending_cml_path(root), pool="cml_primary", tracks=tracks, source_path=export_path)
    result = {"source": "official_cml_export", "cml": len(tracks), "path": str(export_path)}
    _append_refresh_log(root, result)
    return result


def refresh_latest_cml_export(root: Path, *, drop_dir: Path | None = None,
    ...
    if not candidates:
        result = {"source": "official_cml_drop_dir", "status": "no_export_found", "drop_dir": str(inbox)}
        _append_refresh_log(root, result)
        return result
    latest = candidates[0]
...
def main() -> int:
    ...
    ap.add_argument("--latest-cml-export", action="store_true",
                    help="import the newest .json/.csv official CML export from the drop directory")
    ap.add_argument("--drop-dir", type=Path,
    ...
    if args.cml_export:
        results.append(refresh_cml_from_export(args.root, args.cml_export, limit=args.limit))
    if args.latest_cml_export:
        results.append(refresh_latest_cml_export(args.root, drop_dir=args.drop_dir, limit=args.limit))
    if not results:
