...
def generate_thumbnails(
    root: Path, *, clip: str | None = None, overwrite: bool = False
) -> dict:
    ...
    for video in candidates:
        ...
        try:
            created.append(
                str(generate_thumbnail(video, ffmpeg=ffmpeg, overwrite=overwrite))
            )
...
def main() -> int:
    ...
    print(
        json.dumps(
            generate_thumbnails(
                Path(args.root), clip=args.clip, overwrite=args.overwrite
            ),
            indent=2,
        )
    )
