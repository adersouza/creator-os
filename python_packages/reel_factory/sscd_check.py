...
Usage:
    python3 sscd_check.py /path/to/02_processed/clip_001
"""
    ...
    with tempfile.TemporaryDirectory() as td:
        for t in SAMPLE_TIMES:
            ...
            subprocess.run([
                ...
                "-ss", f"{t:.3f}", "-i", str(mp4), "-frames:v", "1",
                "-vf", "scale=320:-1",
                "-y", str(png),
            ], check=True)
...
def main(clip_dir: str) -> None:
    ...
    print("-" * 86)
    for row in rows:
        print(f"{row['recipe']:<18} {row['phash']:>8.2f} {row['dhash']:>8.2f} {row['whash']:>8.2f}  "
    ...
    print(f"  mean: {sum(avgs)/len(avgs):.2f}   min: {min(avgs):.2f}   max: {max(avgs):.2f}")
    print(f"  near-duplicate (<5):   {sum(1 for a in avgs if a < 5)}/{len(avgs)}")
    print(f"  modified (5..10):      {sum(1 for a in avgs if 5 <= a < 10)}/{len(avgs)}")
    print(f"  diff (10..20):         {sum(1 for a in avgs if 10 <= a < 20)}/{len(avgs)}")
    print(f"  novel (>20):           {sum(1 for a in avgs if a >= 20)}/{len(avgs)}")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: python3 sscd_check.py /path/to/02_processed/clip_001", file=sys.stderr)
        sys.exit(2)
    main(sys.argv[1])
