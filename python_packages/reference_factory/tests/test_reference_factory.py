...
from reference_factory.caption_adaptation import (
...
from reference_factory.learning import (
...
from reference_factory.media import (
    ffprobe_video,
    parse_fps,
    probe_videos,
    sample_frames,
...
from reference_factory.patterns import (
    analyze_patterns,
...
GOOD_IMAGE_PROMPT_JSON = {
    ...
    "composition": {
        ...
        "pose": "side profile phone covering face",
    },
    ...
    "environment": {
        "setting": "white bedroom",
        "details": ["black mirror frame", "bed"],
    },
    "lighting_and_camera": {
        "lighting": "soft natural daylight",
        "camera_feel": "iPhone mirror selfie",
    },
    "constraints": {
...
def write_higgsfield_prompt_pair(
    data_root: Path, reference_id: str = "ref_001"
) -> None:
    ...
    image = {
        ...
        "modelProfile": "Stacey",
        "mainPrompt": json.dumps(
            {
                key: value
                for key, value in GOOD_IMAGE_PROMPT_JSON.items()
                if key != "promptMode"
            }
        ),
...
def create_accepted_proof_bundle(root: Path) -> Path:
    ...
    (bundle / "generated_asset_lineage.json").write_text(
        json.dumps(
            {
    ...
    (bundle / "proof_completion_report.json").write_text(
        json.dumps(
            {
    ...
    (bundle / "goal_completion_audit.json").write_text(
        json.dumps(
            {
...
def test_scan_indexes_account_structure_and_marks_other(tmp_path: Path) -> None:
    ...
    row = conn.execute(
        "SELECT account FROM source_files WHERE file_name = 'a.mp4'"
    ).fetchone()
...
def test_caption_pattern_and_label_export(tmp_path: Path) -> None:
    ...
    upsert_caption_pattern(
        ...
        [
            {
                "ocrText": "just cracked",
                "confidence": 90,
                "box": {"x": 10, "y": 200, "w": 100, "h": 40},
            }
        ],
        90,
    )
    ...
    assert (
        conn.execute(
            "SELECT label FROM review_labels WHERE reference_id = ?", (ref,)
        ).fetchone()["label"]
        == "maybe"
    )
    assert (
        conn.execute("SELECT COUNT(*) AS c FROM caption_patterns").fetchone()["c"] == 1
    )
...
def test_caption_adaptation_cleans_described_identity_phrases() -> None:
    ...
    singular, singular_rules = adapt_caption_text(
        'would you give a 5"4 brunette girl a chance?'
    )
...
def test_contact_sheet_generates_html(tmp_path: Path) -> None:
    ...
    result = generate_contact_sheet(
        conn, "random", count=1, data_root=tmp_path / "data"
    )
...
def test_reference_intake_queues_gemini_analysis_and_exports_prompts(
    ...
    assert {item["targetTool"] for item in generated["prompts"]} == {
    ...
    higgsfield_line = (
        Path(generated["export"]["dailyHiggsfieldImageJsonlPath"])
        .read_text(encoding="utf-8")
        .splitlines()[0]
    )
    kling_line = (
        Path(generated["export"]["dailyKlingVideoJsonlPath"])
        .read_text(encoding="utf-8")
        .splitlines()[0]
    )
    assert (
        json.loads(higgsfield_line)["schema"]
        == "reference_factory.higgsfield_soul_image_prompt.v1"
    )
    assert (
        json.loads(kling_line)["schema"] == "reference_factory.kling_3_video_prompt.v1"
    )
...
def test_reference_intake_imports_analysis_before_prompt_generation(
    ...
    conn = make_conn(tmp_path)
    queued = queue_reference_analysis(
        conn, source, data_root=tmp_path / "data", platform="instagram"
    )
    job = queued["jobs"][0]
                ...
                "items": [
                    {
                        ...
                        "winningFormatCard": {
                            ...
                            "camera": {
                                "framing": "vertical medium close",
                                "angle": "mirror selfie",
                                "movement": "tiny handheld drift",
                            },
                            "lighting": "warm lamp light",
                            "setting": "bedroom mirror",
                            "styling": "black fitted top",
                            "textOverlay": {
                                "copy": "relationship hook",
                                "placement": "upper third",
                                "fontStyle": "white stroke",
                            },
                            "pacing": {
                                "energy": "medium",
                                "cutRhythm": "single shot",
                                "durationFeel": "short",
                            },
                            "audioVibe": {
                                "energy": "medium",
                        ...
                        "subject": {
                            "action": "poses near mirror",
                            "pose": "one hand on hip",
                            "expression": "confident smirk",
                            "wardrobe": "black fitted top",
                        },
                        "setting": {
                            "location": "bedroom mirror",
                            "lighting": "warm lamp light",
                            "background": "clean bedroom",
                        },
                        "visualPacing": {
                            "energy": "medium",
                            "cutRhythm": "single shot",
...
def test_reference_intake_minimal_gemini_prompt_and_direct_prompt_import(
                ...
                "items": [
                    {
                        ...
                        "recreation_blueprint": {
                            ...
                            "native_style_constraints": [
                                "do not make cinematic",
                                "imperfect phone-shot framing",
                            ],
                            "copy_risk_notes": ["do not copy the original face"],
                            "required_changes": [
                                "change wardrobe color and room details"
                            ],
                        },
    ...
    assert (
        image_json["must_keep"][0] == "subject scale: upper torso fills most of frame"
    )
    ...
    assert (
        prompts["higgsfield_soul_image"]["recreationBlueprint"]["first_frame"][
            "camera_distance"
        ]
    ...
    assert (
        ...
        in prompts["kling_3_video"]["mainPrompt"]
    )
    assert "Use the generated Soul ID image" in prompts["kling_3_video"]["mainPrompt"]
    assert prompts["kling_3_video"]["motion_directives"]["subject_motion"].startswith(
        "Use the generated Soul ID image"
    )
    ...
    assert (
        prompts["kling_3_video"]["motion_directives"]["fallback_provider"]
        == "grok_imagine"
    )
    assert (
...
def test_imageat_prompt_builders_lock_expected_json_shape() -> None:
    ...
    gemini_prompt = gemini_analysis_prompt(
        source, platform="instagram", prompt_style="minimal"
    )
    grok_prompt = _grok_prompt_builder(
        {"referenceId": "ootd_ref", "fileName": "ootd.mp4"}
    )
...
def test_generate_video_prompts_uses_latest_analysis_per_reference(
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "data"
    ...
    new_analysis = {
        ...
        "recreation_blueprint": {
            "first_frame": {
                "subject_scale": "full body side-profile mirror reflection"
            },
            "motion_beats": [
                {"time_range": "0.0-1.0s", "subject_motion": "tiny phone sway"}
            ],
        },
    }
    conn.execute(
    ...
    assert (
        ...
        == "subject scale: full body side-profile mirror reflection"
    )
    assert "NEW exact hip shift" in prompts["kling_3_video"]["mainPrompt"]
    assert (
        prompts["kling_3_video"]["motion_directives"]["subject_motion"]
        == "NEW exact hip shift and free-arm extension prompt"
    )
...
def test_higgsfield_prompt_pair_matching_and_dry_run(tmp_path: Path) -> None:
    ...
    result = generate_with_higgsfield(
        data_root=data_root, limit=1, dry_run=True, wait=True
    )
...
def test_higgsfield_dry_run_supports_candidates_variation_and_costs(
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "data"
    write_higgsfield_prompt_pair(data_root)
    ...
    assert run["variationCommand"][6].startswith(
        "Make one high-quality variation of this exact pose and background"
    )
    assert (
        "leopard-print fitted mini dress matching the reference outfit"
        in run["variationCommand"][6]
    )
    ...
    assert all(
        "half unbuttoned" not in command[6] for command in run["variationCommands"]
    )
    assert all(
        "white lace camisole" not in command[6] for command in run["variationCommands"]
    )
...
def test_higgsfield_soul_grid_strategy_uses_one_soul_id_grid_prompt(
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "data"
    write_higgsfield_prompt_pair(data_root)
...
def test_higgsfield_runner_prefers_grok_compiled_prompts(tmp_path: Path) -> None:
    ...
    single = generate_with_higgsfield(
        data_root=data_root, limit=1, dry_run=True, wait=True
    )
    ...
    assert (
        single["runs"][0]["videoCommand"][6]
        == "GROK KLING VIDEO PROMPT Negative prompt: GROK KLING NEGATIVE"
    )
...
def test_compile_prompts_with_grok_api_updates_prompt_jsonl(
    ...
    def fake_completion(**kwargs):
        ...
        assert (
            "Extra user instructions: more turquoise, more curve emphasis"
            in kwargs["prompt"]
        )
        return json.dumps(
            {
    ...
    image_row = json.loads(
        (
            data_root / "reference_intake" / "daily_higgsfield_image_prompts.jsonl"
        ).read_text(encoding="utf-8")
    )
    video_row = json.loads(
        (data_root / "reference_intake" / "daily_kling_video_prompts.jsonl").read_text(
            encoding="utf-8"
        )
    )
    assert result["compiledPrompts"]["soul_id_2x3_prompt"].startswith(
        "Create one high-quality six-panel grid image"
    )
    assert image_row["compiledPrompts"]["soul_id_2x3_prompt"].endswith(
        "Grok final 2x3 prompt with deep cleavage and round plump ass"
    )
    assert "no extra panels" in image_row["compiledPrompts"]["soul_id_2x3_prompt"]
    assert (
        image_row["compiledPrompts"]["single_panel_prompt"]
        == "Grok final single panel prompt with tight blue dress"
    )
    assert (
        video_row["compiledPrompts"]["kling_video_prompt"]
        == "Grok final Kling prompt with hip sway and natural breast bounce"
    )
    assert (
        video_row["compiledPrompts"]["kling_negative_prompt"]
        == "blurry, flat chest, flat ass"
    )
    assert image_row["compiledPrompts"]["structured_breakdown"][
        "outfit_variations"
    ] == [
        "blue dress",
        "white dress",
...
def test_compile_prompts_with_grok_api_rejects_weak_breakdown(
    tmp_path: Path, monkeypatch
) -> None:
    data_root = tmp_path / "data"
    write_higgsfield_prompt_pair(data_root, reference_id="ref_grok")
...
def test_higgsfield_variation_moderation_status_is_blocked(tmp_path: Path) -> None:
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(
            cmd,
            0,
            json.dumps({"id": "grid_job", "status": "nsfw", "result_url": ""}),
            "",
        )
...
def test_higgsfield_variation_reruns_blocked_cached_result(
    tmp_path: Path, monkeypatch
) -> None:
    data_root = tmp_path / "data"
    write_higgsfield_prompt_pair(data_root)
    monkeypatch.setattr(
        "reference_factory.higgsfield_runner._day", lambda: "2026-05-27"
    )
    selected_image = tmp_path / "selected.png"
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        assert "grok_image" in cmd
        return subprocess.CompletedProcess(
            cmd, 0, json.dumps({"id": "new_grid", "path": str(grid_image)}), ""
        )
...
def test_higgsfield_individual_variations_assemble_local_grid(
    tmp_path: Path, monkeypatch
) -> None:
    data_root = tmp_path / "data"
    write_higgsfield_prompt_pair(data_root)
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(
            cmd,
            0,
            json.dumps({"id": f"panel_{len(calls)}", "path": str(panel_png)}),
            "",
        )
    ...
    assert Path(
        lineage["generation"]["variationGrid"]["verticalSequenceVideoPath"]
    ).exists()
...
def test_higgsfield_can_animate_individual_variation_panels(
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        if "kling3_0" in cmd:
            return subprocess.CompletedProcess(
                ...
                0,
                json.dumps(
                    {"id": f"panel_video_{len(calls)}", "path": str(panel_video)}
                ),
                "",
            )
    ...
    assert Path(
        lineage["generation"]["variationGrid"]["animatedGridVideoPath"]
    ).exists()
...
def test_higgsfield_reuses_existing_panels_before_kling_animation(
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(
            cmd,
            0,
            json.dumps({"id": f"panel_video_{len(calls)}", "path": str(panel_video)}),
            "",
        )
...
def test_verify_proof_bundle_blocks_audio_or_stale_prompt_warning(
    tmp_path: Path,
) -> None:
    bundle = create_accepted_proof_bundle(tmp_path)
    (bundle / "generated_asset_lineage.json").write_text(
        json.dumps(
            {
                "quality": {
                    "promptScore": {"status": "pass", "warnings": ["stale warning"]}
                },
            }
        ),
        encoding="utf-8",
...
def test_higgsfield_generation_blocks_over_credit_cap(tmp_path: Path) -> None:
    ...
    result = generate_with_higgsfield(
        data_root=data_root, limit=1, dry_run=True, max_credits=1.0
    )
...
def test_higgsfield_mocked_success_writes_lineage(tmp_path: Path) -> None:
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        if "text2image_soul_v2" in cmd:
            return subprocess.CompletedProcess(
                cmd, 0, json.dumps({"id": "img_job", "path": str(image_file)}), ""
            )
        if "kling3_0" in cmd:
            return subprocess.CompletedProcess(
                cmd, 0, json.dumps({"id": "vid_job", "path": str(video_file)}), ""
            )
        raise AssertionError(cmd)
    ...
    assert lineage["generation"]["variationGrid"]["layout"] == "2x3"
    assert (
        "Make a 2x3 variation of this exact pose and background"
        in lineage["generation"]["variationGrid"]["prompt"]
    )
    assert (
        "No text, username, UI, or watermark"
        not in lineage["generation"]["variationGrid"]["prompt"]
    )
    assert (
        "Sharp realistic phone-photo quality"
        in lineage["generation"]["variationGrid"]["prompt"]
    )
    assert (
        lineage["quality"]["promptScore"]["schema"]
        == "reference_factory.prompt_quality_score.v1"
    )
...
def test_provider_doctor_reports_actionable_provider_state(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "reference_factory.provider_doctor.shutil.which",
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        if cmd[:3] == ["higgsfield", "auth", "whoami"]:
            return subprocess.CompletedProcess(
                cmd, 0, stdout='{"email":"operator@example.com"}', stderr=""
            )
        if cmd[:3] == ["higgsfield", "soul-id", "list"]:
            return subprocess.CompletedProcess(
                cmd, 0, stdout="Stacey 5828d958-91dd-4d6d-8909-934503f47644", stderr=""
            )
    ...
    def fake_runner(cmd: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        if "text2image_soul_v2" in cmd:
            return subprocess.CompletedProcess(
                cmd,
                0,
                json.dumps({"result": [{"id": "img_job", "path": str(image_file)}]}),
                "",
            )
        if "kling3_0" in cmd:
            return subprocess.CompletedProcess(
                cmd,
                0,
                json.dumps({"result": [{"id": "vid_job", "path": str(video_file)}]}),
                "",
            )
        raise AssertionError(cmd)
...
def test_reference_intake_imports_gemini_app_response_from_queue(
    ...
    assert Path(
        imported["promptGeneration"]["export"]["dailyHiggsfieldImageJsonlPath"]
    ).exists()
...
def test_reference_local_analysis_creates_pattern_card_and_export(
    ...
    exported = export_video_analyses(
        conn, data_root=tmp_path / "data", provider="local"
    )
...
def test_thumbnail_batch_skips_existing_and_creates_missing(tmp_path: Path) -> None:
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) AS c FROM frame_samples WHERE role='contact'"
        ).fetchone()["c"]
        == 2
    )
...
def test_ocr_cleanup_drops_junk_and_keeps_useful_caption(tmp_path: Path) -> None:
    ...
    remaining = conn.execute("SELECT normalized_text FROM caption_patterns").fetchone()[
        0
    ]
...
def test_import_apify_metrics_matches_local_media_and_generates_prompts(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    conn.execute(
    ...
    imported = import_apify_metrics(
        conn, [apify_path], top_limit=2, output_dir=tmp_path / "apify"
    )
...
def test_pattern_analyzer_labels_top_posts_and_exports_cards(tmp_path: Path) -> None:
    ...
    analyzed = analyze_patterns(
        conn, limit=1, provider="heuristic", output_dir=tmp_path / "learning"
    )
    ...
    assert (
        conn.execute(
            "SELECT label FROM review_labels WHERE reference_id = 'ref_local'"
        ).fetchone()["label"]
        == "gold"
    )
...
def test_public_post_ranking_prefers_measured_prompt_outcomes(tmp_path: Path) -> None:
    ...
    for idx, (media_id, account) in enumerate(
        (("2222222222", "small_account"), ("9999999999", "huge_account")), start=1
    ):
        conn.execute(
            """
            INSERT INTO source_files (
...
def test_learning_system_exports_winner_dna_and_audio_fit_from_measured_winners(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    conn.execute(
    ...
    build_learning_system(
        conn, limit=1, output_dir=tmp_path / "learning", embedding_clusters=False
    )
    summary = learning_summary(conn, limit=1)
    pattern = json.loads(
        conn.execute("SELECT pattern_json FROM reference_patterns").fetchone()[
            "pattern_json"
        ]
    )
    cluster = summary["topClusters"][0]
    ...
    assert (
        cluster["performanceSignals"]["performanceClassCounts"]["performed_well"] == 1
    )
    ...
    assert (
        cluster["audioRecommendations"]["recommendations"][0]["performanceClass"]
        == "performed_well"
    )
    assert (
...
def test_embedding_clusters_group_similar_references_and_preserve_export_shape(
    tmp_path: Path,
) -> None:
    paths = []
...
def test_embedding_cache_reuses_and_invalidates_source_metadata(tmp_path: Path) -> None:
    ...
    build_embedding_clusters(
        None, [card], tmp_path / "learning", model="fake-dino", provider=provider
    )
    build_embedding_clusters(
        None, [card], tmp_path / "learning", model="fake-dino", provider=provider
    )
...
def test_embedding_clusters_fall_back_when_model_provider_is_unavailable(
    tmp_path: Path, monkeypatch
) -> None:
    image = tmp_path / "a.jpg"
    ...
    report = build_embedding_clusters(
        None, [card], tmp_path / "learning", model="fake-dino"
    )
...
def test_tiktok_archive_imports_slideshow_references_and_patterns(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    ...
    write_archive_db(
        appdata / "db_texts.js",
        "dbt_base64",
        {"video456": "POV: swipe to see the answer"},
    )

    imported = import_tiktok_archive(
        conn, archive, top_limit=1, output_dir=tmp_path / "tiktok_out"
    )
    ...
    signal = extract_audio_signal(
        {
            "musicInfo": {
                "audio_id": "123",
                "song_name": "Pretty Girl Era (Sped Up)",
                "artist_name": "LU KALA",
...
def test_competitor_audio_leaderboard_ranks_similar_creator_sounds(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    rows = [
...
def test_audio_catalog_csv_import_list_export_and_recommend(tmp_path: Path) -> None:
    ...
    recommended = recommend_audio(
        conn,
        platform="instagram",
        content_tags=["fit_check", "glam"],
        account_tags=["model_a"],
        limit=2,
    )
...
def test_import_example_reel_audio_creates_campaign_factory_ready_feed(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    reels_path = tmp_path / "example_reels.json"
    export_path = tmp_path / "audio_memory.json"
    reels_path.write_text(
        json.dumps(
            {
                "items": [
                    {
    ...
    clean = next(
        item for item in exported["items"] if item["nativeAudioId"] == "ig_audio_clean"
    )
    unresolved = next(
        item
...
def test_scrape_instagram_audio_prefers_stored_reference_music_info(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    ...
    result = scrape_instagram_audio(
        ...
        fetcher=lambda url: (_ for _ in ()).throw(
            AssertionError("should not fetch public HTML when cache has musicInfo")
        ),
    )
    ...
    assert (
        listed["items"][0]["nativeAudioUrl"]
        == "https://www.instagram.com/reels/audio/cached_audio_1/"
    )


def test_scrape_instagram_audio_keeps_unresolved_rows_when_public_page_blocks_metadata(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
...
def test_audio_catalog_review_queue_flags_stale_and_missing_fields(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    ...
    review = review_audio_catalog(conn, platform="instagram")
    reasons_by_title = {
        item["title"]: item["reviewReasons"] for item in review["items"]
    }
...
def test_audio_trend_snapshots_are_manual_history_and_update_catalog(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    saved = upsert_audio_record(
    ...
    snapshot = upsert_audio_trend_snapshot(
        conn,
        {
...
def test_audio_snapshot_csv_import_matches_existing_catalog_record(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    upsert_audio_record(
    ...
    recommended = recommend_audio(
        conn, platform="instagram", content_tags=["glam"], limit=1
    )

    assert imported["imported"] == 1
    assert len(imported["errors"]) == 1
    assert (
        recommended["recommendations"][0]["latestTrendSnapshot"]["velocityScore"] == 0.8
    )


def test_audio_catalog_flags_generic_tiktok_titles_until_resolved(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    upsert_audio_record(
...
def test_audio_resolution_shortlist_prioritizes_unresolved_tiktok_sounds(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    upsert_audio_record(
...
def test_tiktok_audio_leaderboard_rows_convert_to_catalog_import(
    tmp_path: Path,
) -> None:
    payload = {
        "items": [
...
def write_archive_db(path: Path, var_name: str, value: dict[str, object]) -> None:
    payload = base64.b64encode(gzip.compress(json.dumps(value).encode("utf-8"))).decode(
        "ascii"
    )
    path.write_text(f'window.{var_name}="{payload}";', encoding="utf-8")


def test_learning_system_builds_clusters_playbook_and_campaign_bank(
    tmp_path: Path,
) -> None:
    conn = make_conn(tmp_path)
    for idx in range(3):
    ...
    analyze_patterns(
        conn, limit=3, provider="heuristic", output_dir=tmp_path / "learning"
    )
    ...
    bank = json.loads(
        Path(built["campaignReferenceBankPath"]).read_text(encoding="utf-8")
    )
