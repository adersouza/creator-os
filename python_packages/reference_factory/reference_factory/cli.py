...
from .public_metrics import export_learning_set, generate_prompt_cards, import_apify_metrics, top_public_posts
from .reference_intake import analyze_reference_local, analyze_reference_with_gemini_api, analyze_reference_with_grok_api, compile_prompts_with_grok_api, export_analysis_queue, export_video_prompts, export_video_analyses, generate_video_prompts, import_gemini_app_response, import_reference_analysis, queue_reference_analysis
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-root", default=str(DEFAULT_DATA_ROOT), help="Derived data root")
    ...
    frames = sub.add_parser("sample-frames", help="Extract frame samples from valid videos")
    frames.add_argument("--videos", default="all", help="Compatibility alias; use all")
    frames.add_argument("--limit", default=None)

    thumbs = sub.add_parser("thumbnail-batch", help="Extract missing contact thumbnails")
    thumbs.add_argument("--limit", default="all")

    ocr = sub.add_parser("ocr", help="Run OCR on sampled frames")
    ocr.add_argument("--engine", default="auto", choices=["auto", "apple_vision", "tesseract", "heuristic"])
    ...
    sheet.add_argument("--mode", default="random", choices=["random", "top-accounts", "captioned", "visual", "best", "unreviewed"])
    ...
    review_batch_parser = sub.add_parser("review-batch", help="Print a balanced guided review batch")
    ...
    adapt_captions = sub.add_parser("adapt-captions", help="Rewrite reference captions for the active model profile")
    adapt_captions.add_argument("--profile", default=None, help="Optional caption adaptation profile JSON path")
    adapt_captions.add_argument("--input", default=None, help="Optional caption library JSONL input path")
    adapt_captions.add_argument("--init-profile", action="store_true", help="Create the default profile and exit")

    apify_import = sub.add_parser("import-apify-metrics", help="Import Apify public post metrics")
    apify_import.add_argument("--input", action="append", required=True, help="Apify JSON output path; repeatable")
    ...
    top_posts = sub.add_parser("top-public-posts", help="Print top public posts by plays/views")
    top_posts.add_argument("--limit", type=int, default=300)

    prompts = sub.add_parser("generate-prompt-cards", help="Generate original prompt cards from public winners")
    prompts.add_argument("--limit", type=int, default=50)

    learning = sub.add_parser("export-learning-set", help="Export top public winners with prompt cards and ContentForge references")
    learning.add_argument("--limit", type=int, default=300)
    learning.add_argument("--copy-media", action="store_true", help="Copy matched local videos into the learning set reference folder")

    patterns = sub.add_parser("analyze-patterns", help="Analyze top public winners into reusable pattern labels")
    ...
    competitor_audio = sub.add_parser("competitor-audio", help="Rank audios used by similar creator public posts")
    competitor_audio.add_argument("--platform", default=None)
    competitor_audio.add_argument("--accounts", default="", help="Comma-separated creator usernames to filter, without scraping")
    competitor_audio.add_argument("--caption-keywords", default="", help="Comma-separated caption keywords such as ai, model, ofm")
    ...
    audio_import = sub.add_parser("import-audio-csv", help="Import manually curated trending audio catalog CSV")
    audio_import.add_argument("--input", required=True)

    example_audio_import = sub.add_parser("import-example-reel-audio", help="Import audio signals from existing/reference reel metadata")
    example_audio_import.add_argument("--input", default=None, help="Optional JSON/CSV of example reels; defaults to imported public_posts")
    example_audio_import.add_argument("--limit", type=int, default=500)
    example_audio_import.add_argument("--export", default=None, help="Optional Campaign Factory audio catalog export path")
    example_audio_import.add_argument("--preserve-manual-fields", action=argparse.BooleanOptionalAction, default=True)

    ig_audio_scrape = sub.add_parser("scrape-instagram-audio", help="Scrape public Instagram reel/post URLs for audio metadata")
    ig_audio_scrape.add_argument("--url", action="append", default=[], help="Instagram reel/post URL; repeatable")
    ig_audio_scrape.add_argument("--input", default=None, help="Optional text/JSON/CSV file of Instagram reel URLs")
    ig_audio_scrape.add_argument("--limit", type=int, default=50)
    ig_audio_scrape.add_argument("--export", default=None, help="Optional Campaign Factory audio catalog export path")

    audio_list = sub.add_parser("list-audio", help="List operator-curated audio catalog")
    ...
    audio_recommend = sub.add_parser("recommend-audio", help="Recommend catalog audio by platform and tags")
    ...
    audio_refresh.add_argument("--preserve-manual-fields", action=argparse.BooleanOptionalAction, default=True)

    audio_resolve = sub.add_parser("resolve-audio", help="Resolve a generic native audio ID into an operator-ready catalog record")
    ...
    audio_health = sub.add_parser("audio-health", help="Summarize catalog readiness for Campaign Factory")
    ...
    audio_shortlist = sub.add_parser("audio-resolution-shortlist", help="Show the top unresolved native audio IDs to resolve today")
    ...
    audio_snapshot_import = sub.add_parser("import-audio-snapshot-csv", help="Import manually curated audio trend snapshot CSV")
    audio_snapshot_import.add_argument("--input", required=True)

    audio_snapshot_add = sub.add_parser("add-audio-snapshot", help="Add one manually curated audio trend snapshot")
    ...
    audio_snapshot_list = sub.add_parser("list-audio-snapshots", help="List manually curated audio trend snapshots")
    ...
    doctor = sub.add_parser("provider-doctor", help="Check local provider readiness without printing secrets")
    doctor.add_argument("--require-gemini", action="store_true", help="Treat missing Gemini API key as blocked")
    doctor.add_argument("--skip-xai-check", action="store_true", help="Skip xAI API reachability/billing check")
    doctor.add_argument("--skip-higgsfield-auth", action="store_true", help="Skip Higgsfield auth and Soul ID checks")

    pattern_summary_parser = sub.add_parser("pattern-summary", help="Summarize analyzed reference patterns")
    pattern_summary_parser.add_argument("--limit", type=int, default=300)

    export_patterns_parser = sub.add_parser("export-patterns", help="Export pattern cards for learning/prompt use")
    export_patterns_parser.add_argument("--limit", type=int, default=300)
    export_patterns_parser.add_argument("--for-campaign-factory", action="store_true", help="Also write the stable Campaign Factory handoff bank")

    apply_labels = sub.add_parser("apply-pattern-labels", help="Write machine-suggested gold/maybe/ignore labels into review labels")
    ...
    queue_analysis = sub.add_parser("queue-reference-analysis", help="Queue local TikTok/Reels downloads for Gemini or VLM analysis")
    ...
    queue_analysis.add_argument("--prompt-style", choices=["guided", "minimal"], default="guided")
    queue_analysis.add_argument("--media-kinds", default="video,image", help="Comma-separated media kinds to queue: video,image")
    queue_analysis.add_argument("--limit", type=int, default=None)
    queue_analysis.add_argument("--creative-plan-id", default=None)

    export_analysis = sub.add_parser("export-reference-analysis-queue", help="Export pending reference-analysis prompts")
    export_analysis.add_argument("--provider-target", default="gemini")
    export_analysis.add_argument("--limit", type=int, default=50)

    local_analysis = sub.add_parser("analyze-reference-local", help="Run deterministic local preprocessing for reference videos")
    ...
    export_video_analysis = sub.add_parser("export-video-analyses", help="Export local/Gemini video analysis records")
    export_video_analysis.add_argument("--provider", default=None)
    export_video_analysis.add_argument("--limit", type=int, default=100)

    import_analysis = sub.add_parser("import-reference-analysis", help="Import Gemini/VLM JSON analysis for queued references")
    import_analysis.add_argument("--input", required=True)

    import_gemini_app = sub.add_parser("import-gemini-app-response", help="Import the copied Gemini app/Chrome JSON response for an exported queue job")
    import_gemini_app.add_argument("--queue", required=True, help="Exported Gemini analysis queue JSON")
    import_gemini_app.add_argument("--response", default=None, help="Optional response JSON/text path; defaults to macOS clipboard")
    import_gemini_app.add_argument("--job-index", type=int, default=1, help="1-based job index in the queue")
    import_gemini_app.add_argument("--model-profile", default=None)
    import_gemini_app.add_argument("--generate-prompts", action=argparse.BooleanOptionalAction, default=True)

    generate_prompts = sub.add_parser("generate-video-prompts", help="Generate Higgsfield/Kling prompt drafts from reference analysis")
    ...
    generate_prompts.add_argument("--include-pending", action=argparse.BooleanOptionalAction, default=True)
    generate_prompts.add_argument("--creative-plan-id", default=None)

    export_prompts = sub.add_parser("export-video-prompts", help="Export generated AI video prompt drafts")
    export_prompts.add_argument("--limit", type=int, default=100)
    export_prompts.add_argument("--creative-plan-id", default=None)

    higgsfield = sub.add_parser("generate-with-higgsfield", help="Generate Higgsfield Soul images and Kling 3.0 videos from daily prompt exports")
    higgsfield.add_argument("--data-root", default=argparse.SUPPRESS, help="Derived data root; accepted here for command-local ergonomics")
    higgsfield.add_argument("--limit", type=int, default=1)
    higgsfield.add_argument("--reference-id", default=None, help="Generate only the prompt pair for this source reference id")
    ...
    higgsfield.add_argument("--min-prompt-score", type=int, default=72, help="Block Higgsfield generation below this prompt quality score; use 0 to disable")
    ...
    higgsfield.add_argument("--variation-strategy", default="individual", choices=["individual", "grid", "soul_grid"])
    higgsfield.add_argument("--animate-variation-panels", action="store_true", help="Animate each individual 2x3/3x3 panel with Kling before assembling a video grid")
    higgsfield.add_argument("--variation-panel-dir", default=None, help="Reuse an existing ordered panel image folder instead of regenerating variation stills")
    ...
    daily_generation = sub.add_parser("run-daily-generation", help="Generate a daily Higgsfield/Kling batch and intake finished videos into Campaign Factory")
    daily_generation.add_argument("--data-root", default=argparse.SUPPRESS, help="Derived data root; accepted here for command-local ergonomics")
    ...
    daily_generation.add_argument("--kling-mode", default="std", choices=["std", "pro", "4k"])
    ...
    verify_proof = sub.add_parser("verify-proof-bundle", help="Verify an accepted A-to-Z proof bundle with ffprobe and lineage checks")
    verify_proof.add_argument("--bundle", required=True, help="Accepted proof bundle folder")

    gemini_api = sub.add_parser("analyze-reference-with-gemini-api", help="Use the official Gemini API to analyze queued local reference videos")
    ...
    gemini_api.add_argument("--prompt-style", choices=["guided", "minimal"], default="minimal")

    grok_api = sub.add_parser("analyze-reference-with-grok-api", help="Use xAI/Grok vision to draft ImageAt-style JSON prompts from local references")
    ...
    grok_compile = sub.add_parser("compile-prompts-with-grok-api", help="Use xAI/Grok vision to compile final clean Higgsfield/Kling prose prompts")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "scan":
            ...
            print_json(review_batch(conn, target=args.target, mode=args.mode, account_cap=args.account_cap))
        elif args.command == "label":
            tags = [tag.strip() for tag in args.tags.split(",") if tag.strip()]
            print_json(label_reference(conn, args.reference_id, args.label, tags, args.notes))
            ...
            print_json(export_learning_set(conn, args.limit, data_root / "learning", copy_media=args.copy_media))
            ...
            print_json(analyze_audio_patterns(conn, limit=args.limit, output_dir=data_root / "learning"))
        elif args.command == "competitor-audio":
            print_json(competitor_audio_leaderboard(
                conn,
                platform=args.platform,
                accounts=[item.strip() for item in args.accounts.split(",") if item.strip()],
                caption_keywords=[item.strip() for item in args.caption_keywords.split(",") if item.strip()],
            ...
            print_json(import_example_reel_audio(
                conn,
            ...
            print_json(scrape_instagram_audio(
                conn,
                urls=args.url,
            ...
            if args.export:
                ...
                print_json(review_audio_catalog(conn, platform=args.platform, limit=args.limit))
            else:
                print_json(list_audio_catalog(conn, platform=args.platform, fresh_only=args.fresh_only, limit=args.limit))
        elif args.command == "recommend-audio":
            print_json(recommend_audio(
                conn,
                platform=args.platform,
            ...
            print_json(refresh_tiktok_audio(
                conn,
            ...
            print_json(resolve_audio_record(conn, {
                "platform": args.platform,
            ...
            health = audio_catalog_health(conn, platform=args.platform, limit=args.limit)
            if args.export:
            ...
            shortlist = audio_resolution_shortlist(conn, platform=args.platform, limit=args.limit)
            if args.export:
                path = Path(args.export).expanduser()
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text(json.dumps(shortlist, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
            ...
            print_json(upsert_audio_trend_snapshot(conn, {
            ...
            print_json(list_audio_trend_snapshots(conn, platform=args.platform, audio_catalog_id=args.audio_catalog_id, limit=args.limit))
        elif args.command == "provider-doctor":
            print_json(provider_doctor(
            ...
            if args.for_campaign_factory:
                print_json(build_learning_system(conn, limit=args.limit, output_dir=data_root / "learning"))
            else:
            ...
            print_json(
                queue_reference_analysis(
                    ...
                    media_kinds=[kind.strip() for kind in args.media_kinds.split(",") if kind.strip()],
                    limit=args.limit,
            ...
            print_json(export_analysis_queue(conn, data_root=data_root, provider_target=args.provider_target, limit=args.limit))
        elif args.command == "analyze-reference-local":
            print_json(analyze_reference_local(
                conn,
                Path(args.source),
                data_root=data_root,
                platform=args.platform,
            ...
            print_json(export_video_analyses(conn, data_root=data_root, provider=args.provider, limit=args.limit))
            ...
            print_json(import_gemini_app_response(
                conn,
                queue_path=Path(args.queue),
                response_path=Path(args.response).expanduser() if args.response else None,
                data_root=data_root,
            ...
            print_json(
                generate_video_prompts(
                    conn,
                    data_root=data_root,
                    target_tools=[tool.strip() for tool in args.tools.split(",") if tool.strip()],
                    model_profile=args.model_profile,
                    limit=args.limit,
            ...
            print_json(export_video_prompts(conn, data_root=data_root, limit=args.limit, creative_plan_id=args.creative_plan_id))
        elif args.command == "generate-with-higgsfield":
            print_json(generate_with_higgsfield(
                data_root=data_root,
                limit=args.limit,
                ...
                min_prompt_score=None if args.min_prompt_score <= 0 else args.min_prompt_score,
                ...
                variation_panel_dir=Path(args.variation_panel_dir).expanduser() if args.variation_panel_dir else None,
                selected_image=Path(args.selected_image).expanduser() if args.selected_image else None,
                no_video=args.no_video,
                no_campaign_intake=args.no_campaign_intake,
                campaign_factory_root=Path(args.campaign_factory_root).expanduser() if args.campaign_factory_root else None,
                campaign=args.campaign,
                model=args.model,
                creative_plan=args.creative_plan,
            ))
        elif args.command == "run-daily-generation":
            print_json(run_daily_generation(
                data_root=data_root,
                creative_plan=args.creative_plan,
                limit=args.limit,
                campaign=args.campaign,
            ...
            print_json(analyze_reference_with_gemini_api(
                conn,
                source_root=Path(args.source),
                data_root=data_root,
                platform=args.platform,
            ...
            print_json(analyze_reference_with_grok_api(
                conn,
                source_root=Path(args.source),
                data_root=data_root,
                platform=args.platform,
            ...
            print_json(compile_prompts_with_grok_api(
                data_root=data_root,
                reference_id=args.reference_id,
