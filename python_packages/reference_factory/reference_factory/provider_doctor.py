...
def provider_doctor(
    *,
    ...
    checks = [
        ...
        _env_key_check(
            "gemini", ["GEMINI_API_KEY", "GOOGLE_API_KEY"], required=require_gemini
        ),
        _binary_check(
            "higgsfield",
...
def _env_key_check(
    name: str, keys: list[str], *, required: bool = True
) -> dict[str, Any]:
...
def _xai_models_check(api_key: str, *, urlopen: Urlopen) -> dict[str, Any]:
    ...
    try:
        ...
        model_count = (
            len(parsed.get("data") or []) if isinstance(parsed, dict) else None
        )
        return {
            "name": "xai.spend",
        ...
        return {
            ...
            "status": "warning",
            "httpStatus": status,
            "message": "xAI API check returned a non-success status.",
        }
    except Exception as exc:  # noqa: BLE001 - surfaced to operator
        return {
            "name": "xai.spend",
            "status": "warning",
            "message": f"xAI API check could not complete: {exc}",
        }
...
def _higgsfield_soul_check(runner: Runner) -> dict[str, Any]:
    ...
    if result.returncode == 0 and (
        DEFAULT_STACEY_SOUL_ID in output or "Stacey" in output
    ):
        return {
            "name": "higgsfield.soul_id.stacey",
            "status": "ok",
            "soulId": DEFAULT_STACEY_SOUL_ID,
        }
    return {
        "name": "higgsfield.soul_id.stacey",
        "status": "blocked",
        "message": _short_stderr(result)
        or "Could not confirm the Stacey Soul ID in Higgsfield.",
