...
"""
    ...
    def register_parent_reel(
        ...
        *,
        ...
        asset = self.rendered_asset(rendered_asset_id)
        content_surface = normalize_content_surface(asset.get("content_surface"))
        if content_surface == "reel":
        ...
        self.conn.execute(
            ...
            (
                ...
                audio_id,
                status,
                json.dumps(sanitize_for_storage(payload), ensure_ascii=False, sort_keys=True),
                now,
                now,
            ),
        )
        ...
        self.conn.commit()
        return self._concept_payload(self.conn.execute("SELECT * FROM concepts WHERE id = ?", (concept_id,)).fetchone())
    ...
    def variant_plan(
        ...
        *,
        ...
        preset = contentforge_preset if contentforge_preset in CONTENTFORGE_VARIANT_PRESETS else "caption_safe"
    ...
    def generate_variants(
        ...
        *,
        ...
        try:
            ...
            return self._contentforge_variant_pack_blocked_result(
                plan=plan,
        ...
        if job.get("schema") in CONTENTFORGE_VARIANT_PACK_SCHEMAS:
            report = job
        else:
            ...
            while time.monotonic() < deadline:
                ...
                try:
                    with urlopen(poll_request, timeout=request_timeout_seconds) as response:
        ...
        if report.get("schema") not in CONTENTFORGE_VARIANT_PACK_SCHEMAS:
            return {
                "schema": "campaign_factory.generate_variants.v1",
                "status": "blocked",
        ...
        try:
            for result in report.get("results") or []:
                ...
                if not source_variant.exists():
                    continue
                digest = sha256_file(source_variant)
                ...
                operations = [
                    ...
                    {"type": "contentforge_result", "result": sanitize_for_storage(result)},
                    {
                        "type": "preserve_parent_lineage",
                        "parentAssetId": parent_asset_id,
                        "captionFamilyId": caption_version.get("captionFamilyId") if caption_version else None,
                ...
                """,
                    (
                        ...
                        json.dumps(sanitize_for_storage(caption_context), ensure_ascii=False, sort_keys=True),
                        json.dumps(sanitize_for_storage(caption_generation), ensure_ascii=False, sort_keys=True),
                ...
                variant_payload = self.register_variant_asset(
                    parent_asset_id=parent_asset_id,
    ...
    def register_variant_asset(
        ...
        *,
        ...
        content_surface = normalize_content_surface(variant.get("content_surface") or parent.get("content_surface"))
        self.conn.execute(
            """
        ...
        self.conn.execute(
            ...
            (
                ...
                audio_id,
                content_surface,
                json.dumps(sanitize_for_storage(operations), ensure_ascii=False, sort_keys=True),
                qc_status,
                contentforge_run_id,
                now,
                now,
        ...
        if commit:
            self.conn.commit()
        return self._variant_asset_payload(self.conn.execute("SELECT * FROM variant_assets WHERE variant_asset_id = ?", (variant_asset_id,)).fetchone())

    def parent_variant_inventory(self, campaign_slug: str) -> dict[str, Any]:
        campaign = self.campaign_by_slug(campaign_slug)
        concepts = [self._concept_payload(row) for row in self.conn.execute("SELECT * FROM concepts WHERE campaign_id = ? ORDER BY created_at", (campaign["id"],)).fetchall()]
        families = [self._variant_family_payload(row) for row in self.conn.execute("SELECT * FROM variant_families WHERE campaign_id = ? ORDER BY created_at", (campaign["id"],)).fetchall()]
        variants = [self._variant_asset_payload(row) for row in self.conn.execute("SELECT * FROM variant_assets WHERE campaign_id = ? ORDER BY variant_family_id, variant_index", (campaign["id"],)).fetchall()]
        usage = [self._variant_usage_payload(row) for row in self.conn.execute("SELECT * FROM variant_account_usage WHERE campaign_id = ? ORDER BY created_at DESC", (campaign["id"],)).fetchall()]
    ...
    def variant_metrics_rollup(self, campaign_slug: str) -> dict[str, Any]:
        ...
        return {
            ...
            "parents": self._variant_rollup_group(snapshots, "parentReelId", "parentReelId"),
            "families": self._variant_rollup_group(snapshots, "variantFamilyId", "variantFamilyId"),
            "variants": self._variant_rollup_group(snapshots, "variantId", "variantId"),
            "captions": self._variant_rollup_group(snapshots, "captionHash", "captionHash"),
            "captionFamilies": self._variant_rollup_group(snapshots, "captionFamilyId", "captionFamilyId"),
            "captionVersions": self._variant_rollup_group(snapshots, "captionVersionId", "captionVersionId"),
            "audio": self._variant_rollup_group(snapshots, "audioId", "audioId"),
            "surfaces": self._variant_rollup_group(snapshots, "contentSurface", "contentSurface"),
        }
    ...
    def creator_os_200_account_acceptance_suite(
        ...
        *,
        ...
        for creator in creator_names:
            ...
            readiness_by_creator.append(self.creator_os_execution_readiness(
                creator=creator,
        ...
        health_reports = [
            self.creator_os_account_health_report(
                creator=creator,
                threadsdash_report=threadsdash_report,
                generated_at=generated,
            )
    ...
    def inventory_slo_report(
        ...
        *,
        ...
        daily_posts = max(0, int(accounts or 0)) * max(0, int(posts_per_account_per_day or 0))
        ...
        minimum_by_surface = self._inventory_slo_surface_targets(minimum_buffer)
        current_by_surface = {
            surface: int((current_drafts_by_surface or {}).get(surface) or 0)
            for surface in CONTENT_SURFACES
        }
        health = self._inventory_health(current=max(0, int(current_validated_drafts or 0)), minimum=minimum_buffer)
        return {
            ...
            "currentValidatedDrafts": max(0, int(current_validated_drafts or 0)),
            "currentDraftsPerSurface": current_by_surface,
            "draftShortfall": max(0, minimum_buffer - max(0, int(current_validated_drafts or 0))),
    ...
    def inventory_buffer_report(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return {
            **report,
            ...
            "bufferDaysAvailable": round(current / max(1, int(report.get("dailyPostObligations") or 1)), 2),
            "wouldWrite": False,
        }
    ...
    def inventory_factory_audit(
        ...
        *,
        ...
        counts = self._inventory_stage_counts(creator=creator, campaign_slug=campaign_slug)
        daily_demand = max(0, int(accounts or 0)) * max(0, int(posts_per_account_per_day or 0))
        schedule_safe = int(counts["scheduleSafeAssets"])
        validated = int(counts["validatedAssets"])
        publishable = int(counts["publishableAssets"])
        limiting_stage = self._inventory_limiting_stage(counts)
        return {
            ...
            "dailyCapacityEstimate": min(schedule_safe, daily_demand) if daily_demand else schedule_safe,
            ...
            "inventoryLostByStage": self._inventory_loss_by_stage(counts),
            "wouldWrite": False,
        }
    ...
    def inventory_yield_analysis(
        ...
        *,
        ...
        variant_to_validated = self._ratio(counts["validatedAssets"], counts["variantAssets"])
        validated_to_schedule_safe = self._ratio(counts["scheduleSafeAssets"], counts["validatedAssets"])
        yields = {
        ...
        largest_dropoff = min(non_zero.items(), key=lambda item: item[1])[0] if non_zero else "parent_to_variant"
        return {
    ...
    def inventory_buffer_policy_plan(
        ...
        *,
        ...
        if available_inventory is None:
            inventory = self._build_surface_inventory(creator=self._creator_label(creator)).get("inventoryBySurface") or {}
            available = int((inventory.get(normalized_surface) or {}).get("scheduleSafe") or 0)
        else:
        ...
        return {
            ...
            "health": self._inventory_health(current=available, minimum=required),
            "wouldWrite": False,
        }
    ...
    def inventory_slo_enforcement_audit(
        ...
        *,
        ...
        creator_names = [self._creator_label(item) for item in (creators or ["Creator 1"]) if str(item or "").strip()] or ["Creator 1"]
        total_required = max(0, int(accounts or 0)) * max(0, int(posts_per_account_per_day or 0)) * max(1, int(minimum_inventory_days or 1))
        required_per_creator = math.ceil(total_required / max(1, len(creator_names)))
        required_by_surface = self._inventory_slo_surface_targets(required_per_creator)
        availability = available_by_creator_surface or {}
        violations = []
        creator_shortfalls: dict[str, int] = {}
        surface_shortfalls: dict[str, int] = {surface: 0 for surface in CONTENT_SURFACES}
        for creator in creator_names:
            ...
            for surface, required in required_by_surface.items():
                ...
                if shortfall:
                    ...
                    creator_shortfalls[creator] = creator_shortfalls.get(creator, 0) + shortfall
                    surface_shortfalls[surface] = surface_shortfalls.get(surface, 0) + shortfall
        return {
            ...
            "highestRiskCreator": max(creator_shortfalls.items(), key=lambda item: item[1])[0] if creator_shortfalls else "",
            "highestRiskSurface": max(surface_shortfalls.items(), key=lambda item: item[1])[0] if any(surface_shortfalls.values()) else "",
            "wouldWrite": False,
        }
    ...
    def inventory_consumption_simulation(
        ...
        *,
        ...
        for account_count in tiers:
            daily_demand = max(0, int(account_count or 0)) * max(0, int(posts_per_account_per_day or 0))
            rows.append({
    ...
    def inventory_production_requirements(
        ...
        *,
        ...
        effective_yield = max(0.01, float(variant_to_validated_yield or 0) * float(validated_to_schedule_safe_yield or 0))
        required_variants = math.ceil(posts / effective_yield) if posts else 0
        per_parent = max(1, int(variants_per_parent or 1))
        required_parents = math.ceil(required_variants / per_parent) if required_variants else 0
        return {
            ...
            "requiredCaptionVersionsPerDay": math.ceil(required_variants / 3) if required_variants else 0,
    ...
    def road_to_200_accounts(self) -> dict[str, Any]:
        production = self.inventory_production_requirements(accounts=200, posts_per_account_per_day=3)
    ...
    def inventory_exception_audit(
        ...
        *,
        ...
        top = max(counts.items(), key=lambda item: (item[1], item[0]))[0] if counts else ""
        total = sum(counts.values())
        avoidable = sum(count for reason, count in counts.items() if reason not in {"restriction_event", "account_health_blocked"})
        return {
            ...
            "inventoryLossReasons": [
                {
                    "reason": reason,
                    "count": count,
                    "avoidable": reason not in {"restriction_event", "account_health_blocked"},
        ...
        *,
        ...
        if surface:
            assets = [
                ...
                if normalize_content_surface(asset.get("content_surface") or asset.get("source_content_surface")) == surface
            ]
        ...
        blocked_rows = [self._inventory_recovery_blocked_asset(item) for item in blocked_assets]
        class_rows = self._inventory_recovery_class_rows(blocked_rows)
        priorities = self._inventory_recovery_priorities(class_rows)
        top = priorities[0] if priorities else {}
        top3_classes = [item["repairClass"] for item in priorities[:3]]
        top_recovered = self._inventory_recovery_assets_unlocked(blocked_rows, [str(top.get("repairClass") or "")])
        top3_recovered = self._inventory_recovery_assets_unlocked(blocked_rows, top3_classes)
    ...
    def schedule_safe_production_report(
        ...
        *,
        ...
        surface = normalize_content_surface(content_surface or "reel")
        ...
        capacity = self._schedule_safe_production_capacity(
        ...
        return {
            ...
            "variantsRequiredPerDay": self._schedule_safe_required_variants_per_day(produced_per_day, schedule_safe, len([
    ...
    def fresh_schedule_safe_production_plan(
        ...
        *,
        ...
        current = (
            int(current_inventory)
            if current_inventory is not None
            else self._fresh_reel_current_schedule_safe_inventory(creator=creator, campaign_slug=campaign_slug)
        )
        ...
        stages = self._fresh_reel_expected_stage_rows(
        ...
        execution_batches = self._fresh_reel_execution_batches(
            fresh_needed=needed,
        ...
        conservative_daily = max(1, int(math.floor(daily_schedule_safe_target * (conservative_yield / max(0.01, downstream_yield))))) if daily_schedule_safe_target else 0
        return {
    ...
    def contentforge_visual_qc_failure_report(
        ...
        *,
        ...
        failures = [
            self._contentforge_visual_qc_failure_for_asset(asset, surface)
            for asset in variants
        ]
        visual_failures = [failure for failure in failures if not failure["visualQcPassed"]]
        categories = self._contentforge_visual_qc_category_rows(visual_failures)
        ranked = sorted(
            categories,
            key=lambda row: (
        ...
        top_gain = self._contentforge_visual_qc_recovered_inventory(visual_failures, [str(top.get("failureCategory") or "")])
        top3_gain = self._contentforge_visual_qc_recovered_inventory(visual_failures, top3)
        return {
            ...
            "mostImportantAnswer": self._contentforge_visual_qc_answer(top, len(visual_failures)),
            "wouldWrite": False,
        }
    ...
    def multi_blocker_inventory_unlock_report(
        ...
        *,
        ...
        blocked = [self._multi_blocker_asset_row(item) for item in readiness if not item.get("canHandoff")]
        combo_rows = self._multi_blocker_combo_rows(blocked, current_inventory=current, required_inventory=required_inventory)
        best_single = self._multi_blocker_best_combo(combo_rows, 1)
        best_two = self._multi_blocker_best_combo(combo_rows, 2)
        best_three = self._multi_blocker_best_combo(combo_rows, 3)
        minimal = self._multi_blocker_minimal_fix_set(combo_rows, current_inventory=current, required_inventory=required_inventory)
        all_repairable_gain = self._multi_blocker_assets_unlocked(blocked, list(self.MULTI_BLOCKER_REPAIR_CLASSES))
    ...
    def _multi_blocker_asset_row(self, readiness: dict[str, Any]) -> dict[str, Any]:
        ...
        repair_classes = sorted({
            repair_class for repair_class in (self._multi_blocker_repair_class(reason) for reason in blockers)
            if repair_class
        })
        unrepairable = [reason for reason in blockers if not self._multi_blocker_repair_class(reason)]
        return {
        ...
        *,
        ...
        for size in range(1, len(classes) + 1):
            for combo in combinations(classes, size):
                ...
                assets_unlocked = self._multi_blocker_assets_unlocked(blocked_assets, combo_list)
                minutes = self._multi_blocker_estimated_minutes(blocked_assets, combo_list)
                rows.append({
                    "repairCombo": combo_list,
                    "repairClasses": combo_list,
                    "assetsUnlocked": assets_unlocked,
                    "estimatedOperatorMinutes": minutes,
                    "repairDifficulty": self._multi_blocker_combo_difficulty(combo_list),
    ...
    def _contentforge_visual_qc_failure_for_asset(self, asset: dict[str, Any], surface: str) -> dict[str, Any]:
        readiness = self._surface_handoff_readiness_for_asset(asset)
        publishability = self.explain_publishability(str(asset["id"])) if surface == "reel" else {}
        ...
        category = "" if visual_passed else self._contentforge_visual_qc_failure_category(asset, blockers, readiness, publishability)
        non_visual_blockers = [
            blocker for blocker in blockers
            if self._contentforge_visual_qc_failure_category(asset, [blocker], readiness, publishability) != category
        ]
        inventory_gain = (
            1
            if category
            and self.CONTENTFORGE_VISUAL_QC_REPAIRABLE.get(category, False)
            and self._contentforge_non_visual_gates_pass(checks, readiness, publishability, non_visual_blockers)
            else 0
        )
        return {
            "assetId": asset["id"],
    ...
    def _schedule_safe_production_assets(
        ...
        *,
        ...
        return [
            asset for asset in assets
            if normalize_content_surface(asset.get("content_surface") or asset.get("source_content_surface")) == content_surface
            and self._schedule_safe_asset_created_at(asset) >= cutoff
        ]
    ...
    def _schedule_safe_production_waterfall_rows(self, assets: list[dict[str, Any]], surface: str) -> list[dict[str, Any]]:
        ...
        caption_families = self._schedule_safe_related_count("caption_families", "parent_asset_id", parent_ids)
        caption_versions = self._schedule_safe_related_count("caption_versions", "parent_asset_id", parent_ids)
        checks = [self._schedule_safe_production_variant_checks(asset, surface) for asset in variant_assets]
    ...
    def _fresh_reel_current_schedule_safe_inventory(
        ...
        *,
        ...
        assets = [
            ...
            if normalize_content_surface(asset.get("content_surface") or asset.get("source_content_surface")) == "reel"
        ]
        ...
        *,
        ...
        for stage, yield_pct, risk in self.FRESH_REEL_STAGE_YIELDS:
            ...
            rows.append({
                ...
                "risk": risk,
                "evidence": self._fresh_reel_stage_evidence(stage),
            })
        return rows
    ...
    def _inventory_recovery_blocked_asset(self, readiness: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(blockers) > 1 and "handoff_manifest_missing" in blockers:
            blockers = [reason for reason in blockers if reason != "handoff_manifest_missing"]
        classes = sorted({self._inventory_recovery_class_for_blocker(reason) for reason in blockers})
        return {
            "assetId": readiness.get("assetId") or "",
    ...
    def _inventory_recovery_class_rows(self, blocked_assets: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for repair_class in self.INVENTORY_RECOVERY_CLASSES:
            ...
            unlocked = self._inventory_recovery_assets_unlocked(blocked_assets, [repair_class])
    ...
    def inventory_factory_readiness_report(
        ...
        *,
        ...
        audit = self.inventory_factory_audit(accounts=accounts, posts_per_account_per_day=posts_per_account_per_day)
        ...
        exceptions = self.inventory_exception_audit(execution_readiness=execution_readiness)
        daily_demand = int(slo.get("dailyPostObligations") or 0)
        current_score = self._score_fraction(available_inventory, int(slo.get("minimumValidatedDraftBuffer") or 1))
        capacity_score = self._score_fraction(audit.get("scheduleSafeInventoryCapacity") or 0, daily_demand)
        buffer_score = self._score_fraction(available_inventory, int(slo.get("minimumValidatedDraftBuffer") or 1))
        yield_score = round(10 * min(1.0, max(
        ...
        exception_score = max(0.0, round(10 - ((exceptions.get("avoidableLossPct") or 0) / 20), 1))
        overall = round((current_score + capacity_score + buffer_score + yield_score + exception_score) / 5, 1)
        return {
    ...
    def inventory_factory_master_report(
        ...
        *,
        ...
        simulation = self.inventory_consumption_simulation(available_inventory=available_inventory)
        ...
        return {
            ...
            "requirementsFor200Accounts": self._road_to_accounts_payload(
                accounts=200,
                production=self.inventory_production_requirements(accounts=200, posts_per_account_per_day=posts_per_account_per_day),
            ),
            "requirementsFor500Accounts": self._road_to_accounts_payload(
                accounts=500,
                production=self.inventory_production_requirements(accounts=500, posts_per_account_per_day=posts_per_account_per_day),
            ),
            "wouldWrite": False,
        }
        ...
        *,
        ...
        for account in daily.get("accounts") or []:
            ...
            if str(account.get("state") or "") == "blocked" and reason:
                exceptions.append(self._exception_queue_item(
                    severity="high",
            ...
            for surface, row in (creator.get("surfaceShortfalls") or {}).items():
                if isinstance(row, dict) and int(row.get("shortfall") or 0) > 0:
                    exceptions.append(self._exception_queue_item(
                        ...
                        account="",
                        asset="",
                        reason=f"{normalize_content_surface(surface)}_inventory_shortfall",
                        next_action="fill_surface_inventory_buffer",
                        count=int(row.get("shortfall") or 0),
                    ))
        for blocker in readiness.get("blockers") or []:
            exceptions.append(self._exception_queue_item(
                severity=self._exception_severity_for_reason(str(blocker)),
                system="execution_readiness",
                account="",
                asset="",
                reason=str(blocker),
                next_action=self._exception_next_action(str(blocker)),
            ))
        for source, system in ((publishability_report or {}, "publishability"), (surface_readiness_report or {}, "surface_readiness")):
            for item in source.get("assets") or source.get("items") or []:
                ...
                for reason in item.get("blockingReasons") or item.get("failureReasons") or []:
                    exceptions.append(self._exception_queue_item(
                        severity=self._exception_severity_for_reason(str(reason)),
                        system=system,
                        account=item.get("accountId") or "",
                        asset=item.get("assetId") or item.get("renderedAssetId") or "",
                        reason=str(reason),
                        next_action=self._exception_next_action(str(reason)),
                    ))
        return {
    ...
    def inventory_autopilot_plan(
        ...
        *,
        ...
        days_remaining = round(max(0, int(available_inventory or 0)) / max(1, daily_demand), 2)
        repair_actions = self._inventory_repair_actions(policy)
        return {
            "schema": "creator_os.inventory_autopilot_plan.v1",
            "surface": normalize_content_surface(surface),
            "dailyDemand": daily_demand,
        ...
        *,
        ...
        for key, expected in criteria.items():
            if actuals.get(key) != expected:
                blockers.append(self._live_acceptance_blocker_for(key))
        return {
    ...
    def _live_acceptance_actuals(
        ...
        *,
        ...
        return {
            "missedDispatches": self._live_acceptance_missed_dispatches(threadsdash_report),
            "duplicatePublishes": self._live_acceptance_duplicate_publishes(threadsdash_report),
            "restrictedAccountsScheduled": self._live_acceptance_restricted_scheduled(threadsdash_report),
            "surfaceContractViolations": self._live_acceptance_surface_contract_violations(threadsdash_report),
            "inventoryBufferMaintained": available_inventory >= required_inventory,
            "metricsImported": self._live_acceptance_metrics_imported(),
    ...
    def idempotency_proof(self) -> dict[str, Any]:
        ...
        for name in path_names:
            ...
            row = {
                ...
                "idempotent": True,
                "evidence": self._idempotency_evidence_for_path(name),
                "wouldWrite": False,
            }
            rows.append(row)
    ...
    def core_complexity_reduction_plan(self) -> dict[str, Any]:
        files = self._largest_project_files()
        return {
    ...
    def _inventory_slo_surface_targets(self, minimum_buffer: int) -> dict[str, int]:
        ...
        targets = {surface: int(minimum_buffer * weights[surface]) for surface in CONTENT_SURFACES}
    ...
    def _inventory_stage_counts(self, *, creator: str | None = None, campaign_slug: str | None = None) -> dict[str, int]:
        ...
        if campaign_slug or creator:
            parent_assets = sum(1 for asset in assets if not asset.get("parent_asset_id"))
            variant_assets = sum(1 for asset in assets if asset.get("parent_asset_id") or asset.get("variant_id"))
        else:
            parent_assets = int(self.conn.execute("SELECT COUNT(*) AS c FROM concepts").fetchone()["c"])
            variant_assets = int(self.conn.execute("SELECT COUNT(*) AS c FROM variant_assets").fetchone()["c"])
        caption_families = self._inventory_count_related("caption_families", "parent_asset_id", asset_ids)
        caption_versions = self._inventory_count_related("caption_versions", "parent_asset_id", asset_ids)
        readiness = self._build_surface_readiness(assets)
        schedule_safe = sum(1 for item in readiness if item.get("canHandoff"))
        validated = sum(1 for asset in assets if str(asset.get("review_state") or "").lower() in {"approved", "review_ready"})
        publishable = sum(
            1 for asset in assets
            if str(asset.get("audit_status") or "").lower() in {"passed", "pass", "approved", "approved_candidate"}
            or str(asset.get("review_state") or "").lower() in {"approved", "review_ready"}
        )
        return {
    ...
    def _inventory_limiting_stage(self, counts: dict[str, int]) -> str:
        ...
        if int(counts.get("publishableAssets") or 0) < int(counts.get("validatedAssets") or 0):
            return "publishability"
        if int(counts.get("validatedAssets") or 0) < int(counts.get("variantAssets") or 0):
            return "validation"
    ...
    def _inventory_loss_by_stage(self, counts: dict[str, int]) -> dict[str, int]:
        return {
            "parentsWithoutVariants": max(0, int(counts.get("parentAssets") or 0) - int(counts.get("variantAssets") or 0)),
            "variantsNotValidated": max(0, int(counts.get("variantAssets") or 0) - int(counts.get("validatedAssets") or 0)),
            "validatedNotPublishable": max(0, int(counts.get("validatedAssets") or 0) - int(counts.get("publishableAssets") or 0)),
            "publishableNotScheduleSafe": max(0, int(counts.get("publishableAssets") or 0) - int(counts.get("scheduleSafeAssets") or 0)),
        }
    ...
    def _exception_queue_item(
        ...
        *,
        ...
        category = self._exception_category_for_reason(reason, system)
        owner = self._exception_owner_for_category(category, system)
        ...
        payload = {
            ...
            "reason": reason,
            "nextAction": next_action,
            "repairable": self._exception_repairable(reason),
            "estimatedResolutionMinutes": self._exception_resolution_minutes(reason, count=count),
    ...
    def _inventory_repair_actions(self, policy: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [
            {
                "action": "expand_existing_safe_parents" if surface == "reel" else "register_surface_safe_assets",
                "surface": surface,
    ...
    def _concept_for_parent_asset(self, parent_asset_id: str) -> dict[str, Any] | None:
        ...
        return self._concept_payload(row) if row else None
    ...
    def prepare_reel_inputs(
        ...
        *,
        ...
        try:
            sources = [source for source in self.assets_for_campaign(campaign["id"]) if source.get("media_type") == "video"]
            if not sources:
            ...
            prepared = []
            reused_existing = []
            next_num = self._next_reel_clip_number(raw_dir)
            for source_index, source in enumerate(sources):
                existing = self.conn.execute("SELECT * FROM render_jobs WHERE source_asset_id = ?", (source["id"],)).fetchone()
                ...
                source_hooks = self._rotate_hooks_for_source(hooks, source_index)
                generation_gate = self.discoverability_generation_gate({"hook": source_hooks})
                ...
                render_hooks, hook_metadata = self._reel_sidecar_hooks(source_hooks)
                sidecar = {
                    "hooks": render_hooks,
                ...
                (cap_dir / f"{clip_stem}.json").write_text(json.dumps(sidecar, indent=2, ensure_ascii=False), encoding="utf-8")
                ...
                self.conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'prepared', ?, ?)
                    """,
                    (job_id, campaign["id"], source["id"], clip_stem, json.dumps(source_hooks, ensure_ascii=False), json.dumps(recipes or []), caption_color, now, now),
                )
                render_graph_id = self.ensure_graph_node(
                    "render_job",
                    local_table="render_jobs",
                    local_id=job_id,
                    payload={"campaignId": campaign["id"], "sourceAssetId": source["id"], "clip": clip_stem, "recipes": recipes or []},
                )
                self.ensure_graph_edge(
                    self.graph_id_for("source_assets", source["id"], entity_type="source_asset"),
                    render_graph_id,
    ...
    def run_reel_factory(
        ...
        *,
        ...
        try:
            ...
            for job in jobs:
                cmd = [
                    reel_factory_python(self.settings.reel_factory_root),
                    "reel_pipeline.py",
                    "--root",
                ...
                result = subprocess.run(cmd, cwd=self.settings.reel_factory_root, text=True, capture_output=True, check=False)
            ...
            if returncode == 0:
                ...
                self.fail_pipeline_job(pipeline_job["id"], f"reel_factory returned {returncode}", {
                    "returncode": returncode,
                    "runCount": len(runs),
                    "elapsed_seconds": result_payload["elapsed_seconds"],
                })
    ...
    def sync_reel_outputs(self, *, campaign_slug: str) -> dict[str, Any]:
        campaign = self.campaign_by_slug(campaign_slug)
        pipeline_job = self.create_pipeline_job("sync_reel", campaign["id"], {"campaign": campaign_slug})
        self.start_pipeline_job(pipeline_job["id"])
        model_slug = self._model_slug_for_campaign(campaign["id"])
        dirs = self.campaign_dirs(model_slug, campaign["slug"])
        ...
        try:
            ...
            for job in jobs:
                ...
                for row in rows:
                    ...
                    if existing:
                        self._backfill_synced_reel_output_lineage(
                            asset=dict(existing),
                    ...
                    caption_generation = self._caption_generation_for_clip(job["reel_clip_stem"])
                    lineage = {**caption_generation, **reel_lineage}
                    caption_context = self._caption_outcome_context_for_reel_output(
                        clip_stem=job["reel_clip_stem"],
                    ...
                    if caption_generation.get("audioIntent") is None:
                        audio_intent = self._audio_intent_from_reference_recommendations(caption_generation, now=now)
                        if audio_intent:
                            caption_generation["audioIntent"] = audio_intent
                    if caption_text:
                    ...
                    self.conn.execute(
                        ...
                        (
                            ...
                            json.dumps(sanitize_for_storage(caption_generation), ensure_ascii=False, sort_keys=True),
                            row["recipe"], params.get("_target_ratio") or self._ratio_from_filename(dest.name),
                    ...
                    rendered_graph_id = self.ensure_graph_node(
                        "rendered_asset",
                    ...
                    self.ensure_graph_edge(
                        self.graph_id_for("render_jobs", job["id"], entity_type="render_job"),
                        rendered_graph_id,
                        "render_job_to_rendered_asset",
                        evidence={"jobKey": row["job_key"], "pipelineJobId": pipeline_job["id"]},
                    )
                    self.ensure_graph_edge(
                        self.graph_id_for("source_assets", job["source_asset_id"], entity_type="source_asset"),
                        rendered_graph_id,
            ...
            self.conn.commit()
            self.finish_pipeline_job(pipeline_job["id"], {"syncedCount": len(synced), "newSyncedCount": new_synced})
            return result
        except Exception as exc:
            self.record_event(
                "rendered_asset_synced",
    ...
    def _caption_outcome_context_for_reel_output(
        ...
        *,
        ...
        lineage_placement_decision = self._lineage_placement_decision(lineage)
        placement_policy = (
            context.get("captionPlacementPolicy")
            or self._lineage_first_present(lineage, "captionPlacementPolicy")
            or "focal_safe_v1"
        )
        ...
        if not isinstance(placement_decision, dict) or not placement_decision.get("status"):
            placement_decision = {
                "status": "pending",
                "selectedLane": self._caption_lane_from_render_recipe(recipe),
    ...
    def _backfill_synced_reel_output_lineage(
        ...
        *,
        ...
        caption_generation = {**self._caption_generation_for_clip(clip_stem), **reel_lineage, **existing_generation}
        caption_context = self._caption_outcome_context_for_reel_output(
            clip_stem=clip_stem,
    ...
    def make_batch(
        ...
        *,
        ...
        selected_format = output_format if output_format in {"reel", "slideshow", "auto"} else "auto"
        pipeline_job = self.create_pipeline_job(
            "make_batch",
            None,
            {
        ...
        try:
            ...
            self.set_pipeline_job_campaign(pipeline_job["id"], imported["campaign"]["id"])

            if not self.reference_patterns(limit=1)["patterns"]:
                bank_path = self.settings.reference_reels_root / "learning" / "campaign_reference_bank.json"
                prompt_pack_path = self.settings.reference_reels_root / "learning" / "higgsfield_prompt_pack_top300.json"
                if bank_path.exists():
                    reference_import = self.import_reference_bank(
                        bank_path,
            ...
            cluster_key = None if not reference_pattern or reference_pattern == "auto" else reference_pattern
            source_mix = self._campaign_source_media_summary(imported["campaign"]["id"])
            formats_to_run = self._formats_for_batch(selected_format, source_mix)
            result["sourceMix"] = source_mix
            result["formatsRun"] = formats_to_run
            source_prompt_payload = json_load(source_prompt, {}) if source_prompt else {}
            finished_format_type = (
                source_prompt_payload.get("formatType")
                if isinstance(source_prompt_payload, dict)
                and source_prompt_payload.get("schema") == "campaign_factory.finished_video_intake.v1"
                else None
            )
            reel_caption_band = self._finished_video_caption_band(str(finished_format_type)) if finished_format_type else "auto"
            reel_caption_font = self._finished_video_caption_font(str(finished_format_type)) if finished_format_type else "Instagram Sans Condensed"

            prepared_by_format: dict[str, Any] = {}
            pattern: dict[str, Any] = {}
            for format_name in formats_to_run:
                if format_name == "slideshow":
                    prepared = self._run_slideshow_pack(
                        campaign_slug=campaign_slug,
                        variant_count=max(1, min(int(variant_count or 20), 100)),
                        title=campaign_slug.replace("_", " ").title(),
                        cluster_key=cluster_key,
                        media_types={"image"} if source_mix.get("image", 0) else {"video"},
                    )
                    ...
                    if finished_format_type:
                        ...
                        hooks = self.finished_video_hooks(
                    ...
                    run_result = self.run_reel_factory(
                        campaign_slug=campaign_slug,
                        workers=workers,
                        dry_run=False,
                    ...
                    sync_result = self.sync_reel_outputs(campaign_slug=campaign_slug)
                    sync_retries: list[dict[str, Any]] = []
                    prepared_jobs = (prepared.get("prepare") or {}).get("prepared") or prepared.get("prepared") or []
                    ...
                    if (
                        ...
                        time.sleep(1.0)
                        sync_retry = self.sync_reel_outputs(campaign_slug=campaign_slug)
                        ...
                        if sync_retry_count >= expected_sync_count:
                            ...
                            rerun_result = self.run_reel_factory(
                                campaign_slug=campaign_slug,
                                workers=workers,
                                dry_run=False,
                                caption_band=reel_caption_band,
                    ...
                    prepared["sync"] = {"syncedCount": len(sync_result.get("synced") or []), "retries": sync_retries}
                if not pattern:
            ...
            result["referenceSelection"] = {
                ...
                "recipes": recipes or ((pattern.get("raw") or {}).get("bank") or {}).get("suggestedVariantRecipes") or [],
            }
            prepare_payloads = [item.get("prepare") or {} for item in prepared_by_format.values()]
            result["prepare"] = {
                "preparedCount": sum(len(payload.get("prepared") or []) for payload in prepare_payloads),
                "reusedExistingCount": sum(len(payload.get("reusedExisting") or []) for payload in prepare_payloads),
                "sourceCount": sum(source_mix.values()),
                "byFormat": {
                    format_name: {
                        "preparedCount": len((payload.get("prepare") or {}).get("prepared") or []),
                        "reusedExistingCount": len((payload.get("prepare") or {}).get("reusedExisting") or []),
                    }
                    for format_name, payload in prepared_by_format.items()
                },
            }
            result["run"] = {
                "runCount": sum((payload.get("run") or {}).get("runCount") or 0 for payload in prepared_by_format.values()),
                "byFormat": {format_name: payload.get("run") for format_name, payload in prepared_by_format.items()},
            }
            result["sync"] = {
                "syncedCount": sum((payload.get("sync") or {}).get("syncedCount") or len((payload.get("prepare") or {}).get("prepared") or []) for payload in prepared_by_format.values()),
                "byFormat": {format_name: payload.get("sync") or {"syncedCount": len((payload.get("prepare") or {}).get("prepared") or [])} for format_name, payload in prepared_by_format.items()},
            }
            ...
            result["audit"] = {
                ...
                "failedCount": sum(1 for report in reports if report.get("failedChecks")),
            ...
            if auto_approve_warning_only:
                for asset in self.dashboard(campaign_slug).get("rendered") or []:
                    ...
                    if latest and not blocking:
                        ...
                        result["reviewReady"].append({"renderedAssetId": asset["id"], "filename": asset["filename"]})
                self.conn.commit()

            if user_id:
                readiness_result = evaluate_export_readiness(self, campaign_slug=campaign_slug, user_id=user_id)
                result["readiness"] = {
                    "expectedDraftCount": readiness_result.get("expectedDraftCount"),
                    "liveExportAllowed": readiness_result.get("liveExportAllowed"),
                    "blockingReasonCount": len(readiness_result.get("blockingReasons") or []),
                    "warningCount": len(readiness_result.get("warnings") or []),
                }
                if dry_run_export:
                    export_result = export_threadsdash(self, campaign_slug=campaign_slug, user_id=user_id, dry_run=True)
                    result["dryRunExport"] = {
                        "draftCount": export_result.get("draftCount"),
                        "path": export_result.get("path"),
                        "dryRun": export_result.get("dryRun"),
                    }
            health = self.campaign_health(campaign_slug)
            ranking = self.ranking(campaign_slug)["assets"][:5]
            result["dashboard"] = {
                "health": health,
                "topRanked": [
                    {
    ...
    def intake_finished_video(
        ...
        *,
        ...
        campaign = slugify(campaign_slug or f"finished_{source.stem}")
        digest = sha256_file(source)
        source_probe = probe_video_shape(source)
        source_preflight = self._finished_video_preflight(source_probe)
        intake_dir = self.settings.campaigns_dir / "_finished_video_intake" / f"{campaign}_{digest[:10]}"
        intake_dir.mkdir(parents=True, exist_ok=True)
        staged = intake_dir / f"{slugify(source.stem)}_{digest[:10]}{source.suffix.lower()}"
        if not staged.exists():
            shutil.copy2(source, staged)
        style_lane_format = self._finished_video_style_lane_format(style_lane)
        format_type = style_lane_format or self._classify_finished_video_format(source)
        creative_plan_payload = self.creative_plan(creative_plan) if creative_plan else None
        ...
        result["finishedVideoIntake"] = {
            ...
            "goal": goal,
            "formatType": format_type,
            "sourcePrompt": json_load(source_prompt, {}),
        ...
        if creative_plan_payload and campaign:
            self.conn.execute("UPDATE creative_plans SET linked_campaign_slug = ?, updated_at = ? WHERE id = ?", (campaign, utc_now(), creative_plan_id))
            self._record_creative_plan_event(creative_plan_id, "finished_video_ingested", status="success", message=f"Finished video ingested: {source.name}", metadata={"campaign": campaign, "inputPath": str(source)})
        return result
    ...
    def archive_inventory_report(
        ...
        *,
        ...
        for index, path in enumerate(video_paths, start=1):
            ...
            existing_duplicate = self._archive_existing_content_duplicate(digest)
            if existing_duplicate:
                blocking_reasons.append("duplicate_existing_campaign_asset")
                duplicate_content_hash += 1

            recent_duplicate = self._archive_recent_publish_duplicate(digest, recent_cutoff)
            if recent_duplicate:
    ...
    def archive_candidate_quality_report(
        ...
        *,
        ...
        for item in clean_items:
            ...
            severity, crop_score, crop_delta = self._archive_crop_severity(probe)
            visual_score = self._archive_visual_quality_score(probe, item.get("warnings") or [], crop_score)
            duplicate_confidence = self._archive_duplicate_confidence(item)
    ...
    def _finished_video_style_lane_format(self, style_lane: str | None) -> str | None:
        normalized = slugify(style_lane or "")
        aliases = {
    ...
    def _run_slideshow_pack(
        ...
        *,
        ...
        sources = [asset for asset in self.assets_for_campaign(campaign["id"]) if asset.get("media_type") in desired_types]
        if not sources:
            raise ValueError("no imported image/video sources available for slideshow pack")
        selection = self.select_reference_pattern(
            campaign_slug,
            cluster_key=cluster_key,
            variant_count=variant_count,
        ...
        stamp = re.sub(r"[^0-9A-Za-z]+", "_", utc_now()).strip("_")
        ...
        hooks_file.write_text(json.dumps({"hooks": hooks}, indent=2, ensure_ascii=False), encoding="utf-8")
        seed = int(hashlib.sha256(f"{campaign_slug}:{variant_count}:{selection['pattern']['clusterKey']}".encode("utf-8")).hexdigest()[:8], 16)
        generation_id = new_id("slidegen")
        cmd = [
            reel_factory_python(self.settings.reel_factory_root),
        ...
        if existing:
                    ...
                    json.dumps(
                        {
                            ...
                            "generationId": manifest.get("generation_id") or generation_id,
                            "referencePattern": selection.get("pattern"),
                            "sourceHashes": [item.get("source_hash") for item in manifest.get("items", [])],
                            "captionHashes": [
                                hashlib.sha256(str(item.get("hook") or "").strip().lower().encode("utf-8")).hexdigest()
            ...
            prepared_assets.append(dict(self.conn.execute("SELECT * FROM rendered_assets WHERE id = ?", (rendered_id,)).fetchone()))
            self.record_event(
                "rendered_asset_synced",
                campaign_id=campaign["id"],
                source_asset_id=source["id"],
                ...
                metadata={"format": "slideshow_pack", "manifestPath": str(manifest_path), "contentHash": digest},
                commit=False,
            )
        self.conn.commit()
        return {
    ...
    def _campaign_source_media_summary(self, campaign_id: str) -> dict[str, int]:
        ...
        for source in self.assets_for_campaign(campaign_id):
            media_type = source.get("media_type") or media_type_for_path(source.get("stored_path") or source.get("filename") or "")
    ...
    def _formats_for_batch(self, selected_format: str, source_mix: dict[str, int]) -> list[str]:
        ...
        if selected_format == "slideshow":
            if not (source_mix.get("image") or source_mix.get("video")):
                raise ValueError("slideshow format requires at least one imported image or video")
            return ["slideshow"]
        formats: list[str] = []
    ...
    def surface_handoff_readiness_report(
        ...
        *,
        ...
        assets = self._surface_report_assets(creator=self._creator_label(creator) if creator else None, campaign_slug=campaign_slug)
        if rendered_asset_id:
    ...
    def surface_draft_proof(
        ...
        *,
        ...
        drafts = [
            self._surface_draft_payload_for_readiness(item)
        ...
        return {
            ...
            "creator": self._creator_label(creator) if creator else None,
            "campaign": slugify(campaign) if campaign else None,
            "renderedAssetId": rendered_asset_id,
    ...
    def _surface_report_assets(self, *, creator: str | None = None, campaign_slug: str | None = None) -> list[dict[str, Any]]:
        ...
        if campaign_slug:
            where.append("c.slug = ?")
            params.append(slugify(campaign_slug))
        query = """
            SELECT r.*, c.slug AS campaign_slug, s.media_type AS source_media_type,
                   s.content_surface AS source_content_surface, m.slug AS model_slug, m.name AS model_name
            FROM rendered_assets r
    ...
    def recommend_next_batch(
        ...
        *,
        ...
        if persist:
            ...
            self.conn.execute(
                ...
                (
                    ...
                    campaign["id"],
                    input_hash,
                    json.dumps(sanitize_for_storage(input_snapshot), ensure_ascii=False, sort_keys=True),
                    now,
                    now,
                ),
            )
        ...
        for rank, candidate in enumerate(candidates[:max(1, int(count))], start=1):
            ...
            item = self._recommendation_item_payload(
                campaign=campaign,
        ...
        if not items:
            ...
            fallback = self._reference_only_recommendation_item(
                campaign=campaign,
                campaign_graph_id=campaign_graph_id,
                run_graph_id=run_graph_id,
                account=account,
    ...
    def recommendation_runs(self, campaign_slug: str, *, limit: int = 10) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            runs.append({
                ...
                "updatedAt": row["updated_at"],
                "items": [self._stored_recommendation_item_payload(dict(item)) for item in items],
            })
    ...
    def _recommendation_item_payload(
        ...
        *,
        ...
        performance_score = int(asset.get("performanceScore") or self._best_asset_history_score(asset) or 50)
        reference_score = self._reference_pattern_score(reference_pattern)
        audit_score = int(breakdown.get("quality") or 50)
        account_score = self._recommendation_account_score(asset, account)
        account_fit_evidence = self._recommendation_account_fit_evidence(campaign["id"], asset, account)
        novelty_score = int(breakdown.get("novelty") or 50)
        operational_score = self._operational_recommendation_score(asset)
        score = round(
        ...
        audio_recommendations = self.recommend_audio(
            ...
            account_tags=[str(tag) for tag in [account or self._asset_target_account(asset)] if tag],
            account=account or self._asset_target_account(asset),
            limit=5,
        )
        ...
        output = {
            ...
            "targetAccount": account or self._asset_target_account(asset),
        ...
        if persist:
            ...
            self.conn.execute(
                ...
                (
                    ...
                    json.dumps(sanitize_for_storage(evidence), ensure_ascii=False, sort_keys=True),
                    json.dumps(sanitize_for_storage(data_quality), ensure_ascii=False, sort_keys=True),
                    json.dumps(sanitize_for_storage(output), ensure_ascii=False, sort_keys=True),
                    utc_now(),
                ),
            )
            self._write_recommendation_graph_edges(
            ...
            self._write_audio_recommendation_graph_edges(
        ...
        if persist:
            ...
            self.conn.execute(
                ...
                (
                    ...
                    json.dumps(output["risks"], ensure_ascii=False, sort_keys=True),
                    json.dumps(sanitize_for_storage(output["evidence"]), ensure_ascii=False, sort_keys=True),
                    json.dumps(sanitize_for_storage(output["dataQuality"]), ensure_ascii=False, sort_keys=True),
                    json.dumps(sanitize_for_storage(output), ensure_ascii=False, sort_keys=True),
                    utc_now(),
                ),
            )
    ...
    def _stored_recommendation_item_payload(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(payload, dict):
            ...
            payload["exceptions"] = self._exceptions_for_recommendation(row["id"])
            return payload
        return {
            ...
            "autonomyLevel": self.autonomy_level(),
            "exceptions": self._exceptions_for_recommendation(row["id"]),
        }
    ...
    def recommendation_item(self, recommendation_item_id: str) -> dict[str, Any]:
        row = self._recommendation_item_row(recommendation_item_id)
        return self._stored_recommendation_item_payload(row)
    ...
    def accept_recommendation_item(
        ...
        *,
        ...
        decision = {"action": "accepted", "operator": operator, "notes": notes, "decidedAt": utc_now()}
        return self._update_recommendation_lifecycle(
            recommendation_item_id,
            status="accepted",
            decision=decision,
    ...
    def reject_recommendation_item(
        ...
        *,
        ...
        decision = {"action": "rejected", "reason": reason, "operator": operator, "notes": notes, "decidedAt": utc_now()}
        return self._update_recommendation_lifecycle(
            recommendation_item_id,
            status="rejected",
            decision=decision,
    ...
    def link_recommendation_item(
        ...
        *,
        ...
        if performance_snapshot_id:
            ...
            status = "posted"
        self._validate_recommendation_transition(
        ...
        if evidence:
            link_evidence.setdefault("operatorEvidence", []).append({"at": now, **sanitize_for_storage(evidence)})
        if admin_override:
        ...
        params: list[Any] = [status, "completed", json.dumps(sanitize_for_storage(link_evidence), ensure_ascii=False, sort_keys=True), "", now]
        if status == "posted":
        ...
        for column, value in updates.items():
            ...
            params.append(value)
        payload = self._stored_recommendation_item_payload(dict(row))
        ...
        params[3] = json.dumps(sanitize_for_storage(payload), ensure_ascii=False, sort_keys=True)
    ...
    def measure_recommendation_item(
        ...
        *,
        ...
        if performance_snapshot_id:
            ...
            row = self._recommendation_item_row(recommendation_item_id)
        perf_rows = self._recommendation_performance_rows(row)
        if not perf_rows:
        ...
        *,
        ...
        try:
            if not linked_rendered_id and row.get("reference_pattern_id"):
                ...
                steps.append({"step": "run_reel", "status": "completed", "result": self._compact_execution_result(rendered)})
                synced = self.sync_reel_outputs(campaign_slug=campaign["slug"])
                steps.append({"step": "sync_reel", "status": "completed", "result": self._compact_execution_result(synced)})
            ...
            if run_audit and linked_rendered_id:
                ...
                steps.append({"step": "audit", "status": "completed", "result": self._compact_execution_result(audit_result)})
            current = self._recommendation_item_row(recommendation_item_id)
            asset_payload = None
            if linked_rendered_id:
                asset_payload = self._dashboard_rendered_asset(self.rendered_asset(linked_rendered_id))
                self._create_trust_exceptions_for_recommendation(current, asset_payload, commit=False)
            exceptions = self._exceptions_for_recommendation(recommendation_item_id)
            blocking_exceptions = [
            ...
            payload = self._stored_recommendation_item_payload(current)
            ...
            self.conn.execute(
                ...
                (execution_status, json.dumps(sanitize_for_storage(payload), ensure_ascii=False, sort_keys=True), recommendation_item_id),
            )
            self.record_event(
    ...
    def _compact_execution_result(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        return compact or sanitize_for_storage(result)
    ...
    def _create_trust_exceptions_for_recommendation(
        ...
        *,
        ...
        account_id = row.get("target_account") or self._asset_target_account(asset)
        ...
        account_fit = self._recommendation_account_fit_evidence(campaign["id"], asset, account_id)
        if not account_id:
        ...
        if audio.get("recommendations") and not self._asset_has_final_audio_proof(asset):
            created.append(self.create_exception(
    ...
    def _update_recommendation_lifecycle(
        ...
        *,
        ...
        payload = self._stored_recommendation_item_payload(row)
        now = utc_now()
        payload["status"] = status
        if decision is not None:
            payload["decision"] = sanitize_for_storage(decision)
        if outcome is not None:
            payload["outcome"] = sanitize_for_storage(outcome)
        if baseline is not None:
            payload["baseline"] = sanitize_for_storage(baseline)
        ...
        params: list[Any] = [status, json.dumps(sanitize_for_storage(payload), ensure_ascii=False, sort_keys=True)]
        if decision is not None:
            set_parts.append("decision_json = ?")
            params.append(json.dumps(sanitize_for_storage(decision), ensure_ascii=False, sort_keys=True))
        if outcome is not None:
            set_parts.append("outcome_json = ?")
            params.append(json.dumps(sanitize_for_storage(outcome), ensure_ascii=False, sort_keys=True))
        if baseline is not None:
            set_parts.append("baseline_json = ?")
            params.append(json.dumps(sanitize_for_storage(baseline), ensure_ascii=False, sort_keys=True))
        if measurement_version is not None:
    ...
    def _recommendation_account_score(self, asset: dict[str, Any], account: str | None) -> int:
        evidence = self._recommendation_account_fit_evidence(asset["campaign_id"], asset, account)
    ...
    def _recommendation_account_fit_evidence(
        ...
        target = account or self._asset_target_account(asset)
        if not target:
    ...
    def campaign_readiness(self, campaign_slug: str, *, user_id: str | None = None) -> dict[str, Any]:
        ...
        jobs = self.jobs_for_campaign(campaign_slug, limit=100)
        blocking = []
        warnings = []
    ...
    def lifecycle_report(
        ...
        *,
        ...
        for assignment in self.assignments_for_campaign(campaign_slug):
            assignments_by_asset.setdefault(assignment["rendered_asset_id"], []).append(assignment)
        snapshots_by_asset = self._lifecycle_snapshots_by_asset(campaign["id"])
        posts_by_plan, posts_by_asset, td_evidence = self._lifecycle_threadsdash_indexes(
            campaign_slug=campaign_slug,
            user_id=user_id,
    ...
    def creator_os_lifecycle_dashboard(
        ...
        *,
        ...
        for row in report.get("rows") or []:
            bucket = self._creator_os_lifecycle_bucket(row)
            buckets[bucket] += 1
            rows.append({
    ...
    def _lifecycle_row(
        ...
        *,
        ...
        latest_post = self._latest_lifecycle_post(threadsdash_posts)
        latest_snapshot = snapshots[0] if snapshots else None
        readiness = asset.get("export_readiness") or {}
        context = asset.get("captionOutcomeContext") or load_context_json(asset.get("caption_outcome_context_json")) or {}
        context_fingerprint = self._lifecycle_fingerprint(context) if context else None
        media_issue = self._lifecycle_media_validation_issue(asset=asset, post=latest_post)
        mismatch = self._lifecycle_mismatch(asset=asset, plan=plan, post=latest_post, snapshot=latest_snapshot, context_fingerprint=context_fingerprint)
        current_state, blocking_reason, next_action = self._derive_lifecycle_state(
            asset=asset,
            plan=plan,
        ...
        last_state_change = self._lifecycle_last_state_change(asset=asset, plan=plan, post=latest_post, snapshot=latest_snapshot)
        return {
            "campaignId": campaign["id"],
            "campaign": campaign["slug"],
            "renderedAssetId": asset["id"],
            ...
            "evidence": {
                ...
                "threadsdashPost": self._compact_lifecycle_post(latest_post),
                "performanceSnapshot": self._compact_lifecycle_snapshot(latest_snapshot),
                "lineageMismatch": mismatch,
                "mediaValidation": media_issue,
                "pastDueScheduleResolved": self._lifecycle_past_due_resolved(latest_post),
            },
        }
    ...
    def _derive_lifecycle_state(
        ...
        *,
        ...
        if snapshot and self._lifecycle_snapshot_has_metrics(snapshot):
        ...
        if post:
            ...
            if status == "scheduled":
                if self._lifecycle_is_past_due(post.get("scheduled_for")):
            ...
            meta = self._lifecycle_post_meta(post)
            if (
        ...
        if plan:
            ...
            if blocking:
                return "distribution_planned", self._lifecycle_blocking_reason(blocking), "clear_export_readiness_blockers"
    ...
    def _lifecycle_media_validation_issue(self, *, asset: dict[str, Any], post: dict[str, Any] | None) -> dict[str, Any] | None:
        meta = self._lifecycle_post_meta(post) if post else {}
    ...
    def _lifecycle_is_past_due(self, scheduled_for: Any) -> bool:
        if not isinstance(scheduled_for, str) or not scheduled_for.strip():
            return False
        parsed = self._parse_lifecycle_time(scheduled_for)
    ...
    def _lifecycle_last_state_change(
        ...
        *,
        ...
        parsed = [(self._parse_lifecycle_time(value), value) for value in candidates if value]
    ...
    def _lifecycle_mismatch(
        ...
        *,
        ...
        for label, payload in (("threadsdash", post), ("performance", snapshot)):
            if not payload:
                continue
            meta = self._lifecycle_post_meta(payload) if label == "threadsdash" else {}
            ...
            if context_fingerprint and isinstance(context, dict) and context:
                actual_fingerprint = self._lifecycle_fingerprint(context)
    ...
    def _lifecycle_fingerprint(self, value: Any) -> str:
        payload = self._canonical_lifecycle_context(value)
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                if key == "render_recipe":
                    continue
                canonical = self._canonical_lifecycle_context(item)
                if canonical is None:
                    continue
                normalized[key] = canonical
            return normalized
        if isinstance(value, list):
            return [self._canonical_lifecycle_context(item) for item in value]
        return value
    ...
    def _local_export_readiness(self, asset: dict[str, Any], latest_audit: dict[str, Any] | None) -> dict[str, Any]:
        blocking = []
        warnings = []
        publishability = self._publishability_check(asset, latest_audit)
    ...
    def explain_publishability(
        ...
        *,
        ...
        return self._publishability_check(asset, latest_audit, distribution_plan=plan)
    ...
    def capture_publishability_rejection_evidence(self, rendered_asset_id: str) -> dict[str, Any]:
        ...
        return self._capture_publishability_rejection_evidence_from_result(rendered_asset_id, result, commit=True)
    ...
    def _capture_publishability_rejection_evidence_from_result(
        ...
        *,
        ...
        now = utc_now()
        evidence_rows = [
            row for row in result.get("rejectionEvidence") or []
        ...
        for row in evidence_rows:
            ...
            self.conn.execute(
                ...
                (
                    ...
                    normalize_content_surface(asset.get("content_surface")),
                    row.get("failedStage"),
                    row.get("failureCategory"),
                    row.get("matchedText"),
                    row.get("sourceField"),
                    ...
                    json.dumps(sanitize_for_storage(row), ensure_ascii=False, sort_keys=True),
                    now,
                    now,
                ),
            )
        ...
        for row in evidence_rows:
            ...
            self.conn.execute(
                ...
                (
                    ...
                    source_asset_id,
                    campaign_id,
                    normalize_content_surface(content_surface),
                    failed_stage,
                    row.get("failureCategory"),
                    row.get("matchedText"),
                    row.get("sourceField"),
    ...
    def record_proof_run(
        ...
        *,
        ...
        now = utc_now()
        key = proof_run_id or f"proof_{hashlib.sha256(':'.join(str(part or '') for part in (campaign_id, rendered_asset_id, distribution_plan_id, threadsdash_post_id)).encode('utf-8')).hexdigest()[:12]}"
        self.conn.execute(
            """
            ...
            (
                ...
                now,
                now if status in {"passed", "failed", "complete", "completed"} else None,
                now,
                now,
            ),
        )
        if commit:
            self.conn.commit()
        row = self.conn.execute("SELECT * FROM proof_runs WHERE id = ?", (key,)).fetchone()
    ...
    def _publishability_discoverability_fields(
        ...
        *,
        ...
        candidates = [
            ...
            ("burned_caption_text", post_caption.get("burned_caption_text") or caption_context.get("burned_caption_text")),
            ("instagram_post_caption", post_caption.get("instagram_post_caption") or caption_context.get("instagram_post_caption")),
            ("caption_cta", post_caption.get("caption_cta") or caption_context.get("caption_cta")),
        ]
    ...
    def caption_quality_repair_plan(
        ...
        *,
        ...
        assets = self._surface_report_assets(creator=creator, campaign_slug=campaign_slug)
        if content_surface:
            normalized_surface = normalize_content_surface(content_surface)
            assets = [
                asset for asset in assets
                if normalize_content_surface(asset.get("content_surface") or asset.get("source_content_surface")) == normalized_surface
            ]
        candidates: list[dict[str, Any]] = []
        ...
        for asset in assets:
            ...
            failure_reasons = list(publishability.get("publishability_failure_reasons") or [])
            ...
            non_caption_blockers = sorted(
                ...
                if reason not in {"instagram_post_caption_quality_failed", "unsafe_reel_caption_link_or_dm_reference"}
            )
            post_caption_quality = publishability.get("instagramPostCaptionQuality") if isinstance(publishability.get("instagramPostCaptionQuality"), dict) else {}
            quality_reasons = list(post_caption_quality.get("reasons") or [])
            current_caption = str(publishability.get("instagram_post_caption") or "")
            suggested_caption = self._suggest_simple_instagram_post_caption(
                asset_id=str(asset["id"]),
            ...
            would_pass = bool(suggested_quality.get("passed") and discoverability_contract.get("discoverabilitySafe") and not non_caption_blockers)
            recovery_class = self._caption_quality_recovery_class(quality_reasons)
            ...
            if len(candidates) < max(1, int(limit or 200)):
                candidates.append({
                    ...
                    "recoveryClass": "unrecoverable" if non_caption_blockers else recovery_class,
                    "suggestedInstagramPostCaption": suggested_caption,
                    "suggestedInstagramPostCaptionHash": self._text_hash(suggested_caption) if suggested_caption else "",
                    "suggestedCaptionQuality": suggested_quality,
                    "suggestedDiscoverabilitySafe": bool(discoverability_contract.get("discoverabilitySafe")),
                    "wouldPassQualityGate": would_pass,
                    "burnedCaptionText": publishability.get("burned_caption_text") or "",
                    "burnedCaptionHash": publishability.get("burned_caption_hash") or "",
                    "wouldWrite": False,
                })
        return {
            ...
            "creator": creator,
            "campaign": campaign_slug,
            "contentSurface": normalize_content_surface(content_surface) if content_surface else "",
    ...
    def _caption_quality_recovery_class(self, quality_reasons: list[str]) -> str:
        ...
        if reason_set and reason_set <= {"instagram_post_caption_too_many_hashtags", "instagram_post_caption_too_many_lines"}:
    ...
    def _suggest_simple_instagram_post_caption(self, *, asset_id: str, current_caption: str, burned_caption: str) -> str:
        ...
        for offset in range(len(SIMPLE_INSTAGRAM_POST_CAPTION_REPAIR_POOL)):
            ...
            if normalized == current_normalized or normalized == burned_normalized:
                continue
            quality = self._instagram_post_caption_quality({
    ...
    def _publishability_check(
        ...
        *,
        ...
        output_path = str(asset.get("campaign_path") or asset.get("output_path") or asset.get("filePath") or "")
        ...
        lower_hints = " ".join(str(value or "").lower() for value in (output_path, filename, render_recipe))
        ...
        audit_warning_codes = set(((latest_audit or {}).get("readinessSummary") or {}).get("warningCodes") or [])
        audit_warning_codes.update(latest_audit.get("warnings") or [] if latest_audit else [])
        placement_warning_codes = sorted(audit_warning_codes & CAPTION_PLACEMENT_QC_WARNING_CODES)
        ...
        embedded_audio_verified = self._embedded_audio_verified(output_path) if embedded_audio_required else None
        captioned_render_present = (bool(caption_text) and "passthrough" not in lower_hints) if is_reel_surface else bool(output_path)
        visible_caption_verification = captioned_render_present if is_reel_surface else True
        expected_visual_verification = "passthrough" not in lower_hints
        discoverability_fields = self._publishability_discoverability_fields(
            asset=asset,
            caption_text=caption_text,
        ...
        discoverability_evidence = self._discoverability_evidence_for_fields(discoverability_fields)
        reel_caption_safety_violations = list(discoverability_contract["blockedTerms"]) if is_reel_surface else []
        burned_caption_quality_passed = self._reference_hook_is_schedule_safe(caption_text) if is_reel_surface else True
        checks = {
            ...
            "instagram_post_caption_present": bool(post_caption.get("instagram_post_caption")),
            "caption_placement_qc_passed": placement_qc_passed if is_reel_surface else True,
            "audio_assigned": self._audio_intent_is_attached(audio_intent, audio_id) if is_reel_surface else True,
        ...
        if asset_content_surface == "feed_single" and str(asset.get("media_type") or "").lower() != "image":
        ...
        if not post_caption.get("instagram_post_caption") and asset_content_surface != "story":
        ...
        asset_state = "publishable_candidate" if not failures else "approved_but_not_publishable"
        lifecycle_state = "publishable_candidate" if not failures else ("creative_approved" if approved else "rendered")
        root_cause = None
        if quarantine:
        ...
        manifest = None
        distribution_content_surface = normalize_content_surface((distribution_plan or {}).get("contentSurface") or (distribution_plan or {}).get("content_surface"))
        distribution_surface = _normalize_distribution_surface((distribution_plan or {}).get("surface")) if distribution_plan else None
        ig_media_type = self._ig_media_type_for_surface(distribution_content_surface, str(asset.get("media_type") or "video"))
        instagram_trial_reels = bool((distribution_plan or {}).get("instagramTrialReels") or (distribution_plan or {}).get("instagram_trial_reels"))
        trial_graduation_strategy = (distribution_plan or {}).get("trialGraduationStrategy") or (distribution_plan or {}).get("trial_graduation_strategy")
        if not failures and distribution_plan is not None:
            manifest = {
                ...
                "instagram_post_caption_hash": post_caption["instagram_post_caption_hash"],
                ...
                "igMediaType": ig_media_type,
                "instagram_trial_reels": instagram_trial_reels,
                "trial_graduation_strategy": trial_graduation_strategy if instagram_trial_reels else None,
                "exported_by_system": "campaign_factory",
                "exported_at": utc_now(),
            }
            ...
            if isinstance(caption_context, dict):
                caption_family_id = caption_context.get("caption_family_id") or caption_context.get("captionFamilyId")
                caption_version_id = caption_context.get("caption_version_id") or caption_context.get("captionVersionId")
        ...
        return {
            ...
            "exportable": not failures and distribution_plan is not None and manifest is not None,
            "approved": approved,
            "captionedRenderPresent": captioned_render_present,
            "captioned_render_present": captioned_render_present,
            "visibleCaptionVerification": "heuristic_pass" if visible_caption_verification else "not_verified",
            "visible_caption_verification": visible_caption_verification,
            "expectedVisualVerification": "heuristic_pass" if expected_visual_verification else "failed",
            "expected_visual_verification": expected_visual_verification,
            ...
            **post_caption,
            ...
            "captionPlacementPolicy": placement_policy,
            "captionPlacementDecision": placement_decision,
            "discoverabilitySafe": discoverability_contract["discoverabilitySafe"] if is_reel_surface else True,
            "discoverabilityContract": discoverability_contract if is_reel_surface else {
                **discoverability_contract,
                "discoverabilitySafe": True,
                "blockedTerms": [],
                "blockedReason": "",
            },
            "rejectionEvidence": discoverability_evidence if is_reel_surface and reel_caption_safety_violations else [],
            ...
            **variant_lineage,
            ...
            "blockingReason": blocking_reason,
            "rootCause": root_cause,
            "nextOperatorAction": "replace_with_verified_captioned_asset" if root_cause == "wrong_approved_asset" else ("resolve_publishability_failures" if failures else "export_to_threadsdashboard"),
            "quarantine": quarantine,
            "metricsEligible": False,
            "metrics_eligible": False,
            "handoff_manifest": manifest,
            "contentSurface": distribution_content_surface if distribution_plan is not None else normalize_content_surface(asset.get("content_surface")),
            "content_surface": distribution_content_surface if distribution_plan is not None else normalize_content_surface(asset.get("content_surface")),
            "distributionSurface": distribution_surface,
            "distribution_surface": distribution_surface,
            "igMediaType": ig_media_type if distribution_plan is not None else self._ig_media_type_for_surface(normalize_content_surface(asset.get("content_surface")), str(asset.get("media_type") or "video")),
            "ig_media_type": ig_media_type if distribution_plan is not None else self._ig_media_type_for_surface(normalize_content_surface(asset.get("content_surface")), str(asset.get("media_type") or "video")),
            "instagramTrialReels": instagram_trial_reels,
            "instagram_trial_reels": instagram_trial_reels,
            "trialGraduationStrategy": trial_graduation_strategy if instagram_trial_reels else None,
            "trial_graduation_strategy": trial_graduation_strategy if instagram_trial_reels else None,
            "warnings": sorted(set(warnings)),
        }
