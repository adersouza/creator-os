...
def _audio_learning_snapshot(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def _measured_audio_performance(
    conn: Any,
    *,
    catalog_ids: set[str],
    creator: str,
    ...
    for catalog_id in sorted(catalog_ids):
        ...
        for scope, scoped_rows in scopes:
            for bucket in PRODUCTION_BUCKETS:
                ...
                if len(latest_by_post) >= MINIMUM_PRODUCTION_EXAMPLES:
        ...
        snapshots = [_audio_learning_snapshot(row) for row in selected_rows]
        baselines = account_reward_baselines(
            baseline_snapshots, objective=AUDIO_LEARNING_OBJECTIVE
        )
        ...
        if not isinstance(score, (int, float)):
            continue
        adjustments[catalog_id] = _ranking_adjustment(float(score))
        ...
        evidence.append(
            {
                ...
                "score": score,
                "scoreAdjustment": adjustments[catalog_id],
...
def audio_policy_for_candidates(
    ...
    *,
    ...
    measured_scores, measured_evidence = _measured_audio_performance(
        conn,
