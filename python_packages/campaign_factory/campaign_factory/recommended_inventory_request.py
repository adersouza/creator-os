...
class RecommendedInventoryRequestRepository:
    ...
    def recommended_inventory_request_plan(
        ...
        *,
        ...
        source_plan = daily_plan if isinstance(daily_plan, dict) else self._creator_os_daily_plan(creators=[creator_label])
        ...
        existing_by_parent = self.recommended_inventory_existing_by_parent(variant_inventory_plan)
        ...
        if recommendations:
            for recommendation in recommendations:
                if remaining <= 0:
                    break
                surface = self._normalize_content_surface(recommendation.get("surface") or "reel")
                ...
                batch = {
                    ...
                    "surface": surface,
                    "recommendedAction": self.recommended_inventory_action(surface=surface, story_intent=recommendation.get("storyIntent")),
                ...
                if parent_asset_id and variant_inventory_plan:
                    batch["variantInventoryBatch"] = self.recommended_inventory_variant_batch(parent_asset_id, variant_inventory_plan)
    ...
    def recommended_inventory_existing_by_parent(self, variant_inventory_plan: dict[str, Any] | None) -> dict[str, int]:
        counts: dict[str, int] = {}
