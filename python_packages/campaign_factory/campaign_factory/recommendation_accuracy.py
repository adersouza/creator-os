...
class RecommendationAccuracyRepository:
    ...
    def recommendation_accuracy(
        ...
        *,
        ...
        self.rebuild_recommendation_accuracy_observations(campaign["id"], account=account)
        ...
        if persist:
            self.persist_recommendation_accuracy_report(report, campaign["id"], account=account, window_days=window_days)
        return report
    ...
    def rebuild_recommendation_accuracy(
        ...
        *,
        ...
        return self.recommendation_accuracy(campaign_slug, account=account, window_days=window_days, persist=True)

    def recommendation_proof_summary(self, campaign_id: str) -> dict[str, Any]:
        self.rebuild_recommendation_accuracy_observations(campaign_id, account=None, commit=True)
    ...
    def rebuild_recommendation_accuracy_observations(
        ...
        *,
        ...
        observations = []
        for row in rows:
            observations.append(self.upsert_recommendation_accuracy_observation(row, commit=False))
        if commit:
            self.conn.commit()
        return observations
    ...
    def upsert_recommendation_accuracy_observation(self, row: dict[str, Any], *, commit: bool = False) -> dict[str, Any]:
        ...
        data_quality_level = str(data_quality.get("level") or output.get("dataQuality", {}).get("level") or "low")
        confidence = str(row.get("confidence") or output.get("confidence") or "low")
        confidence_bucket = self.recommendation_confidence_bucket(confidence, data_quality_level)
        ...
        selected_audio_key = selection.get("audioKey") or selection.get("catalogAudioId") or selection.get("platformAudioId")
        measured_at = row.get("measured_at") or outcome.get("measuredAt") or row.get("created_at")
        observation_id = f"recacc_{hashlib.sha256(row['id'].encode('utf-8')).hexdigest()[:12]}"
        payload = {
            ...
            "measurementVersion": row.get("measurement_version") or outcome.get("measurementVersion"),
        ...
        item_graph_id = row.get("recommendation_graph_id") or self._graph_id_for("recommendation_items", row["id"], entity_type="recommendation_item")
        self._ensure_graph_edge(item_graph_id, observation_graph_id, "recommendation_item_to_recommendation_accuracy_observation")
        for snapshot_id in self.recommendation_outcome_snapshot_ids(outcome, evidence):
            perf_graph_id = self._graph_id_for("performance_snapshots", snapshot_id, entity_type="performance_snapshot")
            self._ensure_graph_edge(perf_graph_id, observation_graph_id, "performance_snapshot_to_recommendation_accuracy_observation")
        if selection.get("id"):
            selection_graph_id = self._graph_id_for("audio_selections", selection["id"], entity_type="audio_selection", payload=selection)
            self._ensure_graph_edge(selection_graph_id, observation_graph_id, "audio_selection_to_recommendation_accuracy_observation")
        if commit:
            self.conn.commit()
    ...
    def recommendation_accuracy_observations(
        ...
        *,
        ...
        filtered = []
        for row in rows:
            observed_at = self.parse_datetime(row.get("measured_at") or row.get("updated_at"))
            if window_days is not None and observed_at and observed_at < now - timedelta(days=max(1, int(window_days))):
                continue
            if before_window_days is not None and observed_at and observed_at >= now - timedelta(days=max(1, int(before_window_days))):
                continue
    ...
    def recommendation_accuracy_report_payload(
        ...
        *,
        ...
        return {
            ...
            "campaignGraphId": self._graph_id_for("campaigns", campaign["id"], entity_type="campaign", payload={"slug": campaign["slug"]}),
            "account": account,
            "windowDays": int(window_days),
            "generatedAt": self._utc_now(),
            "recommendationTrustScore": trust,
            "trustConfidence": self.recommendation_trust_confidence(overall["measuredCount"]),
            "overall": overall,
            "calibration": calibration,
            "segments": {
                "accounts": self.accuracy_grouped(observations, "accountId"),
                "dataQuality": self.accuracy_grouped(observations, "dataQualityLevel"),
                "referencePatterns": self.accuracy_grouped(observations, "referencePatternId"),
                "selectedAudio": self.accuracy_grouped(observations, "selectedAudioKey"),
                "audioMatchStatus": self.accuracy_grouped(observations, "audioMatchStatus"),
            },
            "drift": drift,
            "observations": observations[:100],
            "warnings": [] if observations else ["no_measured_recommendations_in_window"],
        }
    ...
    def persist_recommendation_accuracy_report(
        ...
        *,
        ...
        input_hash = hashlib.sha256(json.dumps({
            ...
            "observationIds": [item.get("id") for item in report.get("observations") or []],
        ...
        self.conn.execute(
            ...
            (
                ...
                int(window_days),
                input_hash,
                json.dumps(self._sanitize_for_storage(report), ensure_ascii=False, sort_keys=True),
                now,
                now,
            ),
        )
        ...
        for observation in report.get("observations") or []:
            ...
            self._ensure_graph_edge(observation_graph_id, report_graph_id, "recommendation_accuracy_observation_to_report")
        self.conn.commit()
        return report_id
    ...
    def accuracy_segment(self, observations: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        inconclusive = sum(1 for item in observations if item.get("status") == "measured" or item.get("isInconclusive"))
        denominator = proved + disproved
        lifts = [int(item["lift"]) for item in observations if item.get("lift") is not None]
        return {
    ...
    def accuracy_grouped(self, observations: list[dict[str, Any]], key: str) -> list[dict[str, Any]]:
        ...
        grouped.sort(key=lambda item: (item["measuredCount"], item.get("accuracyRate") or -1), reverse=True)
        return grouped
    ...
    def recommendation_accuracy_drift(
        ...
        *,
        ...
        warnings = []
        for dimension in ("confidenceBucket", "dataQualityLevel", "referencePatternId", "selectedAudioKey", "audioMatchStatus"):
            recent_groups = {item["key"]: item for item in self.accuracy_grouped(recent, dimension)}
            prior_groups = {item["key"]: item for item in self.accuracy_grouped(prior, dimension)}
            for key, recent_segment in recent_groups.items():
                prior_segment = prior_groups.get(key)
                if not prior_segment:
                    continue
                if recent_segment["accuracyDenominator"] < min_sample or prior_segment["accuracyDenominator"] < min_sample:
                    continue
    ...
    def recommendation_trust_score(self, observations: list[dict[str, Any]], drift: list[dict[str, Any]]) -> int:
        ...
        if measured >= 30:
            ...
            score -= 15
        incomplete = sum(1 for item in observations if not (item.get("outcomeScore") is not None and item.get("baselineScore") is not None))
        if observations:
    ...
    def recommendation_audio_selection(self, recommendation_item_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "catalogAudioId": payload.get("audioCatalogId") or audio.get("catalogAudioId") or audio.get("catalog_audio_id"),
            "audioKey": payload.get("audioKey") or audio.get("audioKey"),
            "platformAudioId": audio.get("platformAudioId") or audio.get("platform_audio_id"),
    ...
    def recommendation_audio_match_status(self, output: dict[str, Any], selection: dict[str, Any]) -> str:
        ...
        selected_keys = {
            str(value)
            for value in (selection.get("catalogAudioId"), selection.get("audioKey"), selection.get("platformAudioId"))
            if value
        }
