...
class AutonomyPolicyRepository:
    ...
    def autonomy_level(self) -> str:
        row = self.conn.execute(
            "SELECT value_json FROM trust_settings WHERE key = 'autonomy_level'"
        ).fetchone()
    ...
    def set_autonomy_level(self, level: str) -> dict[str, Any]:
        if level not in self._autonomy_levels:
            raise ValueError(
                f"autonomy level must be one of {sorted(self._autonomy_levels)}"
            )
        now = self._utc_now()
    ...
    def autonomy_policy(self) -> dict[str, Any]:
        return {
            ...
            "levels": [
                ...
                {
                    "level": "level_2",
                    "label": "Accepted recommendations auto-execute generation/render/audit",
                    "publishesAutomatically": False,
                },
                {
                    "level": "level_3",
                    "label": "Reserved for future auto-approval",
                    "publishesAutomatically": False,
                    "reserved": True,
                },
            ],
