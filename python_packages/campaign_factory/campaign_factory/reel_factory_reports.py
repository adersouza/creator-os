...
class ReelFactoryReportRepository:
    ...
    def reel_factory_parent_throughput_proof(
        ...
        *,
        ...
        required_raw = (
            math.ceil(required / overall_rate)
            if overall_rate > 0
            else int(capacity["passRateScenarios"]["60%"])
        )
        ...
        return {
            ...
            "limitingStep": yield_report.get("largestDropoff")
            or "raw_candidate_supply",
            ...
            "publishabilityPassRate": float(
                yield_report.get("publishabilityPassRate") or 0
            ),
            "captionFamilyEligibleRate": float(
                yield_report.get("captionFamilyEligibleRate") or 0
            ),
            "audioValidRate": float(yield_report.get("audioValidRate") or 0),
            "handoffReadyRate": float(yield_report.get("handoffReadyRate") or 0),
            "operatorReviewMinutesPerParent": self.operator_review_minutes_per_parent(
                metrics
            ),
            ...
            "operationalReadiness": self.reel_factory_operational_readiness_metrics(
                yield_report
            ),
    ...
    def reel_factory_yield_analysis(
        ...
        funnel = [
            ...
            {
                "stage": "publishability_pass",
                "count": int(values["publishabilityPass"]),
            },
        ...
        largest_dropoff_row = (
            min(non_zero, key=lambda row: row["yieldRate"])
            if non_zero
            else transitions[0]
        )
        ...
        return {
            ...
            "rawCandidateToParentRate": self._ratio(
                values["parentCandidates"], values["rawCandidates"]
            ),
            "qcPassRate": self._ratio(values["qcPass"], values["parentCandidates"]),
            "publishabilityPassRate": self._ratio(
                values["publishabilityPass"], values["qcPass"]
            ),
            "captionFamilyEligibleRate": self._ratio(
                values["captionFamilyEligible"], values["parentCandidates"]
            ),
            "audioValidRate": self._ratio(
                values["audioValid"], values["parentCandidates"]
            ),
            "handoffReadyRate": self._ratio(
                values["handoffReady"], values["publishabilityPass"]
            ),
            "scheduleSafeRate": self._ratio(
                values["scheduleSafe"], values["handoffReady"]
            ),
    ...
    def reel_factory_failure_analysis(self) -> dict[str, Any]:
        ...
        yield_report = self.reel_factory_yield_analysis(metrics=metrics)
        lost_by_stage = {
            row["to"]: int(row["lost"]) for row in yield_report.get("transitions") or []
        }
        failure_specs = [
            (
                "source_acquisition",
                max(0, 53 - int(metrics["rawCandidates"])),
                "high",
                0,
            ),
            ...
            (
                "schedule_safe_inventory",
                lost_by_stage.get("schedule_safe", 0),
                "medium",
                4,
            ),
        ]
        ...
        failures = sorted(
            failures,
            key=lambda item: (-int(item["operationalImpactScore"]), item["failure"]),
        )
    ...
    def reel_factory_200_account_readiness(self) -> dict[str, Any]:
        ...
        for accounts in (25, 50, 100, 200, 500):
            production = self._inventory_production_requirements(
                accounts=accounts, posts_per_account_per_day=3
            )
            proof = self.reel_factory_parent_throughput_proof(
                required_parents_per_day=int(production["requiredParentsPerDay"])
            )
            scaling[f"{accounts}Accounts"] = {
                "accounts": accounts,
                "requiredParentsPerDay": int(production["requiredParentsPerDay"]),
                "requiredValidatedDraftsPerDay": int(
                    production["requiredValidatedDraftsPerDay"]
                ),
    ...
    def reel_factory_master_report(self) -> dict[str, Any]:
        ...
        final = {
            ...
            "largestHumanBottleneck": "operator_review"
            if proof["operatorReviewMinutesPerParent"] > 0
            else "quality_review_capacity_unproven",
    ...
    def reel_factory_parent_metrics(self) -> dict[str, int]:
        source_rows = [
            ...
            for row in self.conn.execute(
                "SELECT * FROM source_assets WHERE content_surface = 'reel' OR content_surface IS NULL"
            ).fetchall()
        ]
        render_rows = [
            dict(row)
            for row in self.conn.execute("SELECT * FROM render_jobs").fetchall()
        ]
        asset_rows = [
            dict(row)
            for row in self.conn.execute(
                "SELECT * FROM rendered_assets WHERE content_surface = 'reel' OR content_surface IS NULL"
            ).fetchall()
        ]
        ...
        caption_family_count = self._inventory_count_related(
            "caption_families", "parent_asset_id", parent_ids
        )
        concepts_count = self._inventory_count_related(
            "concepts", "parent_asset_id", parent_ids
        )
        raw_candidates = max(len(source_rows), len(render_rows), len(parent_assets))
        parent_candidates = len(parent_assets)
        qc_pass = sum(
            1 for row in parent_assets if self.reel_factory_parent_qc_pass(row)
        )
        explicit_publishability_pass = sum(
            1
            for item in readiness
            if (item.get("publishability") or {}).get("publishableCandidate")
        )
        handoff_ready = sum(
            1
            for item in readiness
            if item.get("handoffManifest") or item.get("handoffManifestV2")
        )
        schedule_safe = sum(1 for item in readiness if item.get("canHandoff"))
        publishability_pass = max(
            explicit_publishability_pass, handoff_ready, schedule_safe
        )
        qc_pass = max(qc_pass, publishability_pass)
        audio_valid = sum(
            1
            for item in readiness
            if (item.get("publishability") or {})
            .get("checks", {})
            .get("embedded_audio_verified")
            is not False
        )
        return {
            ...
            "renderedJobs": sum(
                ...
                in {"rendered", "completed", "synced"}
            ),
            "generationFailures": sum(
                1
                for row in render_rows
            ...
            "captionFamilyEligible": caption_family_count
            if caption_family_count
            else concepts_count,
            ...
            "reviewQueue": sum(
                1
                for row in parent_assets
                if str(row.get("review_state") or "").lower()
                not in {"approved", "review_ready"}
            ),
        }
    ...
    def reel_factory_parent_qc_pass(self, asset: dict[str, Any]) -> bool:
        ...
        placement_pass = not placement or str(
            placement.get("status") or ""
        ).lower() in {"passed", "pass", "ok"}
        has_caption = bool(str(asset.get("caption") or "").strip())
        has_hash = bool(asset.get("content_hash"))
        approved = str(asset.get("review_state") or "").lower() in {
            "approved",
            "review_ready",
        }
    ...
    def reel_factory_intake_metrics(self, metrics: dict[str, int]) -> dict[str, Any]:
        ...
        return {
            ...
            "metadataExtractionSuccessRate": self._ratio(
                metrics.get("sourceAssets"), raw
            ),
            "wouldWrite": False,
        }
    ...
    def reel_factory_parent_creation_metrics(
        ...
        return {
            ...
            "renderFailures": max(
                0, render_jobs - rendered - int(metrics.get("generationFailures") or 0)
            ),
    ...
    def reel_factory_quality_gate_metrics(
        ...
        return {
            ...
            "discoverabilitySafetyPassRate": yield_report.get("publishabilityPassRate")
            or 0,
    ...
    def reel_factory_human_cost(self, metrics: dict[str, int]) -> dict[str, Any]:
        ...
        failed = max(
            0,
            int(metrics.get("parentCandidates") or 0)
            - int(metrics.get("scheduleSafe") or 0),
        )
        return {
    ...
    def reel_factory_rating(self, proof: dict[str, Any]) -> float:
        ...
        score += (
            2.0
            if proof.get("confidence") == "high"
            else (1.0 if proof.get("confidence") == "medium" else 0.3)
        )
