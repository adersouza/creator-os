...
class ParentFactoryReportRepository:
    def __init__(
        ...
        *,
        ...
        self._parent_factory_discoverability_loss_analysis = (
            parent_factory_discoverability_loss_analysis
        )
        self._parent_factory_waterfall_after_discoverability = (
            parent_factory_waterfall_after_discoverability
        )
        self._post_discoverability_downstream_confidence = (
    ...
    def parent_factory_yield_waterfall(
        ...
        required_raw = (
            math.ceil(required / overall_rate) if overall_rate > 0 else required
        )
        return {
    ...
    def parent_factory_loss_analysis(
        ...
        largest = (
            max(losses, key=lambda row: int(row["lossCount"]))["stage"]
            if losses
            else ""
        )
        repairable = [
        ...
        largest_repairable = (
            max(repairable, key=lambda row: int(row["frequency"]))["reason"]
            if repairable
            else ""
        )
        largest_non_repairable = (
            max(non_repairable, key=lambda row: int(row["frequency"]))["reason"]
            if non_repairable
            else ""
        )
    ...
    def parent_factory_rejection_report(
        ...
        discoverability_loss = self._parent_factory_discoverability_loss_analysis(
            waterfall=source
        )
        specs = [
            (
                ...
                True,
                "medium",
                "stabilize render worker throughput and retry classification",
            ),
            (
                "visual_qc_failure",
                "visual_qc_pass",
                True,
                "medium",
                "tighten source selection and visual preflight",
            ),
            (
                "caption_burn_failure",
                "caption_burn_pass",
                True,
                "low",
                "repair caption sidecar/render recipe contract",
            ),
            (
                "audio_invalid",
                ...
                True,
                "medium",
                "repair embedded AAC verification before parent approval",
            ),
            (
                "discoverability_violation",
                "discoverability_safety_pass",
                True,
                "low",
                "block unsafe captions earlier in intake",
            ),
            (
                "publishability_failure",
                "publishability_pass",
                True,
                "high",
                "fix publishability blockers before parent registration",
            ),
            (
                "manifest_failure",
                "handoff_ready",
                True,
                "medium",
                "rebuild handoff manifest and distribution metadata",
            ),
            (
                "schedule_safe_failure",
                "schedule_safe",
                True,
                "medium",
                "repair distribution-plan/account-readiness linkage",
            ),
            (
                ...
                False,
                "high",
                "replace rejected creative with stronger raw candidate",
            ),
            (
                "duplicate_risk",
                "parent_accepted",
                False,
                "medium",
                "import more unique raw candidate supply",
            ),
        ]
        rows = []
            ...
            rows.append(
                {
                    ...
                    "stage": stage,
                    "frequency": frequency,
                    "percentOfFailures": round((frequency / total_failures) * 100, 1)
                    if total_failures
                    else 0,
        ...
        rows = sorted(
            rows, key=lambda row: (-int(row["frequency"]), str(row["reason"]))
        )
        return {
    ...
    def parent_factory_optimization_plan(
        ...
        required = max(1, int(required_parents_per_day or 53))
        waterfall = self.parent_factory_yield_waterfall(
            required_parents_per_day=required
        )
        rejection = self.parent_factory_rejection_report(waterfall=waterfall)
        ...
        for value in scenario_values:
            ...
            label = (
                f"{value:.1f}%"
                if value == current_yield and value % 1
                else f"{int(value)}%"
            )
            scenarios[label] = {
        ...
        top_fixes = self.parent_factory_top_fixes(
            rejection.get("rejectionReasons") or []
        )
        expected_yield = min(
            50.0,
            max(
                ...
                current_yield
                + sum(
                    float(item.get("estimatedYieldLiftPct") or 0)
                    for item in top_fixes[:3]
                ),
            ),
        )
        return {
            ...
            "currentRawCandidatesNeededFor53Parents": int(
                waterfall.get("requiredRawCandidatesPerDay") or 0
            ),
            "yieldScenarios": scenarios,
            "humanBottleneckAnalysis": self.parent_factory_human_bottleneck(
            ...
            "newRawCandidatesNeededFor53Parents": math.ceil(
                required / max(0.01, expected_yield / 100)
            ),
    ...
    def parent_factory_master_optimization_report(
        ...
        loss = self.parent_factory_loss_analysis(
            required_parents_per_day=required_parents_per_day
        )
        rejection = self.parent_factory_rejection_report(waterfall=waterfall)
        quality = self.parent_factory_quality_gate_analysis()
        optimization = self.parent_factory_optimization_plan(
            required_parents_per_day=required_parents_per_day
        )
        top_fixes = optimization.get("whatThreeFixesIncreaseYieldFastest") or []
        single_fix = (
            top_fixes[0]["fix"] if top_fixes else loss.get("highestROIImprovement", "")
        )
        acceptance = {
            "whyYieldIs8_2Pct": self.parent_factory_yield_explanation(waterfall, loss),
            "whatSingleFixImprovesYieldMost": single_fix,
            "whatThreeFixesIncreaseYieldFastest": [
                item["fix"] for item in top_fixes[:3]
            ],
            "expectedYieldAfterFixes": optimization["expectedYieldAfterFixes"],
            "newRawCandidatesNeededFor53Parents": optimization[
                "newRawCandidatesNeededFor53Parents"
            ],
            "canSupport200AccountsAfterFixes": optimization[
                "canSupport200AccountsAfterFixes"
            ],
            "wouldWrite": False,
        }
        return {
    ...
    def parent_factory_recoverable_yield(self) -> dict[str, Any]:
        ...
        discoverability_in = int(
            next(
                (
                    row.get("inputCount")
                    for row in waterfall.get("stages") or []
                    if row.get("stage") == "discoverability_safety_pass"
                ),
                0,
            )
            or 0
        )
        publishability_in = int(
            next(
                (
                    row.get("inputCount")
                    for row in waterfall.get("stages") or []
                    if row.get("stage") == "publishability_pass"
                ),
                0,
            )
            or 0
        ...
        return {
            ...
            "yieldIfDiscoverabilityFixed": round(
                self._ratio(discoverability_fixed, raw) * 100, 1
            ),
            "yieldIfPublishabilityFixed": round(
                self._ratio(publishability_fixed, raw) * 100, 1
            ),
            "yieldIfBothFixed": round(self._ratio(both_fixed, raw) * 100, 1),
            "expectedAcceptedParentsPerDay": both_fixed,
            "requiredRawCandidatesFor53Parents": math.ceil(
                53 / max(0.0001, self._ratio(both_fixed, raw))
            ),
            "measured": True,
            "wouldWrite": False,
        }
    ...
    def parent_factory_throughput_recovery_plan(self) -> dict[str, Any]:
        ...
        expected_gain = max(
            0, int(recovery.get("expectedAcceptedParentsPerDay") or 0) - current
        )
        return {
    ...
    def parent_factory_53_parent_feasibility(self) -> dict[str, Any]:
        ...
        return {
            ...
            "canReach53ParentsWithoutMoreCandidates": int(
                recovery.get("currentYieldPct") or 0
            )
            >= required_yield,
    ...
    def parent_factory_secondary_loss_analysis(self) -> dict[str, Any]:
        ...
        ranked = [
            {
                ...
                "reason": self.secondary_loss_reason(
                    row["stage"], int(row.get("lossCount") or 0)
                ),
                "wouldWrite": False,
            }
        ...
        ranked = sorted(
            ranked, key=lambda row: (-int(row["lossCount"]), str(row["stage"]))
        )
        measured_loss = (
            ranked[0] if ranked and int(ranked[0].get("lossCount") or 0) > 0 else None
        )
        next_stage = (
            measured_loss["stage"]
            if measured_loss
            else "none_measured_after_discoverability"
        )
        next_bottleneck = (
            measured_loss["reason"]
            if measured_loss
            else "downstream_sample_size_uncertainty"
        )
        ...
        return {
            ...
            "realisticYieldAfterDiscoverabilityRepair": model[
                "realisticYieldAfterDiscoverabilityRepair"
            ],
    ...
    def parent_factory_true_yield_model(self) -> dict[str, Any]:
        ...
        return {
            ...
            "requiredCandidatesFor53Parents": math.ceil(
                53 / max(0.0001, realistic_yield / 100)
            ),
    ...
    def parent_factory_realistic_53_parent_plan(self) -> dict[str, Any]:
        ...
        return {
            ...
            "requiredCandidatesFor53Parents": model["requiredCandidatesFor53Parents"],
            "highestROIAfterDiscoverability": secondary[
                "highestROIAfterDiscoverability"
            ],
            "wouldWrite": False,
        }
    ...
    def parent_factory_top_fixes(
        ...
        for row in reasons:
            ...
            lift = round(
                (float(row.get("percentOfFailures") or 0) / 100)
                * 20
                * difficulty_multiplier.get(difficulty, 0.5),
                1,
            )
            fixes.append(
                {
        ...
        return sorted(
            fixes,
            key=lambda item: (
                -float(item["estimatedYieldLiftPct"]),
                -int(item["frequency"]),
                item["fix"],
            ),
        )

    def parent_factory_human_bottleneck(
        ...
        return {
            ...
            "repairMinutesPerFailure": round(
                repair_minutes / max(1, int(rejection.get("totalFailures") or 0)), 1
            ),
            "totalOperatorMinutesPerDay": total_minutes,
            "accountsSupportedPerOperator": accounts_supported,
            "throughputLimiter": "human_review"
            if total_minutes > 480
            else "technical_gates",
            "wouldWrite": False,
        }
