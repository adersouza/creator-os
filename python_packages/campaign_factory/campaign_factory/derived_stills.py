            ...
            for selected in result["selectedFrames"]:
                ...
                receipt = _receipt(
                    ...
                    evidence={
                        ...
                        "parentRawVisualSourceKind": raw["sourceKind"],
                        "providerOutputId": raw.get("providerOutputId"),
...
def edit_still(
    ...
    *,
    ...
    if int(source_receipt["providerEditDepth"]) >= 1:
        raise PermissionError("provider edit depth cannot exceed one")
    ...
    request_core = {
        ...
        "outputSettings": {
            ...
            "grid": (
                {"rows": 2, "columns": 3, "compositeAspectRatio": "1:1"}
    ...
    try:
        ...
        if output_format == "grid_2x3":
            ...
            composite_evidence = {
                ...
                "requestedCompositeAspectRatio": "1:1",
                ...
                "actualPanelAspectRatio": "2:3",
                "panelDimensions": [width // 3, height // 2],
                "staticReelCropFeasible": width // 3 >= 720 and height // 2 >= 1280,
            }
        else:
        ...
        completed = {
            **plan,
            ...
            "providerResult": {
                ...
                "providerVersion": _mapping(provider_result.get("raw")).get(
                    "modelVersion"
                ),
                "requestId": provider_result.get("requestId"),
                "usage": provider_result.get("usage"),
...
def _receipt(
    *,
    ...
    return {
        ...
        "providerEditDepth": (
            1 if operation in {"colorway", "outfit_swap"} and provider else 0
        ),
...
def _source_approval_evidence(
    ...
    is_ai_produced = isinstance(lineage, dict) or isinstance(
        prompt.get("derivedStillSource"), dict
    )
    if tier == "canonical_identity_source" and is_ai_produced:
        raise PermissionError("AI-produced media cannot be canonical identity evidence")
...
def _resolve_raw_visual(factory: Any, parent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    candidates: list[tuple[str, Any]] = [
        ("visualInput", metadata.get("visualInput")),
        (
            "derivation.visualInput",
    ...
    if raw_rendered:
        candidates.append(
            (
                "generationLineage.generated_output",
                {
    ...
    parent_path = Path(str(parent["output_path"])).expanduser().resolve()
    parent_sha = str(parent["content_hash"])
    selected: tuple[str, dict[str, Any]] | None = None
    for source_kind, candidate in candidates:
        if not isinstance(candidate, dict):
            continue
        path = Path(str(candidate.get("path") or "")).expanduser().resolve()
        candidate_sha = str(candidate.get("sha256") or "")
        if (
            not candidate_sha
            or candidate_sha == parent_sha
            or path == parent_path
            or any(
            ...
            or not path.is_file()
            or path.is_symlink()
            or sha256_file(path) != candidate_sha
        ):
            continue
        selected = (source_kind, candidate)
        break
    if selected is None:
        raise ValueError("only captioned or untraceable final media is available")
    source_kind, selected_candidate = selected
    model_id = str(
    ...
    return {
        "path": str(Path(str(selected_candidate["path"])).expanduser().resolve()),
        "sha256": str(selected_candidate["sha256"]),
...
def _approved_tiered_image(
    ...
    if rendered is not None:
        ...
        receipt = _validate_receipt(
            _json_object(row.get("metadata_json")).get("derivedStillSource")
        )
        approval = receipt.get("approval")
        if (
            ...
            or not isinstance(approval, dict)
            or approval.get("decision") != "approved"
            or approval.get("exactOutputSha256") != row["rendered_hash"]
            or not path.is_file()
...
def _validate_receipt(receipt: Any) -> dict[str, Any]:
    if (
        ...
        or receipt.get("canonicalIdentityEligible")
        is not TIER_POLICIES[receipt["sourceTier"]]["canonicalIdentityEligible"]
        or receipt.get("generationEligible")
        is not TIER_POLICIES[receipt["sourceTier"]]["generationEligible"]
        or not isinstance(receipt.get("derivationDepth"), int)
        or receipt["derivationDepth"] < 0
        or receipt.get("providerEditDepth") not in {0, 1}
        or (
            receipt["sourceTier"] == "approved_outfit_derivative"
            and receipt["providerEditDepth"] != 1
        )
        or (
            receipt["sourceTier"] != "approved_outfit_derivative"
            and receipt["providerEditDepth"] != 0
        )
    ):
