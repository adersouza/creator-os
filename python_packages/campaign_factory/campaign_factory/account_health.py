...
class AccountHealthRepository:
    ...
    def creator_os_account_tiers(
        ...
        *,
        ...
        accounts = [
            ...
            and self._creator_label(item.get("creator") or creator_label)
            in {creator_label, "unknown"}
        ]
        ...
        health_by_account = {
            row["accountId"]: row for row in health_report.get("accounts") or []
        }
        rows = []
        for account in accounts:
            ...
            health = health_by_account.get(
                account_id
            ) or self.creator_os_account_health_decision(account, missed=[])
            ...
            rows.append(
                {
                    ...
                    "postingGuidance": health.get("postingGuidance")
                    or self.creator_os_tier_posting_guidance(tier),
    ...
    def creator_os_account_health_report(
        ...
        *,
        ...
        for account in accounts:
            ...
            rows.append(
                self.creator_os_account_health_decision(
        ...
        *,
        ...
        accounts = [
            ...
            and (
                ...
                or row.get("recommendationEligibilityState")
                in {"limited", "not_recommended"}
            )
        ]
        return {
    ...
    def creator_os_execution_account_health_blockers(
        ...
        for account in account_health.get("accounts") or []:
            if (
                ...
                or account.get("safeToSchedule") is not False
            ):
                continue
            for blocker in account.get("blockers") or []:
                ...
                if raw in {
                    ...
                    blockers.add(
                        "recommendation_not_eligible"
                        if raw == "recommendation_not_eligible"
                        else "account_manual_review_required"
                    )
                    ...
                    "recent_publish_failure_unresolved",
    ...
    def creator_os_account_tier_summary(
        ...
        summary = {
            tier: 0
            for tier in ("warming", "normal", "growth", "winner", "resting", "blocked")
        }
        for account in accounts:
    ...
    def creator_os_account_health_decision(
        ...
        warming_stage = self.creator_os_warming_stage(
            account, maturity_score=maturity_score
        )
        explicit_trust = self.creator_os_account_trust_state(account)
        recent_failure_count = max(
            int(
                self.creator_os_numeric(
                    account.get("recentPublishFailureCount")
                    or account.get("recent_publish_failure_count")
                )
            ),
        ...
        link_restricted = bool(
            account.get("linkSharingRestricted")
            or account.get("link_sharing_restricted")
        )
        ...
        if creative_risk["creativeRiskScore"] >= self._creative_risk_block_threshold:
            ...
            creative_risk["creativeRiskScore"] >= self._creative_risk_caution_threshold
        ):
        ...
        if account_tier not in {"blocked", "resting"}:
            guidance = {
                **guidance,
                **self.creator_os_cadence_overrides(
        ...
        unique_blockers = list(
            dict.fromkeys([str(item) for item in blockers if str(item)])
        )
        state = (
            "blocked"
            if unique_blockers
            else ("warming" if account_tier == "warming" else "safe")
        )
    ...
    def creator_os_account_health_summary(
        ...
        trust_summary: dict[str, int] = {
            state: 0 for state in sorted(self._account_trust_states)
        }
        rec_summary: dict[str, int] = {
            state: 0 for state in sorted(self._recommendation_eligibility_states)
        }
        ...
        for row in valid_rows:
            ...
            budget = (
                row.get("similarityBudget")
                if isinstance(row.get("similarityBudget"), dict)
                else {}
            )
        ...
        return {
            ...
            "safeToSchedule": sum(
                1 for row in valid_rows if row.get("safeToSchedule") is True
            ),
            "blockedAccounts": sum(
                1 for row in valid_rows if row.get("safeToSchedule") is False
            ),
            "restrictedAccounts": sum(
                1
                for row in valid_rows
                if (row.get("restrictionStatus") or {}).get("active")
                or row.get("linkSharingRestricted")
            ),
            "manualReviewAccounts": sum(
                1
                for row in valid_rows
                if row.get("accountTrustState") == "manual_review_required"
                or row.get("recommendationEligibilityState") == "manual_review_required"
            ),
    ...
    def creator_os_account_trust_state(self, account: dict[str, Any]) -> str:
        raw = (
            str(
                account.get("accountTrustState")
                or account.get("account_trust_state")
                or account.get("trustState")
                or ""
            )
            .strip()
            .lower()
    ...
    def creator_os_restriction_status(self, account: dict[str, Any]) -> dict[str, Any]:
        restriction = (
            account.get("restrictionStatus")
            if isinstance(account.get("restrictionStatus"), dict)
            else {}
        )
        raw_status = str(
            restriction.get("status")
            or account.get("restriction_status")
            or account.get("restrictionStatus")
            or ""
        ).strip()
        restriction_type = str(
            restriction.get("type")
            or account.get("restrictionType")
            or account.get("restriction_type")
            or ""
        ).strip()
        started_at = str(
            restriction.get("startedAt")
            or account.get("restriction_started_at")
            or account.get("restrictionStartedAt")
            or ""
        ).strip()
        ends_at = str(
            restriction.get("endsAt")
            or account.get("restriction_ends_at")
            or account.get("restrictionEndsAt")
            or ""
        ).strip()
        active = self._truthy(restriction.get("active")) or raw_status.lower() in {
            "active",
            "restricted",
            "blocked",
        }
        if ends_at:
            try:
                active = active and datetime.fromisoformat(
        ...
        return {
            ...
            or (
                "link_sharing_restricted"
                if account.get("linkSharingRestricted")
                or account.get("link_sharing_restricted")
                else ""
            ),
    ...
    def creator_os_maturity_score(self, account: dict[str, Any]) -> int:
        explicit = (
            account.get("accountMaturityScore")
            if account.get("accountMaturityScore") is not None
            else account.get("account_maturity_score")
        )
        ...
        maturity_signal_keys = {
            ...
            "manualVerificationCompleted",
        }
        if not any(key in account for key in maturity_signal_keys):
            return 60
        score = 0
        age_days = self.creator_os_numeric(
            account.get("accountAgeDays") or account.get("account_age_days")
        )
        followers = self.creator_os_numeric(
            account.get("followers")
            or account.get("followersCount")
            or account.get("followers_count")
        )
        published = self.creator_os_numeric(
            account.get("successfulPublishes")
            or account.get("successful_publishes")
            or account.get("publishedCount")
        )
        if age_days >= 30:
            score += 35
        elif age_days >= 14:
        ...
        if account.get("lastWarningAt") or account.get("last_warning_at"):
            score -= 10
        if (
            account.get("lastManualReviewAt")
            or account.get("last_manual_review_at")
            or account.get("manualVerificationCompleted")
        ):
            score += 10
    ...
    def creator_os_warming_stage(
        ...
        raw = (
            str(account.get("warmingStage") or account.get("warming_stage") or "")
            .strip()
            .lower()
            .replace("-", "_")
        )
    ...
    def creator_os_creative_risk(self, account: dict[str, Any]) -> dict[str, Any]:
        explicit = (
            account.get("creativeRisk")
            if isinstance(account.get("creativeRisk"), dict)
            else {}
        )
        score = int(
            self.creator_os_numeric(
                explicit.get("score")
                or account.get("creativeRiskScore")
                or account.get("creative_risk_score")
            )
        )
        reasons = list(
            explicit.get("reasons") or account.get("creativeRiskReasons") or []
        )
        ...
        level = (
            "blocked"
            if score >= self._creative_risk_block_threshold
            else (
                "caution" if score >= self._creative_risk_caution_threshold else "safe"
            )
        )
        return {
            "creativeRiskScore": max(0, min(100, score)),
            "creativeRiskLevel": level,
            "creativeRiskReasons": sorted(
    ...
    def creator_os_similarity_budget(self, account: dict[str, Any]) -> dict[str, Any]:
        budget = (
            account.get("similarityBudget")
            if isinstance(account.get("similarityBudget"), dict)
            else {}
        )
        blocked = bool(
            budget.get("blocked")
            or account.get("similarityBudgetExceeded")
            or account.get("similarity_budget_exceeded")
        )
        warning = bool(
            budget.get("warning")
            or account.get("similarityBudgetWarning")
            or account.get("similarity_budget_warning")
        )
        return {
            "visualSimilarityCluster": budget.get("visualSimilarityCluster")
            or account.get("visual_similarity_cluster")
            or "",
            "captionSimilarityCluster": budget.get("captionSimilarityCluster")
            or account.get("caption_similarity_cluster")
            or "",
            "windowDays": int(
                self.creator_os_numeric(
                    budget.get("windowDays") or budget.get("window_days") or 14
                )
            ),
            "maxAccounts": int(
                self.creator_os_numeric(
                    budget.get("maxAccounts") or budget.get("max_accounts") or 0
                )
            ),
            "currentAccounts": int(
                self.creator_os_numeric(
                    budget.get("currentAccounts") or budget.get("current_accounts") or 0
                )
            ),
            "blocked": blocked,
            "warning": warning,
            "reason": str(
                budget.get("reason")
                or ("similarity_budget_exceeded" if blocked else "")
            ),
        }
    ...
    def creator_os_account_tier_from_health(
        ...
        if trust_state in {"blocked", "restricted", "manual_review_required"}:
            return "blocked"
        raw = (
            str(account.get("accountTier") or account.get("tier") or "")
            .strip()
            .lower()
            .replace("-", "_")
        )
    ...
    def creator_os_cadence_overrides(
        ...
        if (
            account.get("maxPostsPerDay") is not None
            or account.get("max_posts_per_day") is not None
        ):
            max_posts = int(
                self.creator_os_numeric(
                    account.get("maxPostsPerDay") or account.get("max_posts_per_day")
                )
            )
            ...
            max_posts = 2
        if (
            account.get("minimumGapHours") is not None
            or account.get("minimum_gap_hours") is not None
        ):
            gap = int(
                self.creator_os_numeric(
                    account.get("minimumGapHours") or account.get("minimum_gap_hours")
                )
            )
        ...
        if not isinstance(allowed, list):
            allowed = (
                ["story", "reel"]
                if warming_stage in {"day_0_3", "day_4_7"} or maturity_score < 30
                else list(self._content_surfaces)
            )
        return {
            ...
            "minimumGapHours": gap,
            "allowedSurfaces": [
                self._normalize_content_surface(item) for item in allowed
            ],
        }
    ...
    def creator_os_account_over_cadence(
        ...
        max_posts = int(
            self.creator_os_numeric(
                guidance.get("maxPostsPerDay")
                or guidance.get("recommendedPostCount")
                or 0
            )
        )
        posts_today = int(
            self.creator_os_numeric(
                account.get("postsToday")
                or account.get("posts_today")
                or account.get("publishedToday")
                or account.get("published_today")
            )
        )
        scheduled_today = int(
            self.creator_os_numeric(
                account.get("scheduledToday") or account.get("scheduled_today")
            )
        )
    ...
    def creator_os_account_tier(
        ...
        if state == "high-performing":
            return "winner"
        performance = (
            account.get("performance")
            if isinstance(account.get("performance"), dict)
            else {}
        )
        metrics = (
            account.get("metrics") if isinstance(account.get("metrics"), dict) else {}
        )
        views_7d = self.creator_os_numeric(
            performance.get("views7d")
            or performance.get("views_7d")
            or metrics.get("views7d")
            or metrics.get("views_7d")
        )
        posts_7d = self.creator_os_numeric(
            performance.get("posts7d")
            or performance.get("posts_7d")
            or metrics.get("posts7d")
            or metrics.get("posts_7d")
        )
        avg_views = self.creator_os_numeric(
            performance.get("avgViews")
            or performance.get("avg_views")
            or metrics.get("avgViews")
            or metrics.get("avg_views")
        )
    ...
    def creator_os_blocked_reason(
        ...
        if missed:
            return str(
                missed[0].get("blockingReason")
                or missed[0].get("reason")
                or "overdue_dispatch_no_publish_attempt"
            )
