...
class MultiBlockerUnlockRepository:
    ...
    def multi_blocker_inventory_unlock_report(
        ...
        *,
        ...
        current = (
            ...
            else sum(1 for item in readiness if item.get("canHandoff"))
        )
        blocked = [
            self.multi_blocker_asset_row(item)
            for item in readiness
            if not item.get("canHandoff")
        ]
        combo_rows = self.multi_blocker_combo_rows(
            blocked, current_inventory=current, required_inventory=required_inventory
        )
        ...
        minimal = self.multi_blocker_minimal_fix_set(
            combo_rows, current_inventory=current, required_inventory=required_inventory
        )
        all_repairable_gain = self.multi_blocker_assets_unlocked(
            blocked, list(self.MULTI_BLOCKER_REPAIR_CLASSES)
        )
        still_needs = current + int(minimal.get("assetsUnlocked") or 0) < int(
            required_inventory
        )
        ...
        return {
            ...
            "bestSingleRepair": {
                "repairClass": (best_single.get("repairClasses") or [""])[0]
                if best_single
                else "",
                "assetsUnlocked": int(best_single.get("assetsUnlocked") or 0)
                if best_single
                else 0,
            },
            "bestTwoRepairCombo": {
                "repairClasses": best_two.get("repairClasses") or [],
                "assetsUnlocked": int(best_two.get("assetsUnlocked") or 0)
                if best_two
                else 0,
            },
            "bestThreeRepairCombo": {
                "repairClasses": best_three.get("repairClasses") or [],
                "assetsUnlocked": int(best_three.get("assetsUnlocked") or 0)
                if best_three
                else 0,
            },
    ...
    def multi_blocker_asset_row(self, readiness: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(blockers) > 1 and "handoff_manifest_missing" in blockers:
            blockers = [
                reason for reason in blockers if reason != "handoff_manifest_missing"
            ]
        repair_classes = sorted(
            {
                repair_class
                for repair_class in (
                    self.multi_blocker_repair_class(reason) for reason in blockers
                )
                if repair_class
            }
        )
        unrepairable = [
            reason for reason in blockers if not self.multi_blocker_repair_class(reason)
        ]
        return {
            ...
            "wouldBecomeScheduleSafeIfFixed": repair_classes
            if repair_classes and not unrepairable
            else [],
            "wouldWrite": False,
        }
    ...
    def multi_blocker_repair_class(self, reason: str) -> str:
        ...
        if (
            "instagram_post_caption_quality_failed" in lowered
            or "missing_instagram_post_caption" in lowered
            or "instagram_post_caption_missing" in lowered
        ):
        ...
        if (
            ...
            or "unsafe_reel_caption" in lowered
            or "dm" in lowered
            or "link" in lowered
        ):
        ...
        if (
            ...
            or "content_hash" in lowered
            or "caption_hash" in lowered
            or "caption_outcome_context" in lowered
        ):
            return "metadata_failure"
        if "quarantined" in lowered:
            return "quarantined_asset"
        if (
            "wrong_visual" in lowered
            or "visual_quality" in lowered
            or "story_quality" in lowered
            or "head_cutoff" in lowered
        ):
            return "wrong_visual"
        ...
        if (
            "audio" in lowered
            or "embedded_audio" in lowered
            or "missing_audio" in lowered
        ):
        ...
        *,
        ...
        for size in range(1, len(classes) + 1):
            for combo in combinations(classes, size):
                ...
                assets_unlocked = self.multi_blocker_assets_unlocked(
                    blocked_assets, combo_list
                )
                minutes = self.multi_blocker_estimated_minutes(
                    blocked_assets, combo_list
                )
                rows.append(
                    {
                        ...
                        "repairDifficulty": self.multi_blocker_combo_difficulty(
                            combo_list
                        ),
                        "remainingShortfall": max(
                            0,
                            int(required_inventory)
                            - int(current_inventory)
                            - assets_unlocked,
                        ),
                        "wouldPass25Gate": int(current_inventory) + assets_unlocked
                        >= int(required_inventory),
                        "wouldWrite": False,
                    }
                )
    ...
    def multi_blocker_estimated_minutes(
        ...
        for repair_class in repair_classes:
            impacted = sum(
                1
                for asset in blocked_assets
                if repair_class in set(asset.get("repairClasses") or [])
            )
            total += impacted * int(
                self.MULTI_BLOCKER_REPAIR_MINUTES.get(repair_class, 10)
            )
        return total
    ...
    def multi_blocker_combo_difficulty(self, repair_classes: list[str]) -> str:
        ...
        worst = max(
            (
                weights.get(self.MULTI_BLOCKER_REPAIR_DIFFICULTY.get(item, "high"), 3)
                for item in repair_classes
            ),
            default=1,
        )
    ...
    def multi_blocker_best_combo(
        ...
        candidates = [
            row for row in combo_rows if len(row.get("repairClasses") or []) == size
        ]
        if not candidates:
            return {}
        return sorted(
            candidates,
            key=lambda row: (
                -int(row.get("assetsUnlocked") or 0),
                int(row.get("estimatedOperatorMinutes") or 0),
                row.get("repairClasses") or [],
            ),
        )[0]
    ...
    def multi_blocker_minimal_fix_set(
        ...
        *,
        ...
        return {
            ...
            "estimatedOperatorMinutes": int(
                selected.get("estimatedOperatorMinutes") or 0
            ),
            "remainingShortfall": max(
                0,
                int(required_inventory)
                - int(current_inventory)
                - int(selected.get("assetsUnlocked") or 0),
            ),
