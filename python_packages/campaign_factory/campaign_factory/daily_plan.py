...
class DailyPlanRepository:
    def __init__(
        ...
        *,
        ...
        self._creator_os_draft_has_instagram_post_caption = creator_os_draft_has_instagram_post_caption
        ...
        self._creator_os_surface_summary_for_creator = creator_os_surface_summary_for_creator
        ...
        self._creator_os_blocked_account_breakdown = creator_os_blocked_account_breakdown
    ...
    def creator_os_daily_plan(
        ...
        *,
        ...
        accounts = [dict(item) for item in report.get("accounts") or [] if isinstance(item, dict)]
        missed_dispatches = [dict(item) for item in report.get("missedDispatches") or [] if isinstance(item, dict)]
        planner_inputs = [item for item in (time_plan, schedule_plan) if isinstance(item, dict)]
        requested_creators = [self._creator_label(item) for item in (creators or []) if str(item or "").strip()]
        if not requested_creators:
            requested_creators = sorted({self._creator_label(account.get("creator")) for account in accounts if account.get("creator")}) or ["unknown"]
        ...
        for account in accounts:
            ...
            health = health_by_account.get(account_id) or self._creator_os_account_health_decision(account, missed=missed_by_account.get(account_id, []))
            ...
            tier_guidance = health.get("postingGuidance") if isinstance(health.get("postingGuidance"), dict) else self._creator_os_tier_posting_guidance(account_tier)
            needs_post_today = bool(account.get("needsPostToday")) and state != "blocked" and not account.get("nextScheduledPost")
            surface_needs = self._creator_os_account_surface_status(account, reel_needed=needs_post_today)
            eligible_drafts = []
            cooldowns = []
            draft_exclusions = []
            for draft in draft_items:
                ...
                draft_payload = {
                    ...
                    "instagramPostCaptionPresent": self._creator_os_draft_has_instagram_post_caption(draft),
            ...
            account_rows.append({
                ...
                "lastPublishedAt": self._creator_os_post_time(account.get("lastPublishedPost")),
                "nextScheduledAt": self._creator_os_post_time(account.get("nextScheduledPost")),
                "blockedReason": blocked_reason,
                "recommendedPostCount": self._creator_os_recommended_post_count(state, needs_post_today),
                "accountHealth": health,
                "accountTrustState": health.get("accountTrustState"),
                "recommendationEligibilityState": health.get("recommendationEligibilityState"),
        ...
        creator_rows = []
        for creator in requested_creators:
            creator_accounts = [row for row in account_rows if self._creator_label(row.get("creator")) == creator]
            safe_accounts = [row for row in creator_accounts if row["state"] in {"safe", "warming", "high-performing"}]
            blocked_accounts = [row for row in creator_accounts if row["state"] == "blocked"]
            ...
            account_health_report = health_report_by_creator.get(creator) or {
                "summary": self._creator_os_account_health_summary([row.get("accountHealth") for row in creator_accounts if isinstance(row.get("accountHealth"), dict)]),
                "accounts": [row.get("accountHealth") for row in creator_accounts if isinstance(row.get("accountHealth"), dict)],
            }
            ...
            plan_inventory = self._creator_os_inventory_for_creator(creator, planner_inputs, draft_items)
            validated_available = plan_inventory["validatedDraftsAvailable"]
            variant_available = plan_inventory["variantDraftsAvailable"]
            draft_exclusions = self._creator_os_draft_exclusion_counts(creator, draft_items)
            ...
            story_inventory_available = (
                surface_summary["surfaceInventory"]["story"]["threadDashScheduleSafeDrafts"]
                or surface_summary["surfaceInventory"]["story"]["localScheduleSafe"]
            )
            story_inventory_shortfall = max(0, surface_summary["accountsNeedingStories"] - story_inventory_available)
            recommended_story_intent = self._recommended_story_intent_for_date(target_date, creator=creator)
            recommended_inventory = self._creator_os_recommended_inventory(creator=creator)
            creator_rows.append({
                "creator": creator,
                ...
                "recommendationEligibilitySummary": account_health_summary.get("recommendationEligibilitySummary") or {},
                "restrictedAccounts": int(account_health_summary.get("restrictedAccounts") or 0),
                "manualReviewAccounts": int(account_health_summary.get("manualReviewAccounts") or 0),
                "maturityDistribution": account_health_summary.get("maturityDistribution") or {},
                "creativeRiskDistribution": account_health_summary.get("creativeRiskDistribution") or {},
                "similarityBudgetWarnings": account_health_summary.get("similarityBudgetWarnings") or [],
                ...
                "accountsNeedingFeedSingles": surface_summary["accountsNeedingFeedSingles"],
                "accountsNeedingCarousels": surface_summary["accountsNeedingCarousels"],
                "surfaceInventory": surface_summary["surfaceInventory"],
                "surfaceShortfalls": surface_summary["surfaceShortfalls"],
                "surfaceScheduleReadiness": surface_summary["surfaceScheduleReadiness"],
                "storyInventoryAvailable": story_inventory_available,
                "storyInventoryShortfall": story_inventory_shortfall,
                "storyScheduleReadiness": "ready" if story_inventory_shortfall == 0 else "blocked",
                "recommendedStoryIntent": recommended_story_intent,
                "recommendedStoryStyle": self._recommended_story_style_for_intent(recommended_story_intent),
                "recommendedInventory": recommended_inventory,
                "creativeLearningReadiness": "ready" if recommended_inventory else "insufficient_data",
                ...
                "blockedAccountBreakdown": self._creator_os_blocked_account_breakdown(blocked_accounts),
        ...
        return {
            ...
            "recommendedActions": sorted({action for row in creator_rows for action in row["recommendedActions"]}),
            "inputs": {
                ...
                "variantMetricsRollupSchema": variant_metrics_rollup.get("schema") if isinstance(variant_metrics_rollup, dict) else None,
