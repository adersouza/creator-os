...
class AudioOperationsRepository:
    ...
    def attach_audio_to_distribution_plan(
        ...
        *,
        ...
        if not any(
            str(value or "").strip()
            for value in (track_id, audio_url, native_audio_id, local_winner_audio_id)
        ):
            raise ValueError(
        ...
        existing_intent = (
            ...
            if isinstance(caption_generation.get("audioIntent"), dict)
            else {}
        )
        audio_id = str(
            track_id or native_audio_id or local_winner_audio_id or audio_url or ""
        ).strip()
        selection = {
            ...
            "platform_audio_id": str(native_audio_id or track_id).strip()
            if (native_audio_id or track_id)
            else None,
            "native_audio_id": str(native_audio_id).strip()
            if native_audio_id
            else None,
            "platform_url": str(audio_url).strip() if audio_url else None,
            "native_audio_url": str(audio_url).strip() if audio_url else None,
            "local_winner_audio_id": str(local_winner_audio_id).strip()
            if local_winner_audio_id
            else None,
            "selected_at": now,
            "attached_at": now,
            "selected_by": operator,
            "attached_by": operator,
            "selected_reason": str(selected_reason).strip()
            if selected_reason
            else None,
        ...
        if audio_segment:
            ...
            if audio_segment.get("duration_seconds") is not None:
                selection["segment_duration_seconds"] = audio_segment.get(
                    "duration_seconds"
                )
        ...
        audio_intent = {
            **existing_intent,
            ...
            "gates": {
                **(
                    existing_intent.get("gates")
                    if isinstance(existing_intent.get("gates"), dict)
                    else {}
                ),
        ...
        self.conn.execute(
            "UPDATE distribution_plans SET updated_at = ? WHERE id = ?",
            (now, distribution_plan_id),
        )
        selection_id = f"audsel_{hashlib.sha256(f'{distribution_plan_id}:{audio_id}:{source_value}'.encode()).hexdigest()[:12]}"
        selection_payload = {
        ...
        self.conn.execute(
            ...
            (
                ...
                now,
                json.dumps(
                    self._sanitize_for_storage(selection_payload),
                    ensure_ascii=False,
                    sort_keys=True,
                ),
                now,
    ...
    def select_audio_for_recommendation(
        ...
        *,
        ...
        selection_id = (
            f"audsel_{hashlib.sha256(selection_key.encode('utf-8')).hexdigest()[:12]}"
        )
        selection_payload = {
        ...
        output.setdefault("evidence", {}).setdefault("audio", {})["selectionId"] = (
            selection_id
        )
        if row.get("rendered_asset_id"):
            asset_row = self.conn.execute(
                "SELECT caption_generation_json FROM rendered_assets WHERE id = ?",
                (row["rendered_asset_id"],),
            ).fetchone()
            caption_generation = (
                json_load(asset_row["caption_generation_json"], {}) if asset_row else {}
            )
            audio_intent = (
                caption_generation.get("audioIntent")
                if isinstance(caption_generation.get("audioIntent"), dict)
            ...
            caption_generation["audioIntent"] = audio_intent
            caption_generation["audioRecommendations"] = (
                output.get("audioRecommendations") or {}
            )
            self.conn.execute(
                "UPDATE rendered_assets SET caption_generation_json = ?, updated_at = ? WHERE id = ?",
                (
            ...
            (
                ...
                json.dumps(
                    self._sanitize_for_storage(
                        output.get("evidence")
                        or json_load(row.get("evidence_json"), {})
                    ),
                    ensure_ascii=False,
                    sort_keys=True,
                ),
        ...
        self.conn.commit()
        return {
            "schema": "campaign_factory.audio_selection.v1",
            "selection": self.audio_selection_payload(selection_id),
            "recommendation": self._recommendation_item(recommendation_item_id),
        }
    ...
    def verify_audio_for_post(
        ...
        *,
        ...
        meta = (
            ((raw.get("metadata") or {}).get("campaign_factory") or {})
        ...
        selection = (
            ...
            and isinstance(intent.get("operator_selection"), dict)
            else {}
        )
        audio_id = (
        ...
        recommendation_item_id = meta.get("recommendation_item_id") or meta.get(
        ...
        self.link_audio_selection_graph(
            selection_id=selection_id,
            recommendation_item_id=recommendation_item_id,
            recommendation_graph_id=self._graph_id_for(
    ...
    def audio_catalog_row(
        ...
        row = self.conn.execute(
            "SELECT * FROM audio_catalog WHERE id = ?", (audio_id,)
        ).fetchone()
    ...
    def link_audio_selection_graph(
        ...
        *,
        ...
        selection_graph_id = self._ensure_graph_node(
            "audio_selection",
            local_table="audio_selections",
            local_id=selection_id,
            payload={"selectionId": selection_id},
        )
        audio_graph_id = self._graph_id_for(
            "audio_catalog", audio_catalog_id, entity_type="audio_memory"
        )
        audio_rec_graph_id = None
        if recommendation_item_id:
            audio_rec_id = f"audiorec_{hashlib.sha256(f'{recommendation_item_id}:{audio_catalog_id}'.encode()).hexdigest()[:12]}"
            ...
            self._ensure_graph_edge(
                audio_rec_graph_id,
                selection_graph_id,
                "audio_recommendation_to_audio_selection",
            )
        self._ensure_graph_edge(
            audio_graph_id, selection_graph_id, "audio_memory_to_audio_selection"
        )
        if post_id:
            post_graph_id = self._ensure_graph_node(
                "threadsdash_post",
            ...
            self._ensure_graph_edge(
                selection_graph_id, post_graph_id, "audio_selection_to_threadsdash_post"
            )
        ...
        *,
        ...
        for row in rows:
            self._resolve_exception(
                row["id"],
                resolution=f"Native audio verified: {proof_url or 'proof recorded'}",
                operator=operator,
            )
    ...
    def record_audio_performance_snapshot(
        ...
        if not audio_id and not title:
            return None
        platform = (
            str(candidate.get("platform") or snapshot.get("platform") or "instagram")
            .strip()
            .lower()
        )
        audio_row = None
        ...
        audio_key = (
            f"{platform}:{audio_id}"
            if audio_id
            else f"{platform}:{self._slugify(str(title))}:{self._slugify(str(artist or ''))}"
        )
        ...
        rollup_id = (
            f"audperf_{hashlib.sha256(rollup_key.encode('utf-8')).hexdigest()[:12]}"
        )
        existing = self.conn.execute(
            "SELECT * FROM audio_performance_rollups WHERE id = ?", (rollup_id,)
        ).fetchone()
        ...
        if audio_catalog_id:
            ...
            perf_graph_id = self._graph_id_for(
                ...
                snapshot.get("id"),
                entity_type="performance_snapshot",
                payload=self._performance_snapshot_payload(snapshot),
            )
            self._ensure_graph_edge(
                audio_graph_id,
                perf_graph_id,
                "audio_memory_to_performance_snapshot",
                evidence={"audioKey": audio_key},
            )
        if commit:
            self.conn.commit()
        return {
    ...
    def performance_snapshot_score(self, snapshot: dict[str, Any]) -> float:
        ...
        if views <= 0:
            return 0.0
        engagement = (
            (likes * 1.0) + (comments * 2.0) + (shares * 3.0) + (saves * 4.0)
        ) / max(views, 1)
    ...
    def audio_workflow_summary(self, rendered: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for asset in rendered:
            ...
            status = (
                str(intent.get("status") or "needs_operator_selection").strip().lower()
            )
            ...
            for rec in intent.get("recommendations") or []:
                ...
                item = top.setdefault(
                    key,
                    {
                        "audio_title": rec.get("audio_title")
                        or rec.get("audioTitle")
                        or rec.get("title"),
                        "artist_name": rec.get("artist_name") or rec.get("artistName"),
                        "platform_audio_id": rec.get("platform_audio_id")
                        or rec.get("audioId"),
                        "platform_url": rec.get("platform_url")
                        or rec.get("platformUrl"),
    ...
    def dashboard_audio_intent_for_asset(self, asset: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(existing, dict):
            ...
            intent.setdefault(
                "status",
                "recommended" if recommendation_items else "needs_operator_selection",
            )
            intent.setdefault(
                "recommendations",
                recommendation_items if isinstance(recommendation_items, list) else [],
            )
    ...
    def audio_intent_is_attached(
        ...
        selected_at = isinstance(selection.get("selected_at"), str) and bool(
            selection.get("selected_at").strip()
        )
        final_key = "verified_at" if status == "verified" else "attached_at"
        final_at = isinstance(selection.get(final_key), str) and bool(
