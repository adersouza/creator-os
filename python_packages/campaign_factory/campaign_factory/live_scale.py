...
class LiveScaleRepository:
    ...
    def creator_os_live_100_account_readiness(self) -> dict[str, Any]:
        ...
        return {
            ...
            "exactShortfall": ""
            if can_run
            else self.live_100_exact_shortfall(
                accounts=accounts,
            ...
            "expectedOperatorLoad": int(
                operator.get("estimatedHumanTouchesPerDay") or 0
            ),
            "expectedExceptionRate": round(
                (
                    int(exceptions.get("exceptionCount") or 0)
                    / max(1, accounts["totalAccounts"])
                )
                * 100,
                2,
            ),
            ...
            "steps": [
                {
                    ...
                    "status": "passed"
                    if readiness["actualAccounts"] >= 100
                    else "blocked",
                    "wouldWrite": False,
                },
                {
                    ...
                    "status": "passed"
                    if readiness["availableInventory"] >= readiness["requiredInventory"]
                    else "blocked",
                    "wouldWrite": False,
                },
                {
                    ...
                    "status": "passed"
                    if readiness["availableParents"]
                    >= readiness["requiredParentsPerDay"]
                    else "blocked",
                    "wouldWrite": False,
                },
                {
                    ...
                    "status": "passed"
                    if not readiness["blockingReasons"]
                    else "blocked",
                    "wouldWrite": False,
                },
            ],
    ...
    def creator_os_live_scale_scorecard(self) -> dict[str, Any]:
        ...
        return {
            ...
            "scores": {
                "actualAccounts": self._score_fraction(
                    readiness["actualAccounts"], 100
                ),
                "inventory": self._score_fraction(
                    readiness["availableInventory"], readiness["requiredInventory"]
                ),
                "parentFactory": self._score_fraction(
                    readiness["availableParents"], readiness["requiredParentsPerDay"]
                ),
                "exceptionRate": max(
                    0.0, round(10 - readiness["expectedExceptionRate"], 1)
                ),
            },
            "canRun100AccountsToday": readiness["canRun100AccountsToday"],
            "wouldWrite": False,
    ...
    def actual_account_operational_counts(self) -> dict[str, int]:
        rows = [
            dict(row) for row in self.conn.execute("SELECT * FROM accounts").fetchall()
        ]
        blocked = 0
        warming = 0
        for row in rows:
            status = " ".join(
                ...
                for key in ("status", "notes", "platform")
            )
            if any(
                token in status
    ...
    def live_100_exact_shortfall(
        ...
        *,
        ...
        if int(accounts.get("blockedAccounts") or 0) > 0:
            parts.append(
                f"restricted_accounts:{int(accounts.get('blockedAccounts') or 0)}"
            )
        if available_inventory < required_inventory:
            parts.append(
                f"validated_draft_buffer:{required_inventory - available_inventory}"
            )
        if available_parents < required_parents:
