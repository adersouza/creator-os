...
class PerformanceSummaryRepository:
    ...
    def performance_summary(self, campaign_slug: str) -> dict[str, Any]:
        ...
        return {
            ...
            "generatedAt": utc_now(),
            "renderedAssets": self.group_performance(snapshots, "renderedAssetId", account_baselines=account_baselines),
            "sourceAssets": self.group_performance(snapshots, "sourceAssetId", account_baselines=account_baselines),
            "captionHashes": self.group_performance(snapshots, "captionHash", account_baselines=account_baselines),
            "recipes": self.group_performance(snapshots, "recipe", account_baselines=account_baselines),
            "accounts": self.group_performance(snapshots, "instagramAccountId", account_baselines=account_baselines),
            "surfaces": self.group_performance(snapshots, "contentSurface", account_baselines=account_baselines),
            "leaderboards": self.performance_leaderboards(snapshots, account_baselines=account_baselines),
    ...
    def performance_for_asset(self, asset: dict[str, Any]) -> dict[str, Any]:
        ...
        source_snapshots = [self.performance_snapshot_payload(dict(row)) for row in source_rows]
        caption_snapshots = [self.performance_snapshot_payload(dict(row)) for row in caption_rows]
        recipe_snapshots = [self.performance_snapshot_payload(dict(row)) for row in recipe_rows]
        account_baselines = account_reward_baselines(source_snapshots + caption_snapshots + recipe_snapshots)
        source = self.aggregate_performance(source_snapshots, account_baselines=account_baselines)
        caption = self.aggregate_performance(caption_snapshots, account_baselines=account_baselines)
        recipe = self.aggregate_performance(recipe_snapshots, account_baselines=account_baselines)
        score = self.performance_score(source=source, caption=caption, recipe=recipe)
        return {
            "latestPerformance": self.performance_snapshot_payload(dict(latest)) if latest else None,
    ...
    def group_performance(
        ...
        *,
        ...
        return {key: self.aggregate_performance(group, account_baselines=account_baselines) for key, group in groups.items()}
    ...
    def performance_metric_contract(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "metricNames": names if isinstance(names, list) else self.default_performance_metric_names(str(surface)),
        }
    ...
    def performance_leaderboards(
        ...
        *,
        ...
        for snapshot in snapshots:
            ...
            if hook:
                self.add_leaderboard_snapshot(boards["hooks"], hook["key"], snapshot, {"hook": hook})
            if recipe:
                self.add_leaderboard_snapshot(boards["recipes"], str(recipe), snapshot, {"recipe": str(recipe)})
            if audio:
                self.add_leaderboard_snapshot(boards["audioRecommendations"], audio["key"], snapshot, {"audio": audio})
            if reference_format:
                self.add_leaderboard_snapshot(boards["referenceFormats"], reference_format["key"], snapshot, {"referenceFormat": reference_format})
            if prompt_pattern:
                self.add_leaderboard_snapshot(boards["promptPatterns"], prompt_pattern["key"], snapshot, {"promptPattern": prompt_pattern})
            if pattern_card:
                self.add_leaderboard_snapshot(boards["patternCards"], pattern_card["key"], snapshot, {"patternCard": pattern_card})
            if model_account:
                self.add_leaderboard_snapshot(boards["modelAccounts"], model_account["key"], snapshot, {"modelAccount": model_account})
            if caption_formula:
                self.add_leaderboard_snapshot(boards["captionFormulas"], caption_formula["key"], snapshot, {"captionFormula": caption_formula})
            if variation_preset:
                self.add_leaderboard_snapshot(boards["variationPresets"], variation_preset["key"], snapshot, {"variationPreset": variation_preset})
            if hook and recipe:
                self.add_leaderboard_snapshot(boards["hookRecipeCombos"], f"{hook['key']}|{recipe}", snapshot, {"hook": hook, "recipe": str(recipe)})
            if hook and audio:
                self.add_leaderboard_snapshot(boards["hookAudioCombos"], f"{hook['key']}|{audio['key']}", snapshot, {"hook": hook, "audio": audio})
            ...
            if recipe and audio:
                self.add_leaderboard_snapshot(boards["recipeAudioCombos"], f"{recipe}|{audio['key']}", snapshot, {"recipe": str(recipe), "audio": audio})
        ...
        return {name: self.rank_leaderboard_entries(items, account_baselines=account_baselines) for name, items in boards.items()}
    ...
    def caption_outcome_manual_review(self, snapshots: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "byCaptionBank": self.caption_outcome_group(with_context, "captionBank", "captionBank"),
            "byCreatorMix": self.caption_outcome_group(with_context, "creatorMix", "creatorMix"),
            "byCreatorModel": self.caption_outcome_group(with_context, "creatorModel", "creatorModel"),
            "byFrameType": self.caption_outcome_group(with_context, "frameType", "frameType"),
            "byLengthClass": self.caption_outcome_group(with_context, "lengthClass", "lengthClass"),
            "byFormatClass": self.caption_outcome_group(with_context, "formatClass", "formatClass"),
            "byCaptionFitVersion": self.caption_outcome_group(with_context, "captionFitVersion", "captionFitVersion"),
            "byCaptionHash": self.caption_outcome_group(with_context, "captionHash", "captionHash"),
            "byCaptionPlacementLane": self.caption_outcome_group(with_context, "captionPlacementLane", "captionPlacementLane"),
            "byCaptionPlacementStatus": self.caption_outcome_group(with_context, "captionPlacementStatus", "captionPlacementStatus"),
        }
    ...
    def rank_leaderboard_entries(
        ...
        *,
        ...
        for item in items.values():
            ...
            summary = self.aggregate_performance(snapshots, account_baselines=account_baselines)
            entries.append({
    ...
    def performance_snapshot_dimensions(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        variation_preset = self.performance_variation_preset_dimension(campaign_meta, row)
        dimensions: dict[str, Any] = {}
        if recipe:
    ...
    def performance_caption_formula_dimension(self, campaign_meta: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not formula:
            return None
        return {"key": self._slugify(str(formula)), "label": str(formula), "source": "campaign_factory"}
