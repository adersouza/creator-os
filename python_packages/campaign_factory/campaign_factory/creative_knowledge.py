...
class CreativeKnowledgeRepository:
    ...
    def build_creative_knowledge_base(
        ...
        *,
        ...
        rows = self.creative_knowledge_rows(creator=creator_label, campaign_slug=campaign_slug)
        ...
        top_caption_angles = self.creative_result_group(results, "captionAngle", limit=limit)
        top_caption_versions = self.creative_result_group(results, "captionVersionId", limit=limit)
        top_audio_ids = self.creative_result_group(results, "audioId", limit=limit)
        top_surfaces = self.creative_result_group(results, "contentSurface", limit=limit)
        top_story_intents = self.creative_result_group(results, "storyIntent", limit=limit)
        top_account_tiers = self.creative_result_group(results, "accountTier", limit=limit)
        top_posting_windows = self.creative_result_group(results, "postingWindow", limit=limit)
        if insufficient:
            top_concepts = []
            top_caption_angles = []
        ...
        *,
        ...
        if not insufficient:
            for dimension, key_field in [
                ...
                for group in self.creative_result_group(results, key_field, limit=max(limit, 25)):
                    assessment = self.creative_performance_assessment(group, baseline, dimension=dimension)
        ...
        best = sorted(best, key=lambda item: (self.creative_pattern_priority(str(item.get("dimension") or "")), -float(item.get("scoreLiftPct") or 0), -float(item.get("score") or 0), str(item.get("key") or "")))[: max(1, int(limit or 10))]
        weak = sorted(weak, key=lambda item: (self.creative_pattern_priority(str(item.get("dimension") or "")), float(item.get("scoreLiftPct") or 0), float(item.get("score") or 0), str(item.get("key") or "")))[: max(1, int(limit or 10))]
        ...
        return {
            ...
            "recommendedMoreOf": [] if insufficient else self.creative_more_recommendations(best, confidence, limit=limit),
            "recommendedLessOf": [] if insufficient else self.creative_less_recommendations(weak, confidence, limit=limit),
            "wouldWrite": False,
        }

    def creative_performance_baseline(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        count = len(results)
        totals = {
    ...
    def creative_performance_assessment(self, group: dict[str, Any], baseline: dict[str, Any], *, dimension: str) -> dict[str, Any]:
    ...
    def creative_more_recommendations(self, best: list[dict[str, Any]], confidence: str, *, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for item in best:
            ...
            payload["explainability"] = self.recommendation_explainability(payload, item=item, confidence=confidence)
    ...
    def creative_less_recommendations(self, weak: list[dict[str, Any]], confidence: str, *, limit: int = 10) -> list[dict[str, Any]]:
        recommendations: list[dict[str, Any]] = []
        for item in weak:
            lineage = item.get("lineage") if isinstance(item.get("lineage"), dict) else {}
            payload = {
    ...
    def recommendation_explainability(
        ...
        *,
        ...
        return {
            ...
            "confidence": self.confidence_score(confidence if confidence is not None else recommendation.get("confidence")),
    ...
    def learning_confidence_classification(self, results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def creative_fatigue_signals(self, results: list[dict[str, Any]], *, field: str, fatigue_type: str) -> list[dict[str, Any]]:
        grouped: dict[str, list[dict[str, Any]]] = {}
        for item in results:
    ...
    def metric_decline_pct(self, early: list[dict[str, Any]], recent: list[dict[str, Any]], metric: str) -> float:
    ...
    def engagement_decline_pct(self, early: list[dict[str, Any]], recent: list[dict[str, Any]]) -> float:
    ...
    def creative_surface_rows(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        grouped: dict[str, list[dict[str, Any]]] = {}
        for item in items:
            surface = self._normalize_content_surface(item.get("contentSurface") or "reel")
    ...
    def creative_knowledge_results_for_report(self, kb: dict[str, Any], creator: str, campaign_slug: str | None) -> list[dict[str, Any]]:
        if kb.get("insufficientData"):
            return []
        return [self.creative_knowledge_result(row) for row in self.creative_knowledge_rows(creator=kb["creator"] or self._creator_label(creator), campaign_slug=campaign_slug)]

    def creative_knowledge_rows(self, *, creator: str, campaign_slug: str | None = None) -> list[dict[str, Any]]:
    ...
    def creative_result_group(self, results: list[dict[str, Any]], key_field: str, *, limit: int = 10) -> list[dict[str, Any]]:
    ...
    def creative_result_lineage(self, items: list[dict[str, Any]]) -> dict[str, list[str]]:
        fields = {
            "campaigns": "campaign",
