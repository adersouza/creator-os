...
class InventoryRecoveryRepository:
    ...
    def inventory_recovery_report(
        ...
        *,
        ...
        required = (
            ...
            else max(1, int(account_target or 25))
            * max(0, int(posts_per_account_per_day or 3))
            * max(1, int(buffer_days or 3))
        )
        blocked_rows = [
            self.inventory_recovery_blocked_asset(item) for item in blocked_assets
        ]
        ...
        top_recovered = self.inventory_recovery_assets_unlocked(
            blocked_rows, [str(top.get("repairClass") or "")]
        )
        top3_recovered = self.inventory_recovery_assets_unlocked(
            blocked_rows, top3_classes
        )
        ...
        return {
            ...
            "inventoryGateImpact": {
                ...
                "wouldPass25AccountGate": current_inventory + top3_recovered
                >= required,
            },
            "successCriteria": {
    ...
    def inventory_recovery_blocked_asset(
        ...
        classes = sorted(
            {self.inventory_recovery_class_for_blocker(reason) for reason in blockers}
        )
        return {
    ...
    def inventory_recovery_class_rows(
        ...
        for repair_class in self.INVENTORY_RECOVERY_CLASSES:
            ...
            unlocked = self.inventory_recovery_assets_unlocked(
                blocked_assets, [repair_class]
            )
            ...
            rows.append(
                {
                    ...
                    "percentOfTotalBlockedInventory": round(
                        (len(assets_with_class) / total) * 100, 1
                    )
                    if total
                    else 0,
                    "estimatedOperatorMinutes": len(assets_with_class)
                    * minutes_per_asset,
                    "estimatedMinutesPerAsset": minutes_per_asset,
                    "repairDifficulty": self.INVENTORY_RECOVERY_DIFFICULTY[
                        repair_class
                    ],
                    "wouldWrite": False,
                }
    ...
    def inventory_recovery_assets_unlocked(
        ...
        return sum(
            1
            for asset in blocked_assets
            if set(asset.get("repairClasses") or [])
            and set(asset.get("repairClasses") or []).issubset(repaired)
        )
    ...
    def inventory_recovery_priorities(
        ...
        for row in rows:
            ...
            score = round(
                ((gain * 100) + blocked)
                / (minutes * difficulty_weight.get(difficulty, 3)),
                4,
            )
            ranked.append(
                {
