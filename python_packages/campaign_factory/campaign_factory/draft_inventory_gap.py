...
class DraftInventoryGapRepository:
    def __init__(
        ...
        *,
        ...
        self._creator_os_local_schedule_safe_assets = creator_os_local_schedule_safe_assets
    ...
    def creator_os_draft_inventory_gap(
        ...
        *,
        ...
        exported_asset_ids = {
            str(item.get("renderedAssetId") or item.get("campaignFactoryAssetId") or item.get("campaign_factory_asset_id") or "")
            for item in draft_items
            if str(item.get("renderedAssetId") or item.get("campaignFactoryAssetId") or item.get("campaign_factory_asset_id") or "").strip()
        }
        ...
        for item in draft_items:
            if self._creator_label(item.get("creator")) not in {creator_label, "unknown"}:
                continue
            asset_id = str(item.get("renderedAssetId") or item.get("campaignFactoryAssetId") or item.get("campaign_factory_asset_id") or "").strip()
            ...
            if item.get("platformDraftValidated") is not True:
                ...
                blocked_reasons["platform_draft_not_validated"] = blocked_reasons.get("platform_draft_not_validated", 0) + 1
                continue
            if item not in schedule_safe_drafts:
                normalized = self._creator_os_gap_blocking_reason(reason, blockers, item)
        ...
        if not_exported:
            ...
            next_action = "export_validated_drafts" if thread_dash_count < local_count else "fix_validation"
