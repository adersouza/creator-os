...
class CreatorOSDraftRepository:
    ...
    def creator_os_target_date(self, *, date: str | None = None, generated_at: str | None = None) -> str:
        ...
        if raw:
            try:
                ...
                return raw[:10]
        return datetime.fromisoformat(self._utc_now().replace("Z", "+00:00")).date().isoformat()
    ...
    def creator_os_surface_summary_for_creator(
        ...
        *,
        ...
        schedule_safe_drafts = self.creator_os_schedule_safe_drafts(creator, draft_items)
    ...
    def recommended_story_intent_for_date(self, target_date: str, *, creator: str | None = None) -> str:
        if creator:
            try:
                intent_counts = self._story_intent_report(creator=creator).get("intentCounts") or {}
            except Exception:
                intent_counts = {}
            if intent_counts:
    ...
    def creator_os_execution_draft_blockers(self, creator: str, draft_items: list[dict[str, Any]]) -> list[str]:
        ...
        for item in draft_items:
            ...
            elif audio_proof_status in {"missing", "unverified", "pending", "failed"} or (
                audio_status in {"selected", "recommended", "missing"} and audio_proof_status not in {"attached", "ok", "pass", "passed", "valid", "verified"}
            ):
