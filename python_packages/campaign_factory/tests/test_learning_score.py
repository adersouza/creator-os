...
def test_aggregate_uses_latest_snapshot_per_post_for_1h_to_24h_replacement():
    summary = aggregate_performance([
        _snapshot(post_id="post_1", snapshot_at="2026-06-01T01:00:00+00:00", views=100, likes=3),
        _snapshot(post_id="post_1", snapshot_at="2026-06-02T00:00:00+00:00", views=1000, likes=50),
    ])
...
def test_account_normalization_beats_raw_big_account_volume():
    snapshots = [
        ...
        _snapshot(post_id="small_winner", account="small", views=900, likes=120, shares=20, saves=20),
        _snapshot(post_id="big_baseline", account="big", views=50000, likes=2500, shares=100, saves=100),
        _snapshot(post_id="big_raw", account="big", views=70000, likes=2600, shares=100, saves=100),
    ]
...
def test_cold_start_strong_snapshot_scores_above_neutral_without_prior_baseline():
    summary = aggregate_performance([
        _snapshot(post_id="winner", views=10000, likes=1000, comments=100, shares=60, saves=60),
    ])
...
def test_recency_decay_downweights_old_winners():
    ...
    fresh = aggregate_performance([
        _snapshot(post_id="fresh", snapshot_at="2026-06-01T00:00:00+00:00", views=1000, likes=80, shares=10, saves=10),
    ], reference_now=now)
    old = aggregate_performance([
        _snapshot(post_id="old", snapshot_at="2026-04-01T00:00:00+00:00", views=1000, likes=80, shares=10, saves=10),
    ], reference_now=now)

    assert fresh["learning"]["effectiveSampleSize"] > old["learning"]["effectiveSampleSize"]
