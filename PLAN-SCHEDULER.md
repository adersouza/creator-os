...
- Audit follow-up — dead code/deps (commits `ffce2f76`…`cdcce152`): removed stray root `test_repurposer.py` runner, unused root `httpx` dep and `reel_factory` `einops` extra, stale `caption_banks` swipe-decision/intake-archive snapshots (recoverable via git history); repo-wide ruff I001 import-sort autofix. Deferred item RESOLVED (no action): root `pipeline_contracts/` is an intentional generated compatibility mirror, not a duplicate — `__init__.py` is a shim loading the canonical `packages/pipeline_contracts` package; `schemas/` + `typescript/` are written by `scripts/sync-pipeline-contracts.mjs` and verified by `scripts/check-pipeline-contracts-sync.mjs` (checked: currently in sync) and `scripts/doctor.py` `GENERATED_CONTRACT_PATHS`. Edit only canonical, then `pnpm sync:contracts`. Do NOT delete the root mirror. [SUPERSEDED 2026-07-10, commit `d5bad62a`: F3 contracts consolidation deleted the root mirror, outer `packages/pipeline_contracts/schemas`, and both sync scripts; in-package `packages/pipeline_contracts/pipeline_contracts/schemas` is now the single canonical source, TS generated directly from it. Root `pipeline_contracts/__init__.py` shim retained.]
- Audit B12 — dead-contract decisions (no code change): `front_generation_plan.v1` KEEP — it has an active production producer (`campaign_factory/front_generation_stage.py`, invoked from `cli.py`, plan validated via `validate_front_generation_plan` before write); its consumer is intentionally human (operator reviews the plan and accepted still — see `ACCEPTED_STILL_PLACEHOLDER`), so "no machine consumer" is by design, not dead. `repurposing_plan.v1` DEPRECATE (spec-only) — no production producer or consumer; exercised only by `tests/integration/test_cross_pipeline_acceptance.py` and schema-example validation. Decision: keep the schema as a reserved spec, add no new dependencies on it, and remove it in the next contracts-sync major pass if still unused (removal touches canonical schemas, root mirror, generated TS, and the ThreadsDashboard mirror, so it is batched rather than done piecemeal here).
- Deletion audit #6 — `local_api_auth.py` triplication DEFERRED (decision recorded): three identical ~70-LOC copies (campaign_factory, reel_factory, reference_factory) are actively imported by all three FastAPI apps. Consolidation is intentionally deferred until there is a packaged shared owner (a real installable package, not a path-import) plus app-level auth tests covering all three apps; until then the copies stay, and any auth change must be applied to all three files in the same commit.
