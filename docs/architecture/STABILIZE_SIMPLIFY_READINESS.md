...
|---|---|---|
| Source | PASS on `main`; code-bearing baseline `bd96be81` | `make verify` passed with supported Node 24: ContentForge 129, contracts 24 TypeScript + 54 Python, Core 16, Campaign 692, Reference 111, Reel 436, integration 76, and offline prompt regressions 3/3; contracts, lint, formatting, typing, architecture, and artifact gates passed |
| CI | PASS on code PR #444 and status-doc PR #445 | Required Python/JavaScript when selected, architecture, contracts, hygiene, and secret scan passed; CodeQL, Trivy, and SBOM were path-filtered skips rather than failures |
| Runtime | PASS | the clean detached runtime is promoted in lockstep with `main`; `creator-os status --live-read-only` is the authority for the exact current SHA, all eight checks passed, and the promoted code-bearing runtime completed a normal metrics/learning cycle at `2026-07-15T22:45:29Z` |
