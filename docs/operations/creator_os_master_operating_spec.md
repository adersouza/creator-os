...
| Notify Publish and post reconciliation | PROVEN | Trial post `3a69a80f-dda1-4a04-95d6-f60271d4e2aa` reconciles exactly once to Instagram media `18094620473086400` and permalink `https://www.instagram.com/reel/DaqdKAqxUW3/`; read-only closed-loop proof passes against `bennett_s33`. The operator opened and completed the handoff before its original 7:30 PM schedule after the account was rerouted, so the retained batch QStash receipt is scheduling evidence, not a claim that the notification delivered the early handoff | Preserve the same invariant for future posts and separately prove scheduled notification delivery on a post that actually waits for its due time |
