...
| Kling candidate approval | PENDING | The two real static candidates both have current safe `approved_candidate` audits, but remain `review_ready` with no durable `approval_decisions` rows; the ranking gate correctly refuses to select either one | Operator must explicitly approve both static candidates before the free rank-one receipt can be created |
