...
if command -v gitleaks >/dev/null 2>&1; then
  scanner_ran=1
  gitleaks_args=(--redact --no-banner)
  run_gitleaks() {
    if command -v timeout >/dev/null 2>&1; then
      timeout "${GITLEAKS_TIMEOUT_SECONDS:-120}" gitleaks "$@"
    elif command -v gtimeout >/dev/null 2>&1; then
      gtimeout "${GITLEAKS_TIMEOUT_SECONDS:-120}" gitleaks "$@"
    else
      gitleaks "$@"
    fi
  }
  ...
  run_gitleaks detect --source . "${gitleaks_args[@]}" --log-opts HEAD || failures=1
  if ! git diff --quiet --no-ext-diff -- .; then
    git diff --no-ext-diff --binary -- . | run_gitleaks detect --pipe "${gitleaks_args[@]}" || failures=1
  fi
  while IFS= read -r -d '' untracked_file; do
    if [[ -f "$untracked_file" ]] && [[ "$(wc -c < "$untracked_file")" -le 5242880 ]]; then
      run_gitleaks detect --pipe "${gitleaks_args[@]}" < "$untracked_file" || failures=1
    fi
