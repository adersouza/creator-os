...
import type { InstagramAccount, ThreadAccount, ThreadPost } from "../../types.js";
import {
	getUserIdAsync,
	logger,
	neqOrNull,
...
/**
 * Store OAuth state in Redis via server endpoint (best-effort).
 * Fail-open: if the call fails, the OAuth flow continues anyway since
 * callbacks also fail-open on Redis verification.
 */
async function storeOAuthStateOnServer(state: string): Promise<void> {
	try {
		...
		// Best-effort — don't block the OAuth redirect
	}
}
