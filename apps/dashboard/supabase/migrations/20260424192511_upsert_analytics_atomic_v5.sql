-- v5: thread threads_views_by_source through upsert_account_analytics_atomic.
--
-- Companion to 20260425000000 which added the column. The RPC replaces v4
-- (function signature unchanged — still (p_analytics JSONB, p_metrics_history
-- JSONB)) so existing callers pick up the new field transparently. Analytics
-- rows that don't include threads_views_by_source behave exactly as before
-- (NULL written on insert, preserved on update via COALESCE).
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE((p_analytics->>'ig_follower_reach')::integer, 0),
    COALESCE((p_analytics->>'ig_non_follower_reach')::integer, 0),
  ...
  ON CONFLICT (account_id, date) DO UPDATE SET
    ...
    ig_follower_reach = EXCLUDED.ig_follower_reach,
    ig_non_follower_reach = EXCLUDED.ig_non_follower_reach,
