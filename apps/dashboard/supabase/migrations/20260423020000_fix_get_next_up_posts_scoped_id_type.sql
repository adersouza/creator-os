-- Second fix in the 20260418040000 family: get_next_up_posts also declared
-- p_scoped_account_id as uuid and then compared it against posts.account_id
-- (text, threads), raising 42883 for any scoped-Threads dashboard filter.
-- First hot-fix (20260423010000) corrected v_user for all 4 RPCs; this
-- second one corrects the parameter type on this specific RPC.
--
-- Signature change (uuid -> text) requires DROP + CREATE; CREATE OR REPLACE
-- can't modify parameter types. Cast instagram_account_id::text on the IG
-- comparison side so both platforms work through the same text parameter.
