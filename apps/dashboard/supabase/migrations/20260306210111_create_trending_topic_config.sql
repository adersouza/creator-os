...
CREATE POLICY "Users can manage their own trending configs" ON trending_topic_config
  FOR ALL USING (auth.uid()::text = user_id) WITH CHECK (auth.uid()::text = user_id);
CREATE INDEX IF NOT EXISTS idx_trending_topic_config_scan ON trending_topic_config (enabled, last_scan_at) WHERE enabled = true;
