-- New Reel-specific metric columns on posts table
ALTER TABLE posts ADD COLUMN IF NOT EXISTS ig_reels_avg_watch_time DOUBLE PRECISION DEFAULT 0;
ALTER TABLE posts ADD COLUMN IF NOT EXISTS ig_crossposted_views INTEGER DEFAULT 0;
ALTER TABLE posts ADD COLUMN IF NOT EXISTS ig_facebook_views INTEGER DEFAULT 0;

-- Drop old function signature before creating extended version
DROP FUNCTION IF EXISTS update_ig_post_metrics_if_newer(
  TEXT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, DOUBLE PRECISION, BIGINT
);

-- Extend the monotonic metric update RPC with new params
...
GRANT EXECUTE ON FUNCTION update_ig_post_metrics_if_newer(TEXT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, BIGINT, DOUBLE PRECISION, BIGINT, DOUBLE PRECISION, BIGINT, BIGINT) TO service_role;
