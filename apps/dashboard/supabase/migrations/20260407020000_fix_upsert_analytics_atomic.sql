-- Fix upsert_account_analytics_atomic: remove non-existent user_id/platform columns
-- from account_analytics INSERT; account_analytics has no user_id or platform column.
...
AS $$
BEGIN
  -- Upsert account_analytics (no user_id or platform column on this table)
  INSERT INTO account_analytics (
    account_id, date,
    followers_count, follower_count, following_count, follower_growth,
  ...
  VALUES (
    ...
    COALESCE((p_analytics->>'follower_count')::bigint, (p_analytics->>'followers_count')::bigint, 0),
  ...
  ON CONFLICT (account_id, date) DO UPDATE SET
    ...
    follower_count = COALESCE(EXCLUDED.follower_count, account_analytics.follower_count),
