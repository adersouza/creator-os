-- Hot-fix for 20260418040000_juno33_dashboard_rpcs.sql: four RPCs declared
-- v_user as uuid and compared against text user_id columns, raising
-- 42883 "operator does not exist: text = uuid" on every call. Same pattern
-- as the 2026-04-20 calendar RPC hot-fix; caught on 2026-04-23 by
-- `supabase db lint` after the metrics audit work.
--
-- Dashboard hooks affected in the frontend:
--   useSystemStatus → get_system_status
--   useFleetHealth  → get_fleet_health
--   useNextUpPosts  → get_next_up_posts
--   useActivityEvents → get_activity_events
--
-- Only change is v_user uuid -> text (with auth.uid()::text cast). Function
-- signatures + bodies are otherwise identical to the original migration.
...
CREATE OR REPLACE FUNCTION public.get_next_up_posts(
  ...
  p_scoped_account_id uuid DEFAULT NULL,
  ...
  WITH upcoming AS (
    ...
    WHERE p.user_id = v_user
      ...
      AND (p_scoped_account_id IS NULL
           ...
           OR (p_scoped_platform = 'instagram' AND p.instagram_account_id = p_scoped_account_id))
...
GRANT EXECUTE ON FUNCTION public.get_next_up_posts(text, int, int, uuid, text) TO authenticated;
