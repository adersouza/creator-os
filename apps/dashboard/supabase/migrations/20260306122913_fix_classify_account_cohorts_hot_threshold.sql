...
-- Full function replacement — fixes hot threshold to include followers >= 500
...
BEGIN
  ...
  WHERE threads_access_token_encrypted IS NOT NULL AND sync_cohort != 'hot'
  ...
  WHERE threads_access_token_encrypted IS NOT NULL AND sync_cohort NOT IN ('hot', 'warm')
  ...
  WHERE threads_access_token_encrypted IS NOT NULL AND sync_cohort NOT IN ('hot', 'warm', 'cold')
  ...
  WHERE instagram_access_token_encrypted IS NOT NULL AND sync_cohort != 'hot'
  ...
  WHERE instagram_access_token_encrypted IS NOT NULL AND sync_cohort NOT IN ('hot', 'warm')
  ...
  WHERE instagram_access_token_encrypted IS NOT NULL AND sync_cohort NOT IN ('hot', 'warm', 'cold')
  AND (sync_cohort IS DISTINCT FROM 'dormant' OR cohort_updated_at IS NULL OR cohort_updated_at < NOW() - INTERVAL '6 hours');
  RETURN v_count;
END;
$$ LANGUAGE plpgsql;
