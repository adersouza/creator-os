 ...
 * Handles replies, mentions, publish, and delete events from the Threads platform.
 */
...
import { getAccountToken, parseWebhookTimestamp } from "./shared.js";
 ...
 */
async function refreshThreadsPostMetricsFromWebhook(
...
async function processThreadsReplyEvent(
	...
	if (postId && text) {
		...
		trackCommentSentiment(postId, text as string).catch(() => {});
	}
