 ...
 *
 * Handles all Instagram-platform webhook events: comments, mentions, story_insights,
 * messaging, follow, messaging_seen, message_reactions, message_edit.
 */
 ...
 */
export async function refreshIgPostMetricsFromWebhook(
	...
	// Look up post row to get media type for Reel-specific metrics.
	// `ig_media_type` preserves media_product_type=REELS; generic media_type is often VIDEO.
	const { data: post } = (await supabase
		.from("posts")
		.select("id, media_type, ig_media_type")
