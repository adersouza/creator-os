...
export type InstagramAudioReplacementMode = "auto" | "search" | "default";
...
export interface InstagramAudioListResponse {
	success: true;
	audio: InstagramAudioAsset[];
	paging?: unknown;
}

export interface InstagramAudioMetadataResponse {
	success: true;
	audio?: InstagramAudioAsset | undefined;
}

export interface InstagramMediaAudioTypeResponse {
	success: true;
	mediaAudioType?: "MUSIC" | "ORIGINAL_SOUND" | string | null | undefined;
}
...
export const instagramService = {
	...
	getPublishingLimit(accountId: string) {
		return postRequest<InstagramPublishingLimitResponse>("/instagram/insights?action=publishing-limit", {
			accountId,
		});
	},
	...
	searchAudio(
		accountId: string,
		params?: {
			audioType?: InstagramAudioType | undefined;
			query?: string | undefined;
			limit?: number | undefined;
		},
	) {
		return getRequest<InstagramAudioListResponse>("/instagram/audio", {
			action: "search",
			accountId,
			audioType: params?.audioType ?? "music",
			...(params?.query ? { query: params.query } : {}),
			...(params?.limit ? { limit: String(params.limit) } : {}),
		});
	},

	getAudioMetadata(accountId: string, igAudioId: string) {
		return getRequest<InstagramAudioMetadataResponse>("/instagram/audio", {
			action: "metadata",
			accountId,
			igAudioId,
		});
	...
	discoverAudioReplacements(
		accountId: string,
		params: {
			igMediaId: string;
			audioReplacementMode?: InstagramAudioReplacementMode | undefined;
			query?: string | undefined;
			limit?: number | undefined;
		},
	) {
		return getRequest<InstagramAudioListResponse>("/instagram/audio", {
			...
			audioReplacementMode: params.audioReplacementMode ?? "search",
			...(params.query ? { query: params.query } : {}),
			...(params.limit ? { limit: String(params.limit) } : {}),
		});
	},

	getMediaAudioType(accountId: string, igMediaId: string) {
		return getRequest<InstagramMediaAudioTypeResponse>("/instagram/audio", {
			action: "media-audio-type",
			accountId,
			igMediaId,
		});
	},
