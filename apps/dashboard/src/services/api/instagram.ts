...
import { mapInstagramAccountRow } from "@/lib/mappers";
...
export async function getInstagramInsights(
	...
	const response = await fetch(apiUrl("/api/instagram/insights?action=account-insights"),
		{
			method: "POST",
			headers: {
...
export async function getInstagramPostInsights(
	...
	const response = await fetch(apiUrl("/api/instagram/insights?action=post-insights"), {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
			Authorization: `Bearer ${session?.access_token}`,
