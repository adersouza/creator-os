...
const sendInviteEmailResponseSchema = z.object({
...
export const createWorkspace = async (name: string): Promise<Workspace> => {
	...
	return {
		...
		subscriptionTier: (workspace.tier ||
			"free") as import("@/types/team").SubscriptionTier,
	};
};
...
export const deleteWorkspace = async (workspaceId: string): Promise<void> => {
	...
	// Remove workspace-scoped operational data before removing membership rows.
	await Promise.all([
		supabase.from("watchdog_alerts").delete().eq("workspace_id", workspaceId),
		supabase.from("unified_links").delete().eq("workspace_id", workspaceId),
		supabase.from("auto_post_queue").delete().eq("workspace_id", workspaceId),
		supabase
		...
		supabase
			...
			.delete()
			.eq("workspace_id", workspaceId),
		supabase.from("auto_post_config").delete().eq("workspace_id", workspaceId),
		supabase
...
export const createInvite = async (
	...
	if (email) {
		try {
			const emailJson = await apiFetch(
				"/api/team?action=send-invite-email",
				sendInviteEmailResponseSchema,
				{
					method: "POST",
					json: {
...
export const getInviteDetails = async (
	...
	try {
		const data = await apiFetch(
			`/api/team?action=invite-details&code=${encodeURIComponent(inviteCode)}`,
