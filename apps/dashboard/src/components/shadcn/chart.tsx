...
type ChartTooltipContentProps = React.ComponentProps<"div"> & {
	...
	labelFormatter?:
		| ((value: React.ReactNode, payload: ChartPayloadItem[]) => React.ReactNode)
		| undefined;
	formatter?:
		| ((
	...
	(
		...
		return (
			<div
				ref={ref}
				className={cn(
					"grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/70 bg-popover px-2.5 py-2 text-xs text-popover-foreground shadow-xl",
					className,
				)}
			>
				...
				<div className="grid gap-1.5">
					{payload.map((item, index) => {
						...
						const indicatorColor =
							color || String(item.payload?.fill ?? item.fill ?? item.color ?? "");
