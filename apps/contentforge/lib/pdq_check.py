...
try:
    ...
    print(json.dumps({"error": "pdqhash not installed. Run: pip3 install pdqhash Pillow"}))
    sys.exit(1)
    ...
    for fname in files:
        ...
        if fext in ('.mp4', '.mov', '.webm'):
            ...
            if not extract_frame(fpath, frame_path):
                results.append({"name": fname, "distance": None, "safe": None, "error": "frame extraction failed"})
                continue
        ...
        if err:
            results.append({"name": fname, "distance": None, "safe": None, "error": err})
            continue
