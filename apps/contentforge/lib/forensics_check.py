...
def analyze_video(video_path):
    ...
    try:
        ...
        if len(frames) > 30:
            ...
            i_positions = [i for i, f in enumerate(frames) if f.get("key_frame") == 1 or f.get("pict_type") == "I"]
            gop_sizes = [i_positions[i+1] - i_positions[i] for i in range(len(i_positions) - 1)]
            ...
            if len(sizes) > 64:
                ...
                if half > 3:
                    ...
                    peak_strength = float(spectrum[peak_idx] / np.mean(spectrum[2:half]))
            ...
            results["checks"].append(advisory_check({
                ...
                "pass": True,  # Informational
                "detail": "I:" + str(type_counts.get("I", 0)) +
        ...
        if proc.stdout:
            ...
            for track in mi_data.get("media", {}).get("track", []):
                if track.get("@type") == "Video":
                    ...
                    all_encoder_info = (encoder + " " + encoder_settings + " " + writing_lib).lower()
    ...
    try:
        ...
        with open(video_path, "rb") as f:
            ...
            data = f.read(50 * 1024)
            x264_uuid = bytes([0xdc, 0x45, 0xe9, 0xbd, 0xe6, 0xd9, 0x48, 0xb7,
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: forensics_check.py <output_dir> [max_files]"}))
        sys.exit(1)
    ...
    summary["passRate"] = round((summary["passed"] / summary["total"]) * 100) if summary["total"] > 0 else 0
