...
try:
    ...
    print(json.dumps({"error": "jpegio not installed. Run: pip3 install jpegio"}))
    sys.exit(1)
...
def randomize_jpeg_qt(input_path, output_path=None, quality_range=(85, 95), noise_pct=0.05):
    """
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: jpeg_randomize_qt.py <file_or_directory> [quality_min] [quality_max]"}))
        sys.exit(1)
    ...
    if os.path.isdir(target):
        files = sorted([
            ...
            if os.path.splitext(f)[1].lower() in ('.jpg', '.jpeg') and not f.startswith('.')
        ])
