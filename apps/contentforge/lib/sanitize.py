        ...
        while True:
            ...
            while ext_end < len(buf) and ext_end < idx + 64:
                ...
                if b in (0x2e, 0x2d, 0x20) or (0x30 <= b <= 0x39):
                    ...
                    break
            results["patches"].append(f"String '{forensic_str.decode()}' nullified in JPEG")
            patched = True
            search_start = ext_end
...
def sanitize_file(filepath):
    ...
    if tool_count > 0:
        results["patches"].append(f"©too encoder atom: {tool_count} instance(s) nullified")

    # 3. Null out forensic strings in container
    data, string_patches = nullify_container_strings(data)
    if string_patches:
        for p in string_patches:
            results["patches"].append(f"String '{p['string']}' at offset {p['offset']} ({p['length']} bytes)")
...
def main():
    ...
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: sanitize.py <file_or_directory> [--verify]"}))
        sys.exit(1)
    ...
    if verify_only:
        ...
        print(json.dumps({
            "results": results,
            "summary": {"total": total, "clean": clean_count, "dirty": total - clean_count}
        }))
    else:
