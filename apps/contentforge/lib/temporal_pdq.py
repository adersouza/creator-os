...
def main():
    if len(sys.argv) < 3:
        print(json.dumps({"error": "Usage: temporal_pdq.py <source_video> <output_dir> [max_files]"}))
        sys.exit(1)
    ...
    if src_ext not in ('.mp4', '.mov', '.webm', '.avi'):
        print(json.dumps({"available": False, "reason": "Image mode — temporal hashing requires video"}))
        sys.exit(0)
    ...
    if len(src_frames) < 3:
        print(json.dumps({"available": False, "reason": "Source video too short for temporal analysis"}))
        sys.exit(0)

    src_hashes = compute_frame_hashes(src_frames)
    src_descriptor = compute_level1_descriptor(src_hashes)
    if src_descriptor is None:
        print(json.dumps({"available": False, "reason": "Failed to compute source temporal descriptor"}))
        sys.exit(0)
    ...
    for fname in files:
        ...
        if len(var_frames) < 3:
            results.append({"name": fname, "similarity": None, "verdict": None, "error": "Too short"})
            continue
        ...
        if var_descriptor is None:
            results.append({"name": fname, "similarity": None, "verdict": None, "error": "Hash failed"})
            continue
