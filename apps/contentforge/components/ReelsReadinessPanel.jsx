...
function ReadinessSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      {[0, 1, 2, 3].map((item) => (
        <div key={item} className="bg-[#08080c] rounded-lg border border-border p-3 animate-pulse">
          <div className="h-2 w-24 bg-[#20202a] rounded mb-3" />
          <div className="h-3 w-full bg-[#16161e] rounded mb-2" />
          <div className="h-3 w-2/3 bg-[#16161e] rounded" />
        </div>
      ))}
    </div>
  );
...
export default function ReelsReadinessPanel({ runId, mediaType, sourceFile }) {
  ...
  return (
    <div className="bg-card rounded-card border border-border p-5 flex flex-col gap-5">
      ...
      {loading && !data && <ReadinessSkeleton />}
