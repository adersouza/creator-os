...
def _touch_dir(path: Path, ts: datetime) -> None:
    path.mkdir(parents=True, exist_ok=True)
    (path / "payload.bin").write_bytes(b"ok")
    epoch = ts.timestamp()
    os.utime(path / "payload.bin", (epoch, epoch))
...
def test_ops_digest_reports_healthy_runtime_snapshot(tmp_path: Path) -> None:
    ...
    _sqlite(
        ...
        [
            "CREATE TABLE audio_catalog (imported_at TEXT)",
    ...
    _touch_dir(repo / "backups" / "runtime" / "latest", now - timedelta(hours=1))
    ticks = (
        repo
        / "python_packages"
        / "reel_factory"
    ...
    result = ops_digest_module.digest(repo, ops_log, now=now)
...
def test_ops_digest_escalates_stale_sync_backup_and_audio(tmp_path: Path) -> None:
    ...
    _touch_dir(repo / "backups" / "runtime" / "old", now - timedelta(days=3))
