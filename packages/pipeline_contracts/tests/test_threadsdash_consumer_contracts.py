...
def _threadsdash_root() -> Path:
    return Path(os.environ.get("THREADSDASH_ROOT", "/Users/aderdesouza/Developer/ThreadsDashboard"))
...
def test_threadsdashboard_common_schemas_match_canonical(threadsdash_contracts_root: Path) -> None:
...
def test_threadsdashboard_example_payloads_validate_against_canonical(
    ...
    dashboard_schema_dir = threadsdash_contracts_root / "schemas"
    examples = sorted(
        path for path in dashboard_schema_dir.glob("*.example.json")
...
def test_threadsdashboard_ingest_imports_pipeline_contracts(threadsdash_contracts_root: Path) -> None:
    ingest = threadsdash_contracts_root.parent / "api" / "_lib" / "handlers" / "campaign-factory" / "draftIngest.ts"

    assert 'from "../../../../pipeline_contracts/typescript.js"' in ingest.read_text(encoding="utf-8")
