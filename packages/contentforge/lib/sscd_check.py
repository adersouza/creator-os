...
def main():
    if len(sys.argv) < 3:
        print(
            json.dumps(
                {"error": "Usage: sscd_check.py <source_file> <output_dir> [max_files]"}
            )
        )
        sys.exit(1)
    ...
    if not os.path.exists(MODEL_PATH):
        print(
            json.dumps(
                {"error": f"SSCD model not found at {MODEL_PATH}. Download from Meta."}
            )
        )
        sys.exit(1)
    ...
    for fname in files:
        ...
        if err:
            results.append(
                {"name": fname, "similarity": None, "verdict": None, "error": err}
            )
            continue
