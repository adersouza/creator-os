...
def check_c2pa(filepath):
    ...
    try:
        ...
        return {
            ...
            "signals": signals,
            "detail": "C2PA manifest found"
            + (" — AI generation detected" if is_ai else " — no AI signals"),
        }
        ...
        if (
            "no manifest" in err_str.lower()
            or "not found" in err_str.lower()
            or "jumbf" in err_str.lower()
        ):
            return {"found": False, "detail": "No C2PA manifest"}
...
def check_iptc_xmp(filepath):
    ...
    try:
        ...
        return {
            ...
            "signals": signals,
            "metadata": metadata,
            "detail": "IPTC/XMP metadata found"
            + (" — AI signals detected" if is_ai else " — clean"),
        }
...
def check_png_chunks(filepath):
    ...
    try:
        ...
        return {
            ...
            "signals": ai_signals,
            "detail": "SD/ComfyUI parameters detected in PNG chunks"
            if ai_signals
            else "No AI chunks in PNG",
        }
    except Exception as e:
    ...
    try:
        ...
        if proc.returncode != 0 or not proc.stdout.strip():
            return {
                "found": False,
                "available": False,
                "detail": "ffprobe metadata unavailable",
            }
        ...
        has_creation_time = any(
            "creation_time" in key.lower() and value for key, value in tags.items()
        )
        has_encoder = any(
            "encoder" in key.lower() and value for key, value in tags.items()
        )
        has_handler = any(
            "handler_name" in key.lower() and value for key, value in tags.items()
        )
        ...
        return {
            ...
            "detail": "Container metadata inspected"
            + (" — AI signals detected" if signals else ""),
        }
    except Exception as e:
...
def main():
    if len(sys.argv) < 2:
        print(
            json.dumps(
                {"error": "Usage: provenance_check.py <file_or_directory> [max_files]"}
            )
        )
        sys.exit(1)
    ...
    for filepath in files:
        ...
        report["checks"].append(
            {"name": "c2pa", "label": "C2PA Manifest", **c2pa_result}
        )
        ...
        report["checks"].append(
            {"name": "iptc", "label": "IPTC/XMP Metadata", **iptc_result}
        )
        ...
        if filepath.lower().endswith(".png"):
            ...
            report["checks"].append(
                {"name": "png_chunks", "label": "PNG AI Chunks", **png_result}
            )
        ...
        json.dumps(
            {
                ...
                "summary": {
                    ...
                    "passRate": round(
                        ((len(results) - flagged_count) / len(results)) * 100
                    )
                    if results
                    else 0,
                },
