...
def cmd_add(target, max_files=500):
    ...
    if new_pdq:
        ...
        pdq_hashes = (
            np.vstack([pdq_hashes, new_pdq_array])
            if pdq_hashes.size > 0
            else new_pdq_array
        )
...
def cmd_query(target, max_files=50):
    ...
    if index.ntotal == 0:
        return {
            "error": "Reference index is empty. Add published content first.",
            "totalInIndex": 0,
        }
    ...
    for filepath in files:
        ...
        if pdq is not None and pdq_hashes.size > 0:
            ...
            pdq_closest = (
                ref_files[min_idx] if min_idx < len(ref_files) else f"index_{min_idx}"
            )

        top_sim = matches[0]["similarity"] if matches else None
        overall = (
            "fail"
            if (top_sim and top_sim >= 0.75)
            or (pdq_min_dist is not None and pdq_min_dist <= 30)
            else "warn"
            if (top_sim and top_sim >= 0.50)
            or (pdq_min_dist is not None and pdq_min_dist <= 60)
            else "pass"
        )
...
def main():
    if len(sys.argv) < 2:
        print(
            json.dumps(
                {
                    "error": "Usage: reference_db.py <add|query|stats|clear> [path] [max_files]"
                }
            )
        )
        sys.exit(1)
