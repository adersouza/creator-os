...
import { buildVideoAnalysisGate } from "./video-analysis-gate.js";
import { getQualityMetrics } from "./quality-metrics.js";
import { getQaSignals } from "./reels.js";
...
export async function POST(request) {
  ...
  try {
    ...
    if (!sourcePath) {
      return Response.json({ error: "Missing or invalid source path" }, { status: 400 });
    }
    ...
    try { entries = await readdir(finalDir); } catch {
      return Response.json({ error: "No output files found" }, { status: 404 });
    }
    ...
    if (!Array.isArray(requestedComparisonFiles) || requestedComparisonFiles.some(function (file) {
      ...
      return Response.json({ error: "comparisonFiles must be an array of filenames" }, { status: 400 });
    }
    ...
    if (targetFile) {
      ...
      if (!safeTarget || safeTarget !== String(targetFile).replace(/^output\/final\//, "")) {
        return Response.json({ error: "Invalid targetFile" }, { status: 400 });
      }
      files = files.filter(function (file) { return file === safeTarget; });
      if (files.length === 0) {
        return Response.json({ error: "Target file not found" }, { status: 404 });
      }
      for (var requestedComparison of requestedComparisonFiles) {
        ...
        if (!safeComparison || safeComparison !== requestedComparison || safeComparison === safeTarget) {
          return Response.json({ error: "Invalid comparisonFiles entry" }, { status: 400 });
        }
        if (!allFiles.includes(safeComparison)) {
          return Response.json({ error: "Comparison file not found: " + safeComparison }, { status: 400 });
        }
      ...
      return Response.json({ error: "comparisonFiles requires targetFile" }, { status: 400 });
    }

    if (files.length === 0) {
      return Response.json({ error: "No variant files found" }, { status: 404 });
    }
