...
This audit was produced by four parallel read-only passes (seam, crons, backend, schema) and then **reconciled against the live database** — several agent findings were corrected by direct SQL (noted inline). The reconciliations are the most load-bearing part; read them.
...
**[HIGH] A failed TD write can still be recorded as a successful export.**
`threadsdash.py` (~:611) records the export in `threadsdash_exports`; if TD validates then fails the DB write, the HTTP error is raised but the export can surface with **empty `post_ids` and no reconciliation** confirming the draft actually crossed. Nothing re-checks "did this draft become a post?" **Action:** treat empty-`postIds`-on-2xx and HTTP-error as the same retriable state; add a reconciliation read (count posts by `campaign_factory_post_key` / export id) before marking exported.
...
**[HIGH] `cross-reply-publish.ts` returns `200` on unhandled error — masks QStash retry.**
Line 63 returns `status(200).json({ok:true, error})` in the catch, so QStash sees success and **never retries or dead-letters** a genuinely failed reply. Engagement surface (lower blast radius than main publish) but failures are invisible. **Action:** return 5xx on retry-eligible errors.
...
3. **Index diet on `posts` + `post_metric_history`** — drop the 11 unused + dedupe the metric-history pairs. Single biggest DB perf/write-amp win (§4 HIGH-1/2).
