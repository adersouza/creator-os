...
**Execution update (2026-06-18):** TD infra hardening is now an open PR stack, not just an audit. CLEAN at refresh time: TD #131–#143 (A1, A3, C1–C6, D1, B1, B2). Open with Vercel pending/unstable at refresh time: TD #144–#157 (B3, B4, B5, D2, D3, D4, D5, D6). D3 unused-index pruning has started as bounded TD PRs, not a single broad migration: TD #147 (`campaign_factory_*`), TD #152 (`scheduler_decisions`), TD #153 (`meta_api_usage_snapshots`), TD #154 (`competitor_top_posts`), TD #155 (`account_dna*`), TD #156 (`ai_eval_snapshots`), and TD #157 (`operator_action_audit_logs`). The D3 finding remains open until the broader unused-index backlog is pruned or explicitly deferred, but these table-family batches should not be duplicated.
...
3. **Index diet on `posts` + `post_metric_history`** — drop the 11 unused + dedupe the metric-history pairs. Single biggest DB perf/write-amp win (§4 HIGH-1/2). Continue D3 separately as table-family batches; TD #147/#152/#153/#154/#155/#156/#157 are already open and should not be duplicated.
