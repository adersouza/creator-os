...
1. **[priority] Fix flaky `campaignSchedule` tests.** `tests/unit/campaignSchedule.test.ts` (in ThreadsDashboard source; mirrored at `apps/dashboard/`) has 3 non-deterministic tests that intermittently red the `javascript` gate: "reports campaign schedule rows with qstash state and duplicate counts", "surfaces duplicate visual risk in manager reports", "buckets accounts by safe, scheduled, and blocked states without overlap". They depend on wall-clock "now" relative to scheduled times. Fix in **ThreadsDashboard source** (inject/freeze a clock; make the time window deterministic), get the TD suite green deterministically (run several times), merge to TD main per its rules, then re-sync the dashboard mirror in creator-os. Flaky tests in a required gate can randomly block merges — this is the one real blemish.
