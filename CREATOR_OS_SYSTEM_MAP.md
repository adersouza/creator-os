...
| Higgsfield submission ambiguous without an ID | None | Safely blocked; automatic provider-history reconciliation is not proven, so investigate the provider account manually. |
...
correlation ID. A missing generation ID remains a manual reconciliation gap,
