...
| `reel_factory` | `codex/split-review-truth` | `main` | `4bad3acab` | behind 0, ahead 2 | `origin/codex/split-review-truth`, behind 0, ahead 0 after fetch | generated `uv.lock` | `git diff --check` pass; `env UV_CACHE_DIR=/private/tmp/codex-uv-cache uv run --extra dev python -m pytest tests` pass: 307 passed, 48 warnings. User-named `codex/review-truth-port@9b7f61a` was also tested fresh and is red: 4 collection errors from missing `pilmoji`; not mergeable under the hard safety rail. |
...
1. `campaign_factory`, `reel_factory/codex/split-review-truth`, and `pipeline_contracts`: branch heads are green and pre-merge records are staged in `MERGE_PLAN.md`; merge and ancestor proof remain pending.
2. `reel_factory/codex/review-truth-port@9b7f61a`: red in the fresh suite with missing `pilmoji`, so it remains unmerged. The green `codex/split-review-truth@4bad3acab` branch is the superset selected for merge.
...
- Step D split repo ports: pre-merge gates green for `campaign_factory/codex/campaign-caption-inventory@99f27208c`, `reel_factory/codex/split-review-truth@4bad3acab`, and `pipeline_contracts/codex/campaign-draft-contract-sync@e45374ab`; pending integration merges and creator-os mirror re-sync/re-pin afterward. `reel_factory/codex/review-truth-port@9b7f61a` is red and not mergeable.
