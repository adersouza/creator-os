...
| `ThreadsDashboard` | `main` after merge | `main` | merge `7c3757574`; branch tip `e77466ede` | `main` pushed to `origin/main` | `origin/main`, behind 0, ahead 0 after push | clean in main merge worktree after push | `git diff --check` pass; `npm run typecheck` pass; `npm run compat:check` pass; `npm run test` pass: 359 files passed, 1 skipped; 4701 passed, 1 skipped, 3 todo; `npm run build` pass; `npm run test:e2e:critical` pass with local `npm run dev` server: 5 passed, 1 skipped; `graphify update .` pass; ancestor proof `git merge-base --is-ancestor e77466edecd1891962b7ebb52047960b1c284e81 main` returned 0; stray hotfix branches checked and not folded because they are UI-only unique branches that would roll back control-plane files if merged wholesale |
