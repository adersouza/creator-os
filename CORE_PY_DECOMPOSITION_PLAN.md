...
| PR 10 — Decision ledger repository extraction | Ready for PR on `codex/core-decision-ledger-extraction` | Extracts decision-ledger preview/report/summary/filter helpers, decision entry construction, decision dedupe, supported-type metadata, obligation reason/explanation helpers, and story-goal mapping into `campaign_factory.decision_ledger.DecisionLedgerRepository`, composed by `CoreServices` while keeping the public `CampaignFactory` decision-ledger facade and private compatibility helpers unchanged. Verified with a RED missing-module failure first, focused extraction facade tests (15 passed), focused decision-ledger behavior tests (4 passed, 394 deselected, 1 existing Starlette/httpx warning), focused characterization tests (2 passed), and full Campaign Factory pytest (462 passed, 1 existing Starlette/httpx warning). |
