...
| PR 11 — Exception repository extraction | Ready for PR on `codex/core-exceptions-extraction` | Extracts persisted trust-exception creation, lookup, filtering, trust summary, status transitions, and payload construction into `campaign_factory.exceptions.ExceptionRepository`, composed by `CoreServices` while keeping the public `CampaignFactory` exception facade and private compatibility helpers unchanged. Verified with a RED missing-module failure first, focused extraction facade tests (17 passed), focused persisted-exception behavior tests (2 passed, 396 deselected, 1 existing Starlette/httpx warning), focused queue-adjacent behavior tests (3 passed, 395 deselected, 1 existing Starlette/httpx warning), focused characterization tests (2 passed), and full Campaign Factory pytest (464 passed, 1 existing Starlette/httpx warning). |
