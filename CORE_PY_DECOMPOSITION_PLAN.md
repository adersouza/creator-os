...
| PR 12 — Discoverability repository extraction | Local verification on `codex/core-discoverability-extraction` | Extracts discoverability safe-content contract, intake/generation/pre-render gates, violation origin mapping, prevention reports, discoverability loss/waterfall reporting, and captured/observed evidence helpers into `campaign_factory.discoverability.DiscoverabilityRepository`, composed by `CoreServices` while keeping the public `CampaignFactory` discoverability facade and private compatibility helpers unchanged. Verified with a RED missing-module failure first, focused extraction facade tests (19 passed), focused discoverability behavior tests (7 passed, 391 deselected, 1 existing Starlette/httpx warning), focused characterization tests (2 passed), and full Campaign Factory pytest (466 passed, 1 existing Starlette/httpx warning). |
