...
| PR 14 — Carousel integrity repository extraction | Local verification on `codex/core-carousel-integrity-extraction` | Extracts carousel integrity reports, child metrics plans, carousel report asset filtering, slide signature comparison, boundary preservation checks, and Meta child payload previews into `campaign_factory.carousel_integrity.CarouselIntegrityRepository`, composed by `CoreServices` while keeping the public `CampaignFactory` carousel facades and private compatibility helpers unchanged. Surface handoff readiness and draft proof remain in `core.py` and are injected as callables. Verified with a RED missing-module failure first, focused extraction facade tests (23 passed), focused carousel behavior tests (9 passed, 389 deselected, 1 existing Starlette/httpx warning), focused characterization tests (2 passed), and full Campaign Factory pytest (470 passed, 1 existing Starlette/httpx warning). |
