...
account/Instagram account as context, so account-bound drafts can carry
...
Follow-up reference-only planning proof is fixed on
`codex/reference-only-decision-evidence`: when no rendered assets exist,
`recommend_next_batch` still emits audio recommendations, audio selection
status, readiness blockers, and `decisionEvidence` for the reference-pattern
generation fallback. Verification:
`test_reference_only_recommendation_explains_what_to_make_next`.
Follow-up publishability decision proof is fixed on
`codex/recommendation-publishability-decision-proof`: recommendation
`decisionEvidence.readiness` now carries exact publishability failure reasons
...
Follow-up readiness verdict proof is fixed on
`codex/recommendation-readiness-verdict-proof`: recommendation
`decisionEvidence.readiness` now carries a manager-readable verdict, next
action, review/audit state, content surface, and latest audit verdict, so
operators can see whether the item is ready and what to fix next without
parsing raw blockers. Verification:
...
`test_recommend_next_batch_explains_account_audio_caption_decision`, and
`test_reference_only_recommendation_explains_what_to_make_next`.
Follow-up proof-checklist decision proof is fixed on
`codex/recommendation-proof-checklist`: recommendation
`decisionEvidence.proofChecklist` now summarizes account fit, learning, audio,
caption, quality, variation safety, and readiness proof in one operator-facing
block for both rendered-asset and reference-only recommendations. Verification:
`test_recommend_next_batch_explains_readiness_for_blocked_asset`,
`test_recommend_next_batch_explains_account_audio_caption_decision`, and
`test_reference_only_recommendation_explains_what_to_make_next`.
...
3. **Learned intelligence dead-ends as an operator note.** Recommendation arms now rank with explicit decayed Beta-Bernoulli planning stats, selected reference-pattern structure feeds prompt assembly, supplied video-analysis sidecars feed Winner-DNA features, Reference Factory visual clusters can be backed by local embeddings, and generated report sidecars can flow into readiness and Winner-DNA extraction.
...
| Sev | File:line | Fix |
|-----|-----------|-----|
| Fixed | `reference_factory/outcomes.py`, `public_metrics.py`, `patterns.py`, `learning.py`, `embeddings.py` | Reference Factory now imports measured prompt outcomes into `generated_video_prompts`, ranks public posts by measured reward before follower-normalized public rate/raw plays, embeds `measuredOutcome` in pattern metrics, carries measured reward samples into learning-cluster performance signals, and adds local DINOv2/timm visual embedding clusters with cached sidecars plus explicit heuristic fallback. Regression tests prove a lower-raw-play reference with better measured reward outranks a raw-volume winner, and the embedding slice groups similar references while preserving the Campaign Factory bank shape. |
...
| Fixed | `campaign_factory/recommendations.py` proof checklist | Recommendation decision evidence now includes `proofChecklist` summarizing account fit, learning, audio, caption, quality, variation safety, and readiness proof for rendered and reference-only paths. Regression tests: `test_recommend_next_batch_explains_readiness_for_blocked_asset`, `test_recommend_next_batch_explains_account_audio_caption_decision`, `test_reference_only_recommendation_explains_what_to_make_next`. |
...
- **Learning-loop data volume.** F v2 (median-norm + decay + Thompson) is a *better estimator*, not *more data*. Per-arm reward is small-n; reopen stochastic exploration when `campaign-factory closed-loop-learning-status` reports at least 50 Campaign Factory posts with both 1h and 24h metric-history rows.
